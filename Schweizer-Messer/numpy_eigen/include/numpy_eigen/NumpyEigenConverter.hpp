...
struct NumpyEigenConverter {
    ...
    static std::string toString() {
        return std::string() + "Eigen::Matrix<" + TypeToNumPy<scalar_t>::typeString() + ", " +
               castSizeOption(RowsAtCompileTime) + ", " + castSizeOption(ColsAtCompileTime) + ", " +
               boost::lexical_cast<std::string>((int)Options) + ", " + castSizeOption(MaxRowsAtCompileTime) + ", " +
    ...
    static void* convertible(PyObject* obj_ptr) {
        ...
        if (nd != 1 && nd != 2) {
            THROW_TYPE_ERROR("Conversion is only valid for arrays with 1 or 2 dimensions. Argument has "
                             << nd << " dimensions");
        }
    ...
    static void register_converter() {
        ...
        boost::python::converter::registry::push_back(
            &NumpyEigenConverter::convertible, &NumpyEigenConverter::construct, boost::python::type_id<matrix_t>());
    }
};
