...
#define SM_THROW_SFP(exceptionType, SourceFilePos, message)                                   \
    {                                                                                         \
        std::stringstream sm_assert_stringstream;                                             \
        sm_assert_stringstream << message;                                                    \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", SourceFilePos, \
                                                      sm_assert_stringstream.str());          \
    }
...
#define SM_ASSERT_LT(exceptionType, value, upperBound, message)                                                   \
    if ((value) >= (upperBound)) {                                                                                \
        std::stringstream sm_assert_stringstream;                                                                 \
        sm_assert_stringstream << "assert(" << #value << " < " << #upperBound << ") failed [" << (value) << " < " \
                               << (upperBound) << "]: " << message;                                               \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__,  \
                                                      sm_assert_stringstream.str());                              \
    }
...
#define SM_ASSERT_GE(exceptionType, value, lowerBound, message)                                                  \
    if ((value) < (lowerBound)) {                                                                                \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "assert(" << #value << " >= " << #lowerBound << ") failed [" << (value)        \
                               << " >= " << (lowerBound) << "]: " << message;                                    \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
...
#define SM_ASSERT_EQ(exceptionType, value, testValue, message)                                                   \
    if ((value) != (testValue)) {                                                                                \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "assert(" << #value << " == " << #testValue << ") failed [" << (value)         \
                               << " == " << (testValue) << "]: " << message;                                     \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
...
#define SM_ASSERT_NEAR(exceptionType, value, testValue, abs_error, message)                                      \
    if (!(fabs((testValue) - (value)) <= fabs(abs_error))) {                                                     \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "assert(" << #value << " == " << #testValue << ") failed [" << (value)         \
                               << " == " << (testValue) << " (" << fabs((testValue) - (value)) << " > "          \
                               << fabs(abs_error) << ")]: " << message;                                          \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
...
#define SM_ASSERT_GE_LT_DBG(exceptionType, value, lowerBound, upperBound, message)                               \
    if ((value) < (lowerBound) || (value) >= (upperBound)) {                                                     \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "debug assert(" << #lowerBound << " <= " << #value << " < " << #upperBound     \
                               << ") failed [" << (lowerBound) << " <= " << (value) << " < " << (upperBound)     \
                               << "]: " << message;                                                              \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
...
#define SM_ASSERT_LT_DBG(exceptionType, value, upperBound, message)                                              \
    if ((value) >= (upperBound)) {                                                                               \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "debug assert(" << #value << " < " << #upperBound << ") failed [" << (value)   \
                               << " < " << (upperBound) << "]: " << message;                                     \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
...
#define SM_ASSERT_GE_DBG(exceptionType, value, lowerBound, message)                                              \
    if ((value) < (lowerBound)) {                                                                                \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "debug assert(" << #value << " >= " << #lowerBound << ") failed [" << (value)  \
                               << " >= " << (lowerBound) << "]: " << message;                                    \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
...
#define SM_ASSERT_EQ_DBG(exceptionType, value, testValue, message)                                               \
    if ((value) != (testValue)) {                                                                                \
        std::stringstream sm_assert_stringstream;                                                                \
        sm_assert_stringstream << "debug assert(" << #value << " == " << #testValue << ") failed [" << (value)   \
                               << " == " << (testValue) << "]: " << message;                                     \
        sm::detail::sm_throw_exception<exceptionType>("[" #exceptionType "] ", __FUNCTION__, __FILE__, __LINE__, \
                                                      sm_assert_stringstream.str());                             \
    }
