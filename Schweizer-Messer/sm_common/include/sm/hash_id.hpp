...
class HashId {
  public:
     ...
     */
    inline HashId() { setInvalid(); }
    /**
     * Copy constructor
     */
    inline HashId(const HashId& other) { *this = other; }
    ...
    inline const std::string hexString() const {
        ...
        for (size_t i = 0; i < sizeof(val_); ++i) {
            ss << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(val_.c[i]);
        }
        return ss.str();
    }
    ...
    inline void operator=(const HashId& other) { memcpy(&val_, &other.val_, sizeof(val_)); }
    ...
    inline bool operator!=(const HashId& other) const { return !(*this == other); }

    /**
     * Invalidation mechanism
     */
    inline void setInvalid() { memset(&val_, 0, sizeof(val_)); }
    inline bool isValid() const { return val_.u64[0] != 0 || val_.u64[1] != 0; }
...
struct hash<sm::HashId> {
    ...
    value_type operator()(const argument_type& hashId) const { return std::hash<std::string>()(hashId.hexString()); }
};
}  // namespace std
