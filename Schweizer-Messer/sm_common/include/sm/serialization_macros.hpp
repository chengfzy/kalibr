...
template <class T>
sm_serialization::internal_types::yes check(const T&);
...
class HasOStreamOperator {
  ...
  public:
    enum { value = sizeof(check(stream << x)) == sizeof(sm_serialization::internal_types::yes) };
};
...
class HasIsBinaryEqual {
  ...
  public:
    enum { value = (int)((sizeof(func<T>(0)) == sizeof(char)) || (sizeof(funcconst<T>(0)) == sizeof(char))) };
};
template <typename T>
...
struct isSame<true, A> {
    static bool eval(const A& lhs, const A& rhs) { return lhs.isBinaryEqual(rhs); }
};
...
struct isSame<false, A> {
    static bool eval(const A& lhs, const A& rhs) { return lhs == rhs; }
};
...
struct checkTypeIsNotOpencvMat<cv::Mat, B> {
    ...
    BOOST_STATIC_ASSERT_MSG((B == !B) /*false*/,
                            "You cannot use the macro SM_CHECKSAME or SM_CHECKSAMEMEMBER on opencv mat. Use "
                            "sm::opencv::isBinaryEqual instead");
};
...
struct streamIf<false, A> {
    static std::string eval(const A&) { return "NA"; }
};
...
struct VerboseChecker {
    ...
    static bool Verbose() { return Instance().verbose_; }
...
#define IS_CHECKSAME_CURRENTLY_VERBOSE (sm::serialization::internal::VerboseChecker::Verbose())

#define SET_CHECKSAME_VERBOSITY(verbose) sm::serialization::internal::VerboseChecker::SetVerbose(verbose)
...
#define SM_SERIALIZATION_CHECKSAME_VERBOSE(THIS, OTHER, VERBOSE) \
    SET_CHECKSAME_VERBOSITY(VERBOSE) && SM_SERIALIZATION_CHECKSAME_IMPL(THIS, OTHER)
...
#define SM_SERIALIZATION_CHECKSAME_IMPL(THIS, OTHER)                                                               \
    (sm::serialization::internal::checkTypeIsNotOpencvMat<                                                         \
     ...
     sm::serialization::internal::isSame<                                                                          \
         sm::serialization::internal::                                                                             \
             HasIsBinaryEqual</*first run the test of equality: either isBinaryEqual or op==*/                     \
        ...
        (IS_CHECKSAME_CURRENTLY_VERBOSE                                                                            \
             ? (std::cout << "*** Validation failed on " << #OTHER << ":\n"                                        \
                ...
                sm::serialization::internal::streamIf<                                                             \
                    sm::serialization::internal::HasOStreamOperator<                                               \
                        std::ostream, /*here we check whether operator<< is available*/                            \
                        typename sm::common::StripConstReference<decltype(OTHER)>::result_t>::value,               \
...
#define SM_SERIALIZATION_CHECKMEMBERSSAME_IMPL(OTHER, MEMBER)                                                      \
    ((sm::serialization::internal::checkTypeIsNotOpencvMat<                                                        \
     ...
     (sm::serialization::internal::isSame<                                                                         \
         ...
         typename sm::common::StripConstReference<decltype(MEMBER)>::result_t>::eval(this->MEMBER, OTHER.MEMBER))) \
        ? true                                                                                                     \
        : (IS_CHECKSAME_CURRENTLY_VERBOSE                                                                          \
               ? (std::cout                                                                                        \
                  ...
                  << "\nat " << __PRETTY_FUNCTION__ << " In: " << __FILE__ << ":" << __LINE__ << std::endl         \
                  << std::endl) &&                                                                                 \
                     false                                                                                         \
               : false)
...
#define SM_SERIALIZATION_MACRO_CHOOSER_SAME(...) \
    SM_SERIALIZATION_GET_4TH_ARG(__VA_ARGS__, SM_SERIALIZATION_CHECKSAME_VERBOSE, SM_SERIALIZATION_CHECKSAME_IMPL)
