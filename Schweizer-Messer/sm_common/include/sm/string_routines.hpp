...
inline void toStream(std::ostream& stream, ConstIterator begin, ConstIterator end, std::string delimiter = " ",
    ...
    if (i != end) {
        stream << *i;
        i++;
        for (; i != end; ++i) stream << delimiter << *i;
    }
    stream << postfix;
}
...
template <typename ConstIterator>
inline void paddedToStream(std::ostream& stream, int padLength, int decimalLength, ConstIterator begin,
                           ConstIterator end, std::string delimiter = " ", std::string prefix = "",
                           std::string postfix = "") {
    stream << prefix;
...
template <typename ConstIterator>
inline std::string paddedToString(int padLength, int decimalLength, ConstIterator begin, ConstIterator end,
                                  std::string delimiter = " ", std::string prefix = "", std::string postfix = "") {
