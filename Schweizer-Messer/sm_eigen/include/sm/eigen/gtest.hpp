...
void assertNear(const M1& A, const M2& B, T tolerance, sm::source_file_pos const& sfp,
...
template <typename M1, typename M2, typename T>
void expectNear(const M1& A, const M2& B, T tolerance, sm::source_file_pos const& sfp,
                std::string const& message = "") {
...
inline bool compareRelative(double a, double b, double percentTolerance, double* percentError = NULL) {
    ...
    if (diff > percentTolerance * 1e-2) {
        if (percentError) *percentError = diff * 100.0;
        return false;
    }
    return true;
}
