...
Eigen::Matrix<typename Eigen::internal::traits<Derived_>::Scalar, 3, 3> crossMx(Eigen::MatrixBase<Derived_> const &x) {
    return crossMx(x(0, 0), x(1, 0), x(2, 0));
}
...
extern template Eigen::Matrix<double, 3, 3> crossMx(
    const Eigen::MatrixBase<Eigen::Matrix<double, Eigen::Dynamic, 1> > &);
