...
EulerAnglesZXY::EulerAnglesZXY(const EulerAnglesZXY& other) : RotationalKinematics(other) {}
...
Eigen::Matrix3d EulerAnglesZXY::parametersToRotationMatrix(const Eigen::Vector3d& parameters,
                                                           Eigen::Matrix3d* S) const {
    if (S) *S = parametersToSMatrix(parameters);
    ...
    return (Eigen::Matrix3d() << cz * cy - sz * sx * sy, -sz * cx, cz * sy + sz * sx * cy, sz * cy + cz * sx * sy,
            cz * cx, sz * sy - cz * sx * cy, -cx * sy, sx, cx * cy)
        .finished();
}

Eigen::Vector3d EulerAnglesZXY::rotationMatrixToParameters(const Eigen::Matrix3d& rotationMatrix) const {
...
Eigen::Matrix3d EulerAnglesZXY::parametersToSMatrix(const Eigen::Vector3d& parameters) const {
    const double cx = cos(parameters(1));
    const double sx = sin(parameters(1));
    const double cz = cos(parameters(0));
    const double sz = sin(parameters(0));
    return (Eigen::Matrix3d() << 0, -cz, cx * sz, 0, -sz, -cx * cz, -1, 0, -sx).finished();
}

Eigen::Vector3d EulerAnglesZXY::angularVelocityAndJacobian(const Eigen::Vector3d& p, const Eigen::Vector3d& pdot,
