...
Eigen::Matrix3d EulerAnglesZYX::parametersToRotationMatrix(const Eigen::Vector3d& parameters,
    ...
    C << cz * cy, -sz * cx + cz * sy * sx, sz * sx + cz * sy * cx, sz * cy, cz * cx + sz * sy * sx,
        -cz * sx + sz * sy * cx, -sy, cy * sx, cy * cx;
...
Eigen::Matrix3d EulerAnglesZYX::buildSMatrix(double sz, double cz, double sy, double cy) const {
    Eigen::Matrix3d S;
    S << 0.0, -sz, cy * cz, 0.0, cz, cy * sz, 1.0, 0.0, -sy;

    return S;
}

Eigen::Vector3d EulerAnglesZYX::angularVelocityAndJacobian(const Eigen::Vector3d& p, const Eigen::Vector3d& pdot,
