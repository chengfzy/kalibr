...
HomogeneousPoint::HomogeneousPoint() : _ph(0.0, 0.0, 0.0, 1.0) {}

/// \brief Initialize the point from a Euclidean point
HomogeneousPoint::HomogeneousPoint(const Eigen::Vector3d& p) : _ph(p(0), p(1), p(2), 1.0) {}

/// \brief Initialize the point from a Homogeneous point
HomogeneousPoint::HomogeneousPoint(const Eigen::Vector4d& p) : _ph(p) {
    SM_ASSERT_GT_DBG(Exception, _ph.norm(), 0.0,
                     "The point " << p.transpose() << " is not a valid member of the set of homogeneous points");
}
...
HomogeneousPoint& HomogeneousPoint::operator=(const Eigen::Vector4d& rhs) {
    SM_ASSERT_GT_DBG(Exception, rhs.norm(), 0.0,
                     "The point " << rhs.transpose() << " is not a valid member of the set of homogeneous points");
    _ph = rhs;
...
void HomogeneousPoint::oplus(const Eigen::Vector3d& dp) { _ph = updateQuat(_ph, dp); }

/// \brief converts the homogenous point to a pointing vector by setting the homogeneous coordinate to zero
void HomogeneousPoint::convertToVector() { _ph[3] = 0.0; }
...
Eigen::Vector3d HomogeneousPoint::toEuclidean() const {
    SM_ASSERT_GT_DBG(
        Exception, fabs(_ph[3]), 0.0,
        "The point " << _ph.transpose()
                     << " can not be represented in Euclidean space. It is infinitely far away from the origin");

    double recip_p3 = 1.0 / _ph[3];
    return Eigen::Vector3d(_ph[0] * recip_p3, _ph[1] * recip_p3, _ph[2] * recip_p3);
}

/// \brief get the Euclidean representation and the jacobian of the oplus operator with respect to this point. The
...
const Eigen::Vector4d& HomogeneousPoint::toHomogeneous() const { return _ph; }

/// \brief get the homogeneous representation and the Jacobian of the oplus operator with respect to this point. The
/// Jacobian should be 4x3.
...
void HomogeneousPoint::normalize() { _ph = _ph / _ph.norm(); }
...
bool HomogeneousPoint::atInfinity() const { return (_ph[3] == 0.0); }

bool HomogeneousPoint::isVector() const { return atInfinity(); }

bool HomogeneousPoint::isBinaryEqual(const HomogeneousPoint& rhs) const { return SM_CHECKMEMBERSSAME(rhs, _ph); }
