...
UncertainHomogeneousPoint::UncertainHomogeneousPoint() : HomogeneousPoint() { _U.setZero(); }

/// \brief initialize the point with zero uncertainty
UncertainHomogeneousPoint::UncertainHomogeneousPoint(const Eigen::Vector3d& p) : HomogeneousPoint(p) { _U.setZero(); }

/// \brief initialize the point with zero uncertainty
UncertainHomogeneousPoint::UncertainHomogeneousPoint(const Eigen::Vector4d& p) : HomogeneousPoint(p) { _U.setZero(); }

/// \brief initialize the point with zero uncertainty
UncertainHomogeneousPoint::UncertainHomogeneousPoint(const HomogeneousPoint& p) : HomogeneousPoint(p) { _U.setZero(); }
...
const Eigen::Matrix4d& UncertainHomogeneousPoint::U4() const { return _U; }
...
Eigen::Matrix3d UncertainHomogeneousPoint::U_av_form() const {
    SM_ASSERT_NEAR(HomogeneousPoint::Exception, _ph.norm(), 1.0, 1e-6,
                   "The \"oplus\" form of covariance is only valid for points on the unit sphere in R^4");
...
void UncertainHomogeneousPoint::setU(const Eigen::Matrix4d& U) { _U = U; }
...
UncertainHomogeneousPoint UncertainHomogeneousPoint::operator+(const UncertainHomogeneousPoint& rhs) const {
    ...
    return UncertainHomogeneousPoint(rval,
                                     (Eigen::Matrix4d)(PR * U4() * PR.transpose() + PL * rhs.U4() * PL.transpose()));
}
