...
Eigen::MatrixXd fromHomogeneousColumns(const Eigen::MatrixXd& hM) {
    ...
    Eigen::Matrix<double, 1, Eigen::Dynamic> denom(Eigen::Matrix<double, 1, Eigen::Dynamic>::Ones(hM.cols()).array() /
                                                   hM.row(hM.rows() - 1).array());
...
Eigen::Matrix4d toHomogeneousPlus(const Eigen::Vector4d& ph) {
    Eigen::Matrix4d rval;
    rval << ph[3], 0.0, 0.0, ph[0], 0.0, ph[3], 0.0, ph[1], 0.0, 0.0, ph[3], ph[2], 0.0, 0.0, 0.0, ph[3];
