...
TEST(QuaternionAlgebraTestSuite, testRotation) {
    try {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            sm::eigen::assertNear(v_a1, v_a2, 1e-10, SM_SOURCE_FILE_POS,
                                  "The rotation matrix And shortcut rotations are not equal");
        }
...
class Stringer {
  public:
    ...
    operator const std::string() { return ss.str(); }
...
TEST(QuaternionAlgebraTestSuite, testAxisAngle2QuatAndBack) {
    ...
    try {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            sm::eigen::assertNear(q, axisAngle2quat(quat2AxisAngle(q)), q.block<3, 1>(0, 0).norm() * eps * 20,
                                  SM_SOURCE_FILE_POS, "");
            ...
            sm::eigen::assertNear(
                v_b, quat2AxisAngle(axisAngle2quat(v_b)), tolerance, SM_SOURCE_FILE_POS,
                Stringer() << "axisAngle2QuatAndBack is too little accurate : \naA2Q(v_b)=" << axisAngle2quat(v_b)
...
TEST(QuaternionAlgebraTestSuite, testExpJacobian) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        sm::eigen::assertNear(quatExpJacobian(v),
                              sm::eigen::numericalDiff<Eigen::Vector4d, Eigen::Vector3d>(
                                  [&v](const Eigen::Vector3d& x) { return axisAngle2quat(v + x); }, zero3),
                              1E-7, SM_SOURCE_FILE_POS, "");
    }
}
...
TEST(QuaternionAlgebraTestSuite, testLogJacobian) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        sm::eigen::assertNear(quatLogJacobian2(q),
                              sm::eigen::numericalDiff<Eigen::Vector3d, Eigen::Vector4d>(
                                  [&q](const Eigen::Vector4d& x) {
                                      return quat2AxisAngle(qplus(axisAngle2quat(2 * qeps(qplus(x, quatInv(q)))), q));
                                  },
                                  zero4),
                              1E-7, SM_SOURCE_FILE_POS, "");
        // TODO figure out how quatLogJacobian was meant to work and write test (the following does not work)
        // sm::eigen::assertNear(quatLogJacobian(q), sm::eigen::numericalDiff<Eigen::Vector3d, Eigen::Vector4d >(
        // [&q](const Eigen::Vector4d & x) { return  quat2AxisAngle(qplus(axisAngle2quat(2 * qeps(qplus(x,
        // quatInv(q)))), q)); }, zero4), 1E-7, SM_SOURCE_FILE_POS,  "");
    }
}
