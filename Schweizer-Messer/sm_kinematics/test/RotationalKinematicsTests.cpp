...
struct SMatrixFunctor {
    ...
    SMatrixFunctor(const Eigen::Vector3d& pbar, const Eigen::Vector3d& v) : pbar_(pbar), v_(v) {
...
struct SMatrixFunctor2 {
    ...
    SMatrixFunctor2(const Eigen::Vector3d& pbar) : pbar_(pbar) { Cbar_ = rotation.parametersToRotationMatrix(pbar); }
