...
struct NormalizeJacobianFunctor {
    ...
    value_t operator()(const input_t& p) { return sm::kinematics::normalize(p); }
};
...
TEST(HomogeneousCoordinatesTestSuite, testPlus) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        sm::eigen::assertNear(p12, p1 + p2, 1e-6, SM_SOURCE_FILE_POS,
                              "the addition of homogeneous points by matrix multiplication");
    }
}
...
TEST(HomogeneousCoordinatesTestSuite, testMinus) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        sm::eigen::assertNear(p12, p2 - p1, 1e-6, SM_SOURCE_FILE_POS,
                              "the subtraction of homogeneous points by matrix multiplication");
    }
}
