...
void LoggingGlobals::shutdown() { _shutting_down = true; }
...
void LoggingGlobals::checkLogLocationEnabledNoLock(LogLocation* loc) {
    loc->_loggerEnabled =
        loc->_level >= _level && (loc->_streamName.size() == 0 || isNamedStreamEnabled(loc->_streamName));
}
...
void LoggingGlobals::print(const char* streamName, Level level, const char* file, int line, const char* function,
                           const char* fmt, ...) {
    ...
    try {
        _logger->log(
            LoggingEvent(streamName, level, file, line, function, &_globalCharBuffer[0], _logger->currentTimeString()));
...
void LoggingGlobals::print(const char* streamName, Level level, vectorstream& ss, const char* file, int line,
                           const char* function) {
    if (_shutting_down) return;
...
boost::shared_ptr<Logger> LoggingGlobals::getLogger() { return _logger; }

sm::logging::levels::Level getLevel() { return g_logging_globals.getLevel(); }
void setLevel(sm::logging::levels::Level level) { g_logging_globals.setLevel(level); }
void setLogger(boost::shared_ptr<Logger> logger) { g_logging_globals.setLogger(logger); }
boost::shared_ptr<Logger> getLogger() { return g_logging_globals.getLogger(); }
bool isNamedStreamEnabled(const std::string& name) { return g_logging_globals.isNamedStreamEnabled(name); }
void enableNamedStream(const std::string& name) { g_logging_globals.enableNamedStream(name); }
void disableNamedStream(const std::string& name) { g_logging_globals.disableNamedStream(name); }
