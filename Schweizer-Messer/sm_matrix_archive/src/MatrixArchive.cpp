...
Eigen::MatrixXd &MatrixArchive::createMatrix(std::string const &matrixName, int rows, int cols,
    ...
    if (!overwriteExisting) {
        ...
        if (it != m_values.end()) {
            SM_THROW(MatrixArchiveException,
                     "There is already a matrix of name \"" << matrixName << "\" in the archive");
        }
    }
...
void MatrixArchive::validateName(std::string const &name, sm::source_file_pos const & /* sfp */) const {
    ...
    SM_ASSERT_TRUE(MatrixArchiveException, isalpha(name[0]),
                   "The name \"" << name << "\" is invalid. The first character of the name must be a letter");

    for (unsigned i = 1; i < name.size(); i++) {
        SM_ASSERT_TRUE(MatrixArchiveException, isalnum(name[i]) || name[i] == '_',
                       "The name \"" << name
                                     << "\" is invalid. The characters of the name must be alphanumeric or an "
                                        "underscore. (failed at character "
                                     << i << ")");
    }
}
...
size_t MatrixArchive::size() const { return sizeStrings() + sizeMatrices(); }
size_t MatrixArchive::sizeStrings() const { return m_strings.size(); }
size_t MatrixArchive::sizeMatrices() const { return m_strings.size(); }

MatrixArchive::matrix_map_t::const_iterator MatrixArchive::begin() const { return m_values.begin(); }
...
MatrixArchive::BlockType MatrixArchive::readBlock(std::istream &fin, std::string &name, Eigen::MatrixXd &matrix,
    ...
    if (start == s_magicCharStartAStringBlock) {
        ...
        SM_ASSERT_EQ(MatrixArchiveException, start, s_magicCharStartAMatrixBlock,
                     "The block didn't start with the expected character");
        blockType = MATRIX;
    }
    ...
    // end character
    fin.read(&end, 1);
    SM_ASSERT_EQ(MatrixArchiveException, end, s_magicCharEnd,
                 "The matrix block didn't end with the expected character");
    return blockType;
}
...
void MatrixArchive::saveMatrices(std::ostream &fout, std::set<std::string> const &validNames) const {
    ...
    for (; it != m_values.end(); it++) {
        if (validNames.empty() || validNames.count(it->first) > 0) {
            ...
            SM_ASSERT_TRUE(MatrixArchiveException, fout.good(),
                           "Error while writing matrix " << it->first << " to file.");
        }
    }
}
void MatrixArchive::saveStrings(std::ostream &fout, std::set<std::string> const &validNames) const {
    ...
    for (; it != m_strings.end(); it++) {
        if (validNames.empty() || validNames.count(it->first) > 0) {
            ...
            SM_ASSERT_TRUE(MatrixArchiveException, fout.good(),
                           "Error while writing string " << it->first << " to file.");
        }
    }
}
