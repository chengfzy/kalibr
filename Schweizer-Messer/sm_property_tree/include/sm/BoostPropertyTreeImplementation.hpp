...
T BoostPropertyTreeImplementation::get(const std::string& key) const {
    ...
    try {
        ...
        SM_THROW(PropertyTree::InvalidValueException,
                 "Unable to get the value for key \"" << key << "\" with value \""
                                                      << _ptree.get<std::string>(ptree::path_type(key.substr(1), '/'))
                                                      << "\" as type " << typeid(T).name() << ": " << e.what());
