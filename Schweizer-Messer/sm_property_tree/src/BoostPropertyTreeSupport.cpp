...
/**
 * @brief Helper to find a file with name filenameToFind in the directory specified by an environment variable
 *envVarNameContainingSearchDir
 **/
boost::filesystem::path findFile(const std::string& filenameToFind, const std::string& envVarNameContainingSearchDir) {
    ...
    SM_ASSERT_TRUE(std::runtime_error, dir,
                   "Environment variable " << envVarNameContainingSearchDir << " could not be found");
    ...
    SM_ASSERT_TRUE(std::runtime_error, boost::filesystem::exists(fullPath),
                   "Environment variable " << envVarNameContainingSearchDir << " exists, but file " << filenameToFind
                                           << " could not be found in " << searchDirectory);
