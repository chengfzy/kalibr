...
PropertyTree::PropertyTree(boost::shared_ptr<PropertyTreeImplementation> imp, const std::string& baseNamespace)
    ...
    if (_namespace.size() > 0 && _namespace[0] != '/') _namespace = "/" + _namespace;
}

PropertyTree::PropertyTree(const PropertyTree& parent, const std::string& childNamespace) : _imp(parent._imp) {
...
std::string PropertyTree::buildQualifiedKeyName(const std::string& key) const {
    ...
    if (key[0] == '/') {
        ...
        SM_ASSERT_GT(InvalidKeyException, key.size(), 1,
                     "The key \"" << key << "\" is in the global namespace but does not contain a variable name");
        return key;
    } else {
