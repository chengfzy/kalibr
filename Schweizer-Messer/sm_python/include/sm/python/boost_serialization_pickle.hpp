...
struct pickle_suite : boost::python::pickle_suite {
    ...
    //
    static boost::python::tuple getinitargs(const T& /* val */) { return boost::python::tuple(); }

    static boost::python::tuple getstate(const T& val) {
    ...
    static void setstate(T& val, boost::python::tuple state) {
        boost::python::extract<std::string> str(state[0]);
        SM_ASSERT_TRUE(std::runtime_error, str.check(),
                       "Error during de-serialization. Unable to convert the tuple object to a string");
