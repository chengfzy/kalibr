...
void export_transformations() {
    ...
    def("rt2Transform", rt2Transform,
        "creates a transformation matrix T_ba from the rotation matrix C_ba and the translation rho_b_ab");
    ...
    def("transform2rhoHomogeneous", transform2rhoHomogeneous,
        "returns the translation p_b_ab from the transformation matrix T_ba");
    ...
    def("transformationAndJacobian", transformationAndJacobianTuple,
        "Transform a point and return the point and jacobian with respect to local perturbations of the transformation "
        "matrix. (v_a, H) = transformationAndJacobian(T_ab, v_b)");
    def("inverseTransformationAndJacobian", inverseTransformationAndJacobianTuple,
        "Transform a point (using the inverse of the supplied transformation matrix) and return the point and jacobian "
        "with respect to local perturbations of the transformation matrix. (v_b, H) = "
        "inverseTransformationAndJacobian(T_ab, v_a)");
}
