...
template <typename T>
typename TimestampCorrector<T>::time_t TimestampCorrector<T>::correctTimestamp(const time_t& remoteTime,
                                                                               const time_t& localTime) {
    // Make sure this point is forward in time.
    if (!_convexHull.empty()) {
        SM_ASSERT_GT(TimeWentBackwardsException, remoteTime, _convexHull[_convexHull.size() - 1].x,
                     "The correction algorithm requires that times are passed in with monotonically increasing remote "
                     "timestamps");
    }
    ...
    if (!isAboveTopLine(p)) {
        ...
        while (_convexHull.size() >= 2 &&
               isAboveLine(_convexHull[_convexHull.size() - 2], p, _convexHull[_convexHull.size() - 1])) {
    ...
    if (_convexHull.size() >= 3) {
        ...
        SM_ASSERT_LT_DBG(Exception, _midpointSegmentIndex, _convexHull.size() - 1,
                         "The computed midpoint segment is out of bounds. Elements in hull: "
                             ...
                             << ", End time: " << _convexHull[_convexHull.size() - 1].x << ", midpoint: " << midpoint);
...
double TimestampCorrector<T>::getSlope() const {
    SM_ASSERT_GE(NotInitializedException, _convexHull.size(), 2,
                 "The timestamp correction requires at least two data points before this funciton can be called");
...
bool TimestampCorrector<T>::isAboveTopLine(const Point& p) const {
    ...
    return isAboveLine(_convexHull[_convexHull.size() - 2], _convexHull[_convexHull.size() - 1], p);
}
