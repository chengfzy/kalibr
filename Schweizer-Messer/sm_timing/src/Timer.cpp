...
Timing::Timing() : m_maxTagLength(0) {
...
{
    SM_ASSERT_LT(
        TimerException, handle, Timing::instance().m_timers.size(),
...
bool Timer::isTiming() { return m_timing; }
...
double Timing::getTotalSeconds(size_t handle) {
    SM_ASSERT_LT(TimerException, handle, instance().m_timers.size(),
...
void Timing::reset(std::string const& tag) { return reset(getHandle(tag)); }
