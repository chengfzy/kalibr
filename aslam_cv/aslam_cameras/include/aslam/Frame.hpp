...
class Frame : public FrameBase {
  public:
    ...
    void setGeometry(const boost::shared_ptr<camera_geometry_t>& cameraGeometryPtr);
    ...
    virtual void setGeometryBase(const boost::shared_ptr<cameras::CameraGeometryBase>& geometry);
    ...
    /// \brief get the number of keypoints
    virtual size_t size() const { return numKeypoints(); }
    ...
    inline keypoint_t& keypointUnchecked(size_t i) { return _keypoints[i]; }

    /// \brief get keypoint i and do not check that i is in bounds (fast version)
    inline const keypoint_t& keypointUnchecked(size_t i) const { return _keypoints[i]; }

    /// \brief get keypoint i
    const keypoint_t& operator[](size_t i) const { return keypoint(i); }

    /// \brief get keypoint i
    keypoint_t& operator[](size_t i) { return keypoint(i); }
    ...
    virtual bool computeReprojectionError3(size_t i, const Eigen::Vector3d& p, double& outReprojectionError) const;

    /// \brief compute the reprojection error for this keypoint from a Homogeneous point
    ///        returns true if the reprojection was successful
    virtual bool computeReprojectionError4(size_t i, const Eigen::Vector4d& p, double& outReprojectionError) const;

    /// \brief compute the reprojection error for this keypoint from an uncertain Homogeneous point
    ///        returns true if the reprojection was successful
    virtual bool computeReprojectionErrorUhp(size_t i, const sm::kinematics::UncertainHomogeneousPoint& p,
    ...
    /// \brief compute the reprojection error for this keypoint from a Euclidean point
    ///        returns true if the reprojection was successful
    virtual bool computeReprojectionError3(const KeypointIdentifier& kid, const Eigen::Vector3d& p,
                                           double& outReprojectionError) const;
    /// \brief compute the reprojection error for this keypoint from a Homogeneous point
    ///        returns true if the reprojection was successful
    virtual bool computeReprojectionError4(const KeypointIdentifier& kid, const Eigen::Vector4d& ph,
                                           double& outReprojectionError) const;
    /// \brief compute the reprojection error for this keypoint from an uncertain Homogeneous point
    ///        returns true if the reprojection was successful
    virtual bool computeReprojectionErrorUhp(const KeypointIdentifier& kid,
    ...
    virtual bool computeProjection4(const Eigen::Vector4d& ph, Eigen::VectorXd& outReprojection) const;
    /// \brief compute the projection for a 3x1 point
    ///        returns true if the projection was successful
    virtual bool computeProjection3(const Eigen::Vector3d& ph, Eigen::VectorXd& outReprojection) const;
    /// \brief compute the projection for an uncertain homogeneous point
    ///        returns true if the projection was successful
    virtual bool computeProjectionUhp(const sm::kinematics::UncertainHomogeneousPoint& ph,
                                      Eigen::VectorXd& outReprojection) const;

    /// \brief get the keypoint associated with this identifier.
    void getKeypoint(const KeypointIdentifier& kid, keypoint_t& outKeypoint) const;
    ...
    virtual void getBackProjection(size_t i, BackProjection& outBackProjection) const;

    /// \brief get the back projection for keypoint kid
    virtual void getBackProjection(const KeypointIdentifier& kid, BackProjection& outBackProjection) const;

    /// \brief get the back projection for keypoint i
    virtual void getUncertainBackProjection(size_t i, UncertainBackProjection& outBackProjection) const;

    /// \brief get the back projection for keypoint kid
    virtual void getUncertainBackProjection(const KeypointIdentifier& kid,
    ...
    //         virtual bool getLandmark(const KeypointIdentifier & k, UncertainHomogeneousPoint & outLandmark) const =
    //         0; virtual bool getLandmark(size_t k, UncertainHomogeneousPoint & outLandmark) const = 0;
    ...
    /// \brief get the landmark for keypoint k. Return true on success
    virtual bool getLandmark(const KeypointIdentifier kid, Eigen::Vector4d& outLandmark) const;
    ...
    virtual const FrameId& id() const { return _id; }

    /// \brief set the frame id
    virtual void setId(const FrameId& id) { _id = id; }
    ...
    virtual const Time& time() const { return _stamp; }

    /// \brief set the base time for this frame
    virtual void setTime(const Time& time) { _stamp = time; }
    ...
    virtual const cv::Mat& image(int octave = 0) const { return _image->getOctave(octave); }

    /// \brief Set the image
    virtual void setImage(const cv::Mat& image, int octave = 0) { _image->getOctaveMutable(octave) = image; }

    virtual boost::shared_ptr<const aslam::Image> getOctaves() const { return _image; }

    virtual boost::shared_ptr<aslam::Image> getOctavesMutable() { return _image; }
