...
class FrameBase {
  public:
    ...
    virtual void setGeometryBase(const boost::shared_ptr<cameras::CameraGeometryBase>& geometry) = 0;
    ...
    virtual bool computeReprojectionError3(size_t i, const Eigen::Vector3d& p, double& outReprojectionError) const = 0;

    /// \brief compute the reprojection error for this keypoint from a Homogeneous point
    ///        returns true if the reprojection was successful
    virtual bool computeReprojectionError4(size_t i, const Eigen::Vector4d& p, double& outReprojectionError) const = 0;
    ...
    virtual bool computeProjection4(const Eigen::Vector4d& ph, Eigen::VectorXd& outReprojection) const = 0;
    /// \brief compute the reprojection for a 3x1 point
    ///        returns true if the projection was successful
    virtual bool computeProjection3(const Eigen::Vector3d& ph, Eigen::VectorXd& outReprojection) const = 0;
    ...
    virtual void getBackProjection(size_t i, BackProjection& outBackProjection) const = 0;

    /// \brief get the back projection for keypoint kid
    virtual void getBackProjection(const KeypointIdentifier& kid, BackProjection& outBackProjection) const = 0;

    /// \brief get the back projection for keypoint i
    virtual void getUncertainBackProjection(size_t i, UncertainBackProjection& outBackProjection) const = 0;
    ...
    /// \brief get the landmark for keypoint k. Return true on success
    virtual bool getLandmark(const KeypointIdentifier kid, Eigen::Vector4d& outLandmark) const = 0;
