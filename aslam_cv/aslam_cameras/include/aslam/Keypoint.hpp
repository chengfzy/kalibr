...
class Keypoint : public KeypointBase {
  public:
    ...
    enum {
        KeypointDimension =
            KEYPOINT_DIMENSION /*!< The dimension of the keypoint associated with this geometry policy */
    };
    ...
    virtual size_t dimension() const { return KeypointDimension; }
    ...
    void setInverseCovariance(const inverse_covariance_t& invR) { setInverseMeasurementCovariance(invR); }
  ...
  private:
    ...
    /// \brief unique ground truth ID for a keypoint trace (i.e. to easily verifiy 2D2D matching with ground truth
    /// simulation)
    size_t _traceId;
};
