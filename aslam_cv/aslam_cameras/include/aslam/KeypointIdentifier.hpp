...
struct KeypointIdentifier {
    KeypointIdentifier(MultiFrameId fi = MultiFrameId(), size_t ci = 0, size_t ki = 0)
    ...
    MultiFrameId getFrameId() { return frameId; }
    void setFrameId(MultiFrameId fid) { frameId = fid; }
    ...
    bool isBinaryEqual(const KeypointIdentifier& rhs) const {
        return frameId == rhs.frameId && cameraIndex == rhs.cameraIndex && keypointIndex == rhs.keypointIndex;
    }

    bool operator==(const KeypointIdentifier& rhs) const { return isBinaryEqual(rhs); }
...
inline std::ostream& operator<<(std::ostream& os, const aslam::KeypointIdentifier& kid) {
    os << "[" << kid.frameId << ", " << kid.cameraIndex << ", " << kid.keypointIndex << "]";
    return os;
}
