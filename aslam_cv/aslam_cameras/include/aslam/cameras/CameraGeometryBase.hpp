...
class CameraGeometryBase {
  public:
    ...
    // will be inefficient as it will call the fixed-size derived functions and copy the data into the variable size
    // matrices. Still, this base class is useful as it allows us to maintain a heterogeneous list of camera geometries
    // and simplify the python interface.
    ...
    virtual bool vsHomogeneousToKeypoint(const Eigen::Vector4d& ph, Eigen::VectorXd& outKeypoint) const = 0;
    ...
    virtual bool vsKeypointToEuclidean(const Eigen::VectorXd& keypoint, Eigen::Vector3d& outPoint) const = 0;

    virtual bool vsKeypointToEuclidean(const Eigen::VectorXd& keypoint, Eigen::VectorXd& outPoint,
                                       Eigen::MatrixXd& outJacobian) const = 0;

    virtual bool vsKeypointToHomogeneous(Eigen::VectorXd const& keypoint, Eigen::VectorXd& outPoint) const = 0;
    ...
    static boost::shared_ptr<CameraGeometryBase> create(const sm::PropertyTree& config);
    ...
    virtual void update(const double* v, bool projection, bool distortion, bool shutter) = 0;

    /// \brief Get the total number of dimensions of the intrinsic parameters
    virtual int minimalDimensions(bool projection, bool distortion, bool shutter) const = 0;

    /// \brief get the intrinsic parameters.
    virtual void getParameters(Eigen::MatrixXd& P, bool projection, bool distortion, bool shutter) const = 0;

    /// \brief set the intrinsic parameters.
    virtual void setParameters(const Eigen::MatrixXd& P, bool projection, bool distortion, bool shutter) = 0;
