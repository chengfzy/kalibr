 ...
 *
 * See "A Generic Camera Model and Calibration Method for Conventional, Wide-Angle, and Fish-Eye Lenses" by Juho Kannala
 * and Sami S. Brandt for further information
 *
 *
 ...
 *
 * This class represents a standard implementation of the distortion block. The function "distort" applies this
 * nonlinear transformation. The function "undistort" applies the inverse transformation. Note that the inverse
 * transformation in this case is not avaialable in closed form and so it is computed iteratively.
 *
 */
class EquidistantDistortion {
  public:
    ...
    void undistort(const Eigen::MatrixBase<DERIVED>& y, const Eigen::MatrixBase<DERIVED_JY>& outJy) const;

    /**
     * \brief Apply distortion to the point and provide the Jacobian of the distortion with respect to small changes in
    ...
    void distortParameterJacobian(const Eigen::MatrixBase<DERIVED_Y>& imageY,
                                  const Eigen::MatrixBase<DERIVED_JD>& outJd) const;

    /**
     * \brief A function for compatibility with the aslam backend. This implements an update of the distortion
     * parameter.
     *
     ...
     *
     * @param v The number of parameters expected by the update equation. This should also define the number of columns
     * in the matrix returned by distortParameterJacobian.
     */
