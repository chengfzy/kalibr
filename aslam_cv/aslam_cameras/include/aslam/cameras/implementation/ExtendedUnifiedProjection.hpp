...
ExtendedUnifiedProjection<DISTORTION_T>::ExtendedUnifiedProjection()
    : _alpha(0.0), _beta(0.0), _fu(0.0), _fv(0.0), _cu(0.0), _cv(0.0), _ru(1), _rv(1) {
    updateTemporaries();
...
bool ExtendedUnifiedProjection<DISTORTION_T>::euclideanToKeypoint(
    ...
    if (z <= -(_fov_parameter * d)) return false;
...
bool ExtendedUnifiedProjection<DISTORTION_T>::keypointToEuclidean(
    ...
    const double k =
        (1 - _alpha * _alpha * _beta * r2) / (_alpha * std::sqrt(1 - (_alpha - gamma) * _beta * r2) + gamma);
...
bool ExtendedUnifiedProjection<DISTORTION_T>::keypointToEuclidean(const Eigen::MatrixBase<DERIVED_K>& keypoint,
    ...
    const double d_k_d_r2 =
        0.5 * _alpha * _beta * (-2 * _alpha * tmp2 + tmp1 * (_alpha - gamma) / tmp_sqrt) * tmp2_inv2;
...
template <typename DISTORTION_T>
Eigen::Vector3d ExtendedUnifiedProjection<DISTORTION_T>::createRandomVisiblePoint(double depth) const {
...
template <typename DISTORTION_T>
bool ExtendedUnifiedProjection<DISTORTION_T>::isUndistortedKeypointValid(const double rho2_d) const {
...
bool ExtendedUnifiedProjection<DISTORTION_T>::isBinaryEqual(const ExtendedUnifiedProjection<distortion_t>& rhs) const {
    return _alpha == rhs._alpha && _beta == rhs._beta && _fu == rhs._fu && _fv == rhs._fv && _cu == rhs._cu &&
           ...
           _one_over_beta_2alpha_m_1 == rhs._one_over_beta_2alpha_m_1 && _fov_parameter == rhs._fov_parameter &&
