...
void FovDistortion::distort(const Eigen::MatrixBase<DERIVED_Y>& yconst,
    ...
    if (_w * _w < 1e-5) {
        ...
        const double duf_du = (atan_wrd) / (_w * r_u) - (u * u * atan_wrd) / (_w * r_u_cubed) +
                              (2 * u * u * tanwhalf) / (_w * (u * u + v * v) * (4 * tanwhalfsq * (u * u + v * v) + 1));
        const double duf_dv = (2 * u * v * tanwhalf) / (_w * (u * u + v * v) * (4 * tanwhalfsq * (u * u + v * v) + 1)) -
                              (u * v * atan_wrd) / (_w * r_u_cubed);
        const double dvf_du = (2 * u * v * tanwhalf) / (_w * (u * u + v * v) * (4 * tanwhalfsq * (u * u + v * v) + 1)) -
                              (u * v * atan_wrd) / (_w * r_u_cubed);
        const double dvf_dv = (atan_wrd) / (_w * r_u) - (v * v * atan_wrd) / (_w * r_u_cubed) +
                              (2 * v * v * tanwhalf) / (_w * (u * u + v * v) * (4 * tanwhalfsq * (u * u + v * v) + 1));
...
void FovDistortion::distortParameterJacobian(const Eigen::MatrixBase<DERIVED_Y>& y,
    ...
    if (_w * _w < 1e-5) {
        ...
        const double dxd_d_w = (2 * u * (tanwhalfsq / 2 + 0.5)) / (_w * (4 * tanwhalfsq * r_u * r_u + 1)) -
                               (u * atan_wrd) / (_w * _w * r_u);

        const double dyd_d_w = (2 * v * (tanwhalfsq / 2 + 0.5)) / (_w * (4 * tanwhalfsq * r_u * r_u + 1)) -
