...
OmniProjection<DISTORTION_T>::OmniProjection() : _xi(0.0), _fu(0.0), _fv(0.0), _cu(0.0), _cv(0.0), _ru(1), _rv(1) {
    updateTemporaries();
}
...
template <typename DISTORTION_T>
OmniProjection<DISTORTION_T>::OmniProjection(double xi, double focalLengthU, double focalLengthV, double imageCenterU,
...
bool OmniProjection<DISTORTION_T>::euclideanToKeypoint(const Eigen::MatrixBase<DERIVED_P>& p,
    ...
    if (p[2] <= -(_fov_parameter * d)) return false;
...
bool OmniProjection<DISTORTION_T>::keypointToEuclidean(const Eigen::MatrixBase<DERIVED_K>& keypoint,
    ...
    if (!isUndistortedKeypointValid(rho2_d)) return false;
...
bool OmniProjection<DISTORTION_T>::keypointToEuclidean(const Eigen::MatrixBase<DERIVED_K>& keypoint,
    ...
    double mul = -_xi * (2.0 * recip_tmpA + recip_tmpA2 * (_xi * _xi - 1.0) * tmpB * (rho2_d + 1.0));
    Eigen::Vector2d J3;
    J3[0] = r0 * mul;
    J3[1] = r1 * mul;

    Jk.row(2) = J3.transpose() * Jd * Eigen::Vector2d(_recip_fu, _recip_fv).asDiagonal();
...
template <typename DISTORTION_T>
Eigen::Vector3d OmniProjection<DISTORTION_T>::createRandomVisiblePoint(double depth) const {
...
template <typename DISTORTION_T>
bool OmniProjection<DISTORTION_T>::isUndistortedKeypointValid(const double rho2_d) const {
...
OmniProjection<DISTORTION_T> OmniProjection<DISTORTION_T>::getTestProjection() {
    return OmniProjection<DISTORTION_T>(0.9, 400, 400, 320, 240, 640, 480, DISTORTION_T::getTestDistortion());
}
...
inline double hypot(double a, double b) { return sqrt(square(a) + square(b)); }
            ...
            if (count > MIN_CORNERS) {
                ...
                if (numReprojected > MIN_CORNERS) {
                    ...
                    if (avgReprojErr < minReprojErr) {
                        // SM_DEBUG_STREAM("Row " << r << " produced the new best estimate: " << avgReprojErr << " < "
                        // << minReprojErr);
                ...
                // SM_DEBUG_STREAM("Skipping row " << r << " because it only had " << count << " corners. Minimum: " <<
                // MIN_CORNERS);
            }
