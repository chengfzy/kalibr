...
class CameraGeometryTestHarness {
  public:
    ...
    CameraGeometryTestHarness() : _tolerance(1e-2) { _geometry = camera_geometry_t::getTestGeometry(); }

    CameraGeometryTestHarness(const camera_geometry_t& geometry, double tolerance)
        : _geometry(geometry), _tolerance(tolerance) {}

    CameraGeometryTestHarness(const camera_geometry_t& geometry) : _geometry(geometry), _tolerance(1e-2) {}

    CameraGeometryTestHarness(double tolerance) : _tolerance(tolerance) {
    ...
    void testIntrinsicsJacobian() {
        ...
        if (_geometry.projection().minimalDimensions() > 0) {
            for (int i = 0; i < 20; i++) {
                ...
                _geometry.euclideanToKeypointIntrinsicsJacobianFiniteDifference(p, Jest);
                ASSERT_DOUBLE_MX_EQ(J, Jest, _tolerance, "");

                Eigen::Vector4d ph = sm::kinematics::toHomogeneous(p);
                _geometry.homogeneousToKeypointIntrinsicsJacobian(ph, J);
                _geometry.homogeneousToKeypointIntrinsicsJacobianFiniteDifference(ph, Jest);
                ASSERT_DOUBLE_MX_EQ(J, Jest, _tolerance, "");
    ...
    void testDistortionJacobian() {
        ...
        if (_geometry.projection().distortion().minimalDimensions() > 0) {
            ...
            if (_geometry.isEuclideanVisible(p)) {
                ...
                _geometry.euclideanToKeypointDistortionJacobianFiniteDifference(p, Jest);
                sm::eigen::assertNear(J, Jest, 1e-5, SM_SOURCE_FILE_POS, "Distortion Jacobian");
            ...
            for (int i = 0; i < 200; i++) {
                ...
                SCOPED_TRACE("Trial " + boost::lexical_cast<std::string>(i) + " P: " + sm::s_tuple(p(0), p(1), p(2)));
                // SM_OUT(p);
                // keypoint_t k;
