...
bool Frame<C>::isBinaryEqual(const Frame<C>& rhs) const {
    ...
    isEqual = isEqual && SM_CHECKMEMBERSSAME(rhs, _image) && SM_CHECKMEMBERSSAME(rhs, _stamp) &&
              SM_CHECKMEMBERSSAME(rhs, _id) && SM_CHECKSAME(_keypoints.size(), rhs._keypoints.size());
...
void Frame<C>::setRandom() {
    // Random image.
    _image->getOctaveMutable(0).create(std::max(8, rand() & 0x111), std::max(8, rand() & 0x111), CV_8UC1);
...
void Frame<C>::load(Archive& ar, const unsigned int version) {
    ...
    // *>(NULL), static_cast< FrameBase * >(NULL) );
    if (version >= 1) {
        ar >> BOOST_SERIALIZATION_BASE_OBJECT_NVP(FrameBase);
    }
