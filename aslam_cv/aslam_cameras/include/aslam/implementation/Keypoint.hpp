...

{
    ...
    if (rhs._backProjection) {
        _backProjection = new sm::kinematics::UncertainVector3(*rhs._backProjection);
    }
}

template <int D>
template <typename DERIVED>
double Keypoint<D>::mahalonibisDistance(const Eigen::MatrixBase<DERIVED>& y) const {
    EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE_OR_DYNAMIC(Eigen::MatrixBase<DERIVED>, D);
...
template <int D>
void Keypoint<D>::setInverseMeasurementCovariance(const inverse_covariance_t& invR) {
...
template <int D>
void Keypoint<D>::vsSetInverseMeasurementCovariance(const Eigen::MatrixXd& invR) {
...
template <int D>
void Keypoint<D>::setDescriptorPtr(boost::shared_ptr<DescriptorBase> descriptor) {
...
const sm::kinematics::UncertainHomogeneousPoint& Keypoint<D>::landmark() const {
    SM_ASSERT_TRUE_DBG(Exception, _landmark != NULL, "Getting a null landmark is bad, okay?");
    return *_landmark;
}

/// \brief set the landmark
template <int D>
void Keypoint<D>::setLandmark(const sm::kinematics::UncertainHomogeneousPoint& landmark) {
    if (!_landmark) {
...
template <int D>
void Keypoint<D>::setBackProjection(const sm::kinematics::UncertainVector3& v) {
    if (_backProjection) {
        delete _backProjection;
    }
...
void Keypoint<D>::setRandom() {
    ...
    U.setRandom();
    _inverseMeasurementCovariance = U.transpose() * U + inverse_covariance_t::Identity();
