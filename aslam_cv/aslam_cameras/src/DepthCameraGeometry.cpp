...
DepthCameraGeometry::DepthCameraGeometry() { *this = createTestGeometry(); }

DepthCameraGeometry::DepthCameraGeometry(double focalLengthU, double focalLengthV, double imageCenterU,
...
Duration DepthCameraGeometry::temporalOffset(const keypoint_t& keypoint) const { return Duration(0); }

DepthCameraGeometry::keypoint_t DepthCameraGeometry::maxKeypoint() const {
    return keypoint_t(
        _ru, _rv,
        1.0e3);  // that's kind of ridiculously a too large distance, but numerics don't allow smaller distances...
}

DepthCameraGeometry::keypoint_t DepthCameraGeometry::minKeypoint() const { return keypoint_t(0, 0, 1.0e-12); }

std::string DepthCameraGeometry::typeName() const { return "PinholeCamera"; }
