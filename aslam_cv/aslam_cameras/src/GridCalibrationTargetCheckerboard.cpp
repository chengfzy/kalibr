...
GridCalibrationTargetCheckerboard::GridCalibrationTargetCheckerboard(size_t rows, size_t cols, double rowSpacingMeters,
    ...
    : GridCalibrationTargetBase(rows, cols),
      ...
      _options(options) {
    SM_ASSERT_GT(Exception, rowSpacingMeters, 0.0, "rowSpacingMeters has to be positive");
    SM_ASSERT_GT(Exception, colSpacingMeters, 0.0, "colSpacingMeters has to be positive");
...
void GridCalibrationTargetCheckerboard::createGridPoints() {
    for (unsigned int r = 0; r < _rows; r++)
        for (unsigned int c = 0; c < _cols; c++)
            _points.row(gridCoordinatesToPoint(r, c)) =
                Eigen::Matrix<double, 1, 3>(_rowSpacingMeters * r, _colSpacingMeters * c, 0.0);
}
...
bool GridCalibrationTargetCheckerboard::computeObservation(const cv::Mat &image, Eigen::MatrixXd &outImagePoints,
    ...
    if (_options.useAdaptiveThreshold) flags += cv::CALIB_CB_ADAPTIVE_THRESH;
    if (_options.normalizeImage) flags += cv::CALIB_CB_NORMALIZE_IMAGE;
    if (_options.filterQuads) flags += cv::CALIB_CB_FILTER_QUADS;
    ...
    if (_options.doSubpixelRefinement && success) {
        cv::cornerSubPix(image, centers, cv::Size(_options.windowWidth, _options.windowWidth), cv::Size(-1, -1),
    ...
    for (unsigned int i = 0; i < size(); i++)
        outImagePoints.row(i) = Eigen::Matrix<double, 1, 2>(centers.row(i).at<float>(0), centers.row(i).at<float>(1));
