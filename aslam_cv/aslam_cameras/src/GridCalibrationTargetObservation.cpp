...
GridCalibrationTargetObservation::GridCalibrationTargetObservation(GridCalibrationTargetBase::Ptr target, cv::Mat image)
    : _imRows(0), _imCols(0), _T_t_c_isSet(false) {
    setImage(image);
    setTarget(target);
}

GridCalibrationTargetObservation::GridCalibrationTargetObservation(GridCalibrationTargetBase::Ptr target)
    : _imRows(0), _imCols(0), _T_t_c_isSet(false) {
    setTarget(target);
}
...
unsigned int GridCalibrationTargetObservation::getCornersIdx(std::vector<unsigned int>& outCornerIdx) const {
    ...
    outCornerIdx.clear();
    for (unsigned int i = 0; i < numCorners; i++)
        if (_success[i]) outCornerIdx.push_back(i);
...
void GridCalibrationTargetObservation::clearImage() { _image = cv::Mat(); }
