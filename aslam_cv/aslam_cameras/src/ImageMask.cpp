...
ImageMask::ImageMask(const sm::PropertyTree& config) : _scale(1.0) {
...
ImageMask::ImageMask(const cv::Mat& mask, double scale) {
    SM_ASSERT_EQ(std::runtime_error, mask.type(), CV_8UC1, "The mask must be 8-bit one channel");
    _mask = mask.clone();
    _scale = scale;
}
...
bool ImageMask::isSet() const { return _mask.data != NULL; }

const cv::Mat& ImageMask::getMask() const { return _mask; }

void ImageMask::setScale(double scale) { _scale = scale; }

double ImageMask::getScale() const { return _scale; }

bool ImageMask::isBinaryEqual(const ImageMask& rhs) const { return sm::opencv::isBinaryEqual(_mask, rhs._mask); }
