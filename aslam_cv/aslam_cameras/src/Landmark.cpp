...
Landmark::Landmark(const Landmark& rhs)
    : _descriptor(rhs._descriptor), _ph(rhs._ph), _landmarkId(rhs._landmarkId), _frameId(rhs._frameId) {}
...
boost::shared_ptr<DescriptorBase> Landmark::descriptorPtr() { return _descriptor; }

void Landmark::setDescriptor(const DescriptorBase& descriptor) { _descriptor.reset(descriptor.clone()); }

void Landmark::setDescriptorPtr(boost::shared_ptr<DescriptorBase> descriptor) { _descriptor = descriptor; }

void Landmark::setDescriptorRawPtr(DescriptorBase* descriptor) { _descriptor.reset(descriptor); }

// UncertainHomogeneousPoint

const sm::kinematics::UncertainHomogeneousPoint& Landmark::point() const { return _ph; }
...
void Landmark::setPoint(const sm::kinematics::UncertainHomogeneousPoint& ph) { _ph = ph; }

// LandmarkId

const LandmarkId& Landmark::landmarkId() const { return _landmarkId; }

void Landmark::setLandmarkId(const LandmarkId& landmarkId) { _landmarkId = landmarkId; }

// FrameId

const boost::uint64_t& Landmark::frameId() const { return _frameId; }

void Landmark::setFrameId(const boost::uint64_t& frameId) { _frameId = frameId; }
...
void Landmark::fromTransformedKeypoint(boost::uint64_t frameId, const KeypointBase& kp,
                                       const sm::kinematics::Transformation& T_v_c) {
    setFrameId(frameId);
    setDescriptorPtr(kp.descriptorPtr());
    const sm::kinematics::UncertainHomogeneousPoint* pp_c = kp.landmarkPtr();
    SM_ASSERT_TRUE(std::runtime_error, pp_c != NULL, "The landmark should not be null!");
