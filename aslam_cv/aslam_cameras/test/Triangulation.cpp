...
TEST(AslamCamerasTestSuite, testTriangulationIdeal) {
    ...
    aslam::cameras::triangulate(p0, r0, p1, r1, outTriangulatedPoint, outGap, outS0, outS1);
...
TEST(AslamCamerasTestSuite, testTriangulationNoisy) {
    ...
    ASSERT_DOUBLE_MX_EQ(outTriangulatedPoint, (p0 + (outS0 * r0) + p1 + (outS1 * r1)) * 0.5, 1,
                        "The triangulated point does not match");
