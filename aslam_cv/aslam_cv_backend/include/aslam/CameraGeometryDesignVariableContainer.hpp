...
class CameraGeometryDesignVariableContainer {
  public:
    ...
    boost::shared_ptr<ReprojectionError> createReprojectionError(const Eigen::VectorXd& y, const Eigen::MatrixXd& invR,
    ...
    void evaluateJacobians(backend::JacobianContainer& outJacobians, Eigen::Vector4d ph) const;

    /// \brief Evaluate the jacobians of the homogeneousToKeypoint() with respect to the camera parameters.
    void evaluateJacobians(backend::JacobianContainer& outJacobians, const Eigen::MatrixXd& applyChainRule,
                           Eigen::Vector4d ph) const;

    /// \brief return the temporal offset with respect to the intrinsics.
    void temporalOffsetIntrinsicsJacobian(const Eigen::VectorXd& keypoint, Eigen::MatrixXd& outJi) const;

    void getDesignVariables(backend::DesignVariable::set_t& designVariables) const;
    ...
    boost::shared_ptr<cameras::CameraGeometryBase> camera() { return _camera; }
    ...
    boost::shared_ptr<aslam::backend::DesignVariable> getDesignVariable() const { return _cameraDv; }
  ...
  private:
    ...
    boost::shared_ptr<backend::DesignVariableAdapter<CameraGeometryDesignVariableContainer> > _cameraDv;
};
