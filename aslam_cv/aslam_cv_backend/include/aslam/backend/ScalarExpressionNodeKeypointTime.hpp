...
template <typename CAMERA_T>
class ScalarExpressionNodeKeypointTime : public aslam::backend::ScalarExpressionNode {
    typedef CAMERA_T camera_t;

  public:
    ScalarExpressionNodeKeypointTime(const aslam::Time& stamp, const Eigen::VectorXd& y,
    ...
    virtual void evaluateJacobiansImplementation(backend::JacobianContainer& outJacobians,
                                                 const Eigen::MatrixXd& applyChainRule) const;
    virtual void getDesignVariablesImplementation(backend::DesignVariable::set_t& designVariables) const;
