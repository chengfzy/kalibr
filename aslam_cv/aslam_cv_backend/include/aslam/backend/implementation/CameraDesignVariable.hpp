...
CameraDesignVariable<CAMERA_T>::CameraDesignVariable(const boost::shared_ptr<CAMERA_T>& camera) : _camera(camera) {
...
template <typename CAMERA_T>
void CameraDesignVariable<CAMERA_T>::evaluateJacobians(JacobianContainer& outJacobians,
...
template <typename CAMERA_T>
backend::ScalarExpression CameraDesignVariable<CAMERA_T>::keypointTime(const aslam::Time& imageStamp,
                                                                       const Eigen::VectorXd& y) {
    if (_shutterDv->isActive()) {
        boost::shared_ptr<backend::ScalarExpressionNode> root(
            new ScalarExpressionNodeKeypointTime<CAMERA_T>(imageStamp, y, this->shared_from_this()));
...
template <typename CAMERA_T>
backend::ScalarExpression CameraDesignVariable<CAMERA_T>::temporalOffset(const Eigen::VectorXd& y) {
