...
ScalarExpressionNodeKeypointTime<CAMERA_T>::ScalarExpressionNodeKeypointTime(
    const aslam::Time& stamp, const Eigen::VectorXd& y, boost::shared_ptr<backend::CameraDesignVariable<CAMERA_T> > dv)
    : _stamp(stamp), _y(y), _dv(dv) {
    SM_ASSERT_TRUE(std::runtime_error, _dv.get() != NULL, "Unable to intialize with a null design variable");
}
...
template <typename CAMERA_T>
void ScalarExpressionNodeKeypointTime<CAMERA_T>::evaluateJacobiansImplementation(
    backend::JacobianContainer& outJacobians, const Eigen::MatrixXd& applyChainRule) const {
    Eigen::MatrixXd Ji;
