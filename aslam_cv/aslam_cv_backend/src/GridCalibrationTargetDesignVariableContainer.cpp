...
void GridCalibrationTargetDesignVariableContainer::setPointActive(size_t i, bool active) {
    SM_ASSERT_LT(std::runtime_error, i, _points.size(), "Out of bounds");
    _points[i]->setActive(active);
}

/// \brief get the expression for point i
backend::EuclideanExpression GridCalibrationTargetDesignVariableContainer::getPoint(size_t i) {
    SM_ASSERT_LT(std::runtime_error, i, _points.size(), "Out of bounds");
