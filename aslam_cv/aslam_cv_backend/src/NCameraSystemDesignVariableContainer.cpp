...
NCameraSystemDesignVariableContainer::NCameraSystemDesignVariableContainer(
    NCameraSystem::Ptr cameraSystem, bool estimateExtrinsicsRotation, bool estimateExtrinsicsTranslation,
    bool estimateProjection, bool estimateDistortion, bool estimateShutter)
    : _cameraSystem(cameraSystem) {
    SM_ASSERT_TRUE(std::runtime_error, _cameraSystem.get() != NULL, "Cannot initialize with a null camera system");
    ...
    for (size_t i = 0; i < cameraSystem->numCameras(); ++i) {
        boost::shared_ptr<sm::kinematics::Transformation> T_v_c = cameraSystem->ptrT_v_c(i);
        _T_c_v[i] = aslam::backend::transformationToExpression(*T_v_c, _q_v_c[i], _p_v_c[i]).inverse();
    ...
    for (size_t i = 0; i < cameraSystem->numCameras(); ++i) {
        _cameraDvs[i].reset(new CameraGeometryDesignVariableContainer(cameraSystem->geometry(i), estimateProjection,
                                                                      estimateDistortion, estimateShutter));
    }
...
void NCameraSystemDesignVariableContainer::setCameraIntrinsicsActive(size_t cameraIndex, bool active) {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _cameraDvs.size(), "Index out of bounds");
...
/// \brief set the intrinsics of a specific camera active
void NCameraSystemDesignVariableContainer::setCameraExtrinsicsActive(size_t cameraIndex, bool active) {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _q_v_c.size(), "Index out of bounds");
...
void NCameraSystemDesignVariableContainer::setCameraRotationExtrinsicsActive(size_t cameraIndex, bool active) {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _q_v_c.size(), "Index out of bounds");
    _q_v_c[cameraIndex]->setActive(active);
}

/// \brief set the translation extrinsics of a specific camera active
void NCameraSystemDesignVariableContainer::setCameraTranslationExtrinsicsActive(size_t cameraIndex, bool active) {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _p_v_c.size(), "Index out of bounds");
    _p_v_c[cameraIndex]->setActive(active);
}

backend::TransformationExpression NCameraSystemDesignVariableContainer::getT_c_v(size_t cameraIndex) {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _T_c_v.size(), "Index out of bounds");
    return _T_c_v[cameraIndex];
}

/// \brief are the rotation extrinsics of a specific camera active?
bool NCameraSystemDesignVariableContainer::isCameraRotationExtrinsicsActive(size_t cameraIndex) const {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _q_v_c.size(), "Index out of bounds");
    return _q_v_c[cameraIndex]->isActive();
}

/// \brief are the translation extrinsics of a specific camera active?
bool NCameraSystemDesignVariableContainer::isCameraTranslationExtrinsicsActive(size_t cameraIndex) const {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _p_v_c.size(), "Index out of bounds");
    return _p_v_c[cameraIndex]->isActive();
}

/// \brief are the intrinsics of a specific camera active?
bool NCameraSystemDesignVariableContainer::isCameraIntrinsicsActive(size_t cameraIndex) const {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _cameraDvs.size(), "Index out of bounds");
    return _cameraDvs[cameraIndex]->isActive();
}

boost::shared_ptr<CameraGeometryDesignVariableContainer>
NCameraSystemDesignVariableContainer::getCameraGeometryDesignVariableContainer(size_t cameraIndex) {
    SM_ASSERT_LT(std::runtime_error, cameraIndex, _cameraDvs.size(), "Index out of bounds");
...
boost::shared_ptr<ReprojectionError> NCameraSystemDesignVariableContainer::createReprojectionError(
    ...
    re = _cameraDvs[kid.cameraIndex]->createReprojectionError(frame->keypointBase(kid.keypointIndex).vsY(),
...
NCameraSystem::Ptr NCameraSystemDesignVariableContainer::getCameraSystem() const { return _cameraSystem; }
