...
class ReprojectionError : public ErrorTermFs<CAMERA_GEOMETRY_T::KeypointDimension> {
  public:
    ...
    ReprojectionError();
    ReprojectionError(const measurement_t& measurement, const inverse_covariance_t& inverseCovariance,
                      const HomogeneousExpression& point, CameraDesignVariable<camera_geometry_t> camera);
    ReprojectionError(const frame_t* frame, int keypointIndex, HomogeneousExpression point,
    ...
    void updateMeasurementAndCovariance(const measurement_t& measurement, const inverse_covariance_t& invR);
