...
double CovarianceReprojectionError<F>::evaluateErrorImplementation() {
    ...
    if (_spline) {
        ...
        parent_t::setInvR(
            ((A * _frame->keypoint(_keypointIndex).invR().inverse() * A.transpose()).inverse()));  // invert
        // setInvR(A*_frame->keypoint(_keypointIndex).invR()*A.transpose());

        // std::cout << "A: " << std::endl << A << std::endl ;
        // std::cout << "ARAT: " << std::endl << A*_frame->keypoint(_keypointIndex).invR()*A.transpose() << std::endl <<
        // std::endl;
    }
