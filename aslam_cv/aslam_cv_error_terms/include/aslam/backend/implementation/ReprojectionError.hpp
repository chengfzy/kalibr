...
template <typename F>
ReprojectionError<F>::ReprojectionError(const frame_t* frame, int keypointIndex, HomogeneousExpression point,
                                        CameraDesignVariable<camera_geometry_t> camera)
    : _point(point), _camera(camera) {
...
template <typename F>
ReprojectionError<F>::ReprojectionError(const measurement_t& measurement, const inverse_covariance_t& inverseCovariance,
                                        const HomogeneousExpression& point,
                                        CameraDesignVariable<camera_geometry_t> camera)
    : _y(measurement), _point(point), _camera(camera) {
...
template <typename F>
void ReprojectionError<F>::evaluateJacobiansImplementation(aslam::backend::JacobianContainer& _jacobians) const {
...
template <typename F>
void ReprojectionError<F>::updateMeasurement(const measurement_t& measurement) {
    _y = measurement;
}

template <typename F>
void ReprojectionError<F>::updateMeasurementAndCovariance(const measurement_t& measurement,
