...
class HpErr : public aslam::backend::ErrorTermFs<4> {
  public:
    ...
    HpErr(const Eigen::Vector4d& p) : _p(p) {
        _p.setActive(true);
        _p.setBlockIndex(0);
    ...
    virtual void evaluateJacobiansImplementation(aslam::backend::JacobianContainer& J) { _p.evaluateJacobians(J); }
};
...
TEST(ReprojectionErrorTestSuite, testSimpleError) {
    try {
        ...
        SCOPED_TRACE("");
        sm::eigen::assertNear(J.asDenseMatrix(), estJ.asDenseMatrix(), 1e-6, SM_SOURCE_FILE_POS,
                              "Checking the jacobian vs. finite differences");
...
TEST(ReprojectionErrorTestSuite, testEstimator) {
    try {
        ...
        for (int k = 0; k < K; ++k) {
            for (int n = 0; n < N; ++n) {
                // create a point.
                Eigen::Vector4d p_ck = sm::kinematics::toHomogeneous(
                    frame_k[k].geometry().createRandomVisiblePoint(sm::random::randLU(1.0, 10.0)));
        ...
        for (size_t l = 0; l < landmarks.size(); ++l) {
            ...
            if (seenCount > 1) {
                ...
                for (int k = 0; k < K; k++) {
                    ...
                    // Invert this expression to get a new expression that takes points from the world frame to the
                    // camera frame.
                    TransformationExpression T_ck_w = T_w_ck.inverse();

                    // Multiplying the above expression with a landmark HomogeneousPointExpression makes a new
                    // HomogeneousPointExpression representing the landmark expressed in the camera frame.
                    ...
                    if (frame_k[k].geometry().isValid(y) &&
                        sm::kinematics::fromHomogeneous(p_ck.toHomogeneous())[2] > 0.0) {
                        // Now we can add an error term...First add the measurement to the frame.
                        keypoint_t kp;
                        kp.setMeasurement(y +
                                          Eigen::Vector2d(sm::random::randn() * sigma, sm::random::randn() * sigma));
                        ...
                        boost::shared_ptr<error_t> error(
                            new error_t(&frame_k[k], frame_k[k].numKeypoints() - 1, p_ck, cameraDv));
