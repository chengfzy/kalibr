...
void exportKeypoint() {
    ...
    class_<keypoint_t, bases<KeypointBase> >(str.str().c_str(), init<>())
        ...
        .def("octave", &keypoint_t::octave)
        .def("setOctave", &keypoint_t::setOctave)
        .def("inverseMeasurementCovariance", &keypoint_t::inverseMeasurementCovariance,
             return_value_policy<copy_const_reference>())
        .def("invR", &keypoint_t::invR, return_value_policy<copy_const_reference>())
        .def("setInverseMeasurementCovariance", &keypoint_t::setInverseMeasurementCovariance)
...
//       ErrorTerm > >( (name + "Simple").c_str(),
//     		  init<const frame_t * , int ,HomogeneousExpression >( (name + "Simple( frame, keypointIndex,
//     homogeneousPointExpression )").c_str()) )
// 			;
...
//       class_< CovarianceReprojectionError<frame_t>, boost::shared_ptr<CovarianceReprojectionError<frame_t> >, bases<
//       ErrorTerm > >( name.c_str(),
//     		  init<const frame_t * , int ,HomogeneousExpression, CameraDesignVariable<geometry_t>,
//     aslam::splines::BSplinePoseDesignVariable*, aslam::backend::Scalar* >( (name + "( frame, keypointIndex,
//     homogeneousPointExpression, CameraDesignVariable, bsplineDesignVariable, lineDelayDv)").c_str()) )
...
//   class_< ReprojectionIntrinsicsError<frame_t>, boost::shared_ptr<ReprojectionIntrinsicsError<frame_t> >, bases<
//   ErrorTerm > >( name.c_str(), init<>() )
// .def(init<const frame_t * , int ,HomogeneousExpression >( (name + "( frame, keypointIndex,
// homogeneousPointExpression)").c_str()));
