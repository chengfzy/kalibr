...
// void distortParameterJacobian(const Eigen::MatrixBase<DERIVED_Y> & imageY, const Eigen::MatrixBase<DERIVED_JD> &
// outJd) const;
template <typename D>
Eigen::MatrixXd distortParameterJacobian(const D* distortion, Eigen::Vector2d imageY) {
    Eigen::MatrixXd J;
...
void exportGenericProjectionFunctions(T& proj) {
    proj.def("keypointDimension", &C::keypointDimension, "Get the dimension of the keypoint type");
    proj.def("isProjectionInvertible", &C::isProjectionInvertible,
             "Is the sensor model invertible? Ususally this is only true for a range/bearing sensor.");
    proj.def("createRandomKeypoint", &C::createRandomKeypoint,
             "Create a valid, random keypoint. This is useful for unit testing and experiments.");
    proj.def("createRandomVisiblePoint", &C::createRandomVisiblePoint,
             "Create a valid point in space visible by the camera.\np = createRandomVisiblePoint(depth).");
    proj.def("euclideanToKeypoint", &e2k<C>, "Map a 3x1 Euclidean point to a keypoint.\nk = euclideanToKeypoint(p)");
    proj.def("euclideanToKeypointJp", &e2kJp<C>,
             "Map a 3x1 Euclidean point to a keypoint and get the Jacobian of the mapping with respect to small "
             "changes in the point.\n(k, Jp) = euclideanToKeypoint(p)");
    proj.def("homogeneousToKeypoint", &eh2k<C>,
             "Map a 4x1 homogeneous Euclidean point to a keypoint.\nk = euclideanToKeypoint(p)");
    proj.def("homogeneousToKeypointJp", &eh2kJp<C>,
             "Map a 4x1 homogeneous Euclidean point to a keypoint and get the Jacobian of the mapping with respect to "
             "small changes in the point.\n(k, Jp) = homogeneousToKeypointJp(p)");
    proj.def("keypointToHomogeneous", &k2eh<C>,
             "Map a keypoint to a 4x1 homogeneous Euclidean point.\np = keypointToHomogeneous(k)");
    proj.def("keypointToHomogeneousJk", &eh2kJp<C>,
             "Map a keypoint to a 4x1 homogeneous Euclidean point and get the Jacobian of the mapping with respect to "
             "small changes in the keypoint.\n(p, Jk) = keypointToHomogeneousJk(k)");
    proj.def("keypointToEuclidean", &k2e<C>, "Map a keypoint to a 3x1 Euclidean point.\np = keypointToEuclidean(k)");
    proj.def("keypointToEuclideanJk", &e2kJp<C>,
             "Map a keypoint to a 3x1 Euclidean point and get the Jacobian of the mapping with respect to small "
             "changes in the keypoint.\n(p, Jk) = keypointToEuclideanJk(k)");
    proj.def("isValid", &C::template isValid<Eigen::VectorXd>);
    proj.def("setParameters", &C::setParameters, "Set the Parameter Vector/Matrix");
    proj.def("getParameters", &getParameters<C>, "Get the Parameter Vector/Matrix");
}

template <typename C, typename T>
void exportGenericDistortionFunctions(T& dist) {
    dist.def("distort", &distort1<C>, "Apply distortion to a keypoint");
    dist.def("distortJd", &distort2<C>, "Apply distortion and get the Jacobian with respect to the input point");
    dist.def("undistort", &undistort1<C>, "Undistort a keypoint");
    dist.def(
        "undistort", &undistort2<C>,
        "Undistort a keypoint and get the Jacobian with respect to small chages in the input (distorted) keypoint.");
    dist.def("distortParameterJacobian", &distortParameterJacobian<C>,
             "Get the Jacobian of the distortion operation with respect to small changes in the distortion parameters");
    dist.def("setParameters", &C::setParameters, "Set the Parameter Vector/Matrix");
    dist.def("getParameters", &getParameters<C>, "Get the Parameter Vector/Matrix");
}
...
void exportRadialTangentialDistortionFunctions() {
    class_<RadialTangentialDistortion, boost::shared_ptr<RadialTangentialDistortion> > rtDistortion(
    ...
    rtDistortion.def(init<double, double, double, double>(
...
void exportOmniProjection(std::string name) {
    ...
    class_<OmniProjection<D>, boost::shared_ptr<OmniProjection<D> > > omniProjection(name.c_str(), init<>());
    sm::python::unique_register_ptr_to_python<boost::shared_ptr<OmniProjection<D> > >();

    omniProjection.def(init<>((name + "(distortion_t distortion)").c_str()))
        .def(init<double, double, double, double, double, int, int, D>(
            (name + "(double xi, double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, "
                    "int resolutionU, int resolutionV, distortion_t distortion)")
                .c_str()))
        .def(init<double, double, double, double, double, int, int>(
            (name + "(double xi, double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, "
        ...
        .def("rv", &OmniProjection<D>::rv)
        .def("focalLengthCol", &OmniProjection<D>::focalLengthCol)
        .def("focalLengthRow", &OmniProjection<D>::focalLengthRow)
        .def("opticalCenterCol", &OmniProjection<D>::opticalCenterCol)
        .def("opticalCenterRow", &OmniProjection<D>::opticalCenterRow)
        .def("distortion", omnidistortion, return_internal_reference<>())
        .def("setDistortion", &OmniProjection<D>::setDistortion)
        .def_pickle(sm::python::pickle_suite<OmniProjection<D> >());
...
void exportExtendedUnifiedProjection(std::string name) {
    ...
    extendedUnifiedProjection.def(init<>((name + "(distortion_t distortion)").c_str()))
        .def(init<double, double, double, double, double, double, int, int, D>(
            (name + "(double alpha, double beta, double focalLengthU, double focalLengthV, double imageCenterU, double "
                    "imageCenterV, int resolutionU, int resolutionV, distortion_t distortion)")
                .c_str()))
        .def(init<double, double, double, double, double, double, int, int>(
            (name + "(double alpha, double beta, double focalLengthU, double focalLengthV, double imageCenterU, double "
                    "imageCenterV, int resolutionU, int resolutionV)")
                .c_str()))
        ...
        /// \brief The vertical resolution in pixels.
        .def("rv", &ExtendedUnifiedProjection<D>::rv)
        .def("focalLengthCol", &ExtendedUnifiedProjection<D>::focalLengthCol)
        .def("focalLengthRow", &ExtendedUnifiedProjection<D>::focalLengthRow)
        .def("opticalCenterCol", &ExtendedUnifiedProjection<D>::opticalCenterCol)
        .def("opticalCenterRow", &ExtendedUnifiedProjection<D>::opticalCenterRow)
        .def("distortion", distortion, return_internal_reference<>())
        .def("setDistortion", &ExtendedUnifiedProjection<D>::setDistortion)
        .def_pickle(sm::python::pickle_suite<ExtendedUnifiedProjection<D> >());
...
void exportDoubleSphereProjection(std::string name) {
    ...
    doubleSphereProjection.def(init<>((name + "(distortion_t distortion)").c_str()))
        .def(init<double, double, double, double, double, double, int, int, D>(
            (name + "(double xi, double alpha, double focalLengthU, double focalLengthV, double imageCenterU, double "
                    "imageCenterV, int resolutionU, int resolutionV, distortion_t distortion)")
                .c_str()))
        .def(init<double, double, double, double, double, double, int, int>(
            (name + "(double xi, double alpha, double focalLengthU, double focalLengthV, double imageCenterU, double "
        ...
        /// \brief The vertical resolution in pixels.
        .def("rv", &DoubleSphereProjection<D>::rv)
        .def("focalLengthCol", &DoubleSphereProjection<D>::focalLengthCol)
        .def("focalLengthRow", &DoubleSphereProjection<D>::focalLengthRow)
        .def("opticalCenterCol", &DoubleSphereProjection<D>::opticalCenterCol)
        .def("opticalCenterRow", &DoubleSphereProjection<D>::opticalCenterRow)
        .def("distortion", distortion, return_internal_reference<>())
        .def("setDistortion", &DoubleSphereProjection<D>::setDistortion)
        .def_pickle(sm::python::pickle_suite<DoubleSphereProjection<D> >());
...
void exportPinholeProjection(std::string name) {
    typename PinholeProjection<D>::distortion_t& (PinholeProjection<D>::*distortion1)() =
        &PinholeProjection<D>::distortion;

    class_<PinholeProjection<D>, boost::shared_ptr<PinholeProjection<D> > > pinholeProjection(name.c_str(), init<>());
    ...
    pinholeProjection.def(init<>((name + "(distortion_t distortion)").c_str()))
        .def(init<double, double, double, double, int, int, D>(
            (name + "(double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, int "
                    "resolutionU, int resolutionV, distortion_t distortion)")
                .c_str()))
        .def(init<double, double, double, double, int, int>(
            (name + "(double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, int "
                    "resolutionU, int resolutionV)")
                .c_str()))
        /// \brief The horizontal focal length in pixels.
        ...
        .def("distortion", distortion1, return_internal_reference<>())
        .def("setDistortion", &PinholeProjection<D>::setDistortion)
        .def_pickle(sm::python::pickle_suite<PinholeProjection<D> >());
...
void exportCameraProjections() {
    ...
    class_<NoDistortion, boost::shared_ptr<NoDistortion> > noDistortion("NoDistortion", init<>());
    ...
    exportGenericDistortionFunctions<EquidistantDistortion>(equidistantDistortion);
    ...
    exportPinholeProjection<RadialTangentialDistortion>("DistortedPinholeProjection");
    exportPinholeProjection<EquidistantDistortion>("EquidistantPinholeProjection");
    exportPinholeProjection<FovDistortion>("FovPinholeProjection");
    ...
    // distortion:
    // exportAPrioriInformationError<aslam::backend::DesignVariableAdapter< RadialTangentialDistortion >
    // >("RadialTangentialDistortionAPrioriInformationError");
    // exportAPrioriInformationError<aslam::backend::DesignVariableAdapter<
    // PinholeProjection<RadialTangentialDistortion> > >("DistortedPinholeProjectionAPrioriInformationError");
    // exportAPrioriInformationError<aslam::backend::DesignVariableAdapter< OmniProjection<RadialTangentialDistortion> >
    // >("DistortedOmniProjectionAPrioriInformationError");
}
