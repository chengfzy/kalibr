...
void briskInitFromArray(BriskDescriptor* bd,
...
Eigen::Matrix<float, 64, 1> surfGetDescriptor(SurfDescriptor* sd) { return sd->descriptor; }

void surfSetDescriptor(SurfDescriptor* sd, Eigen::Matrix<float, 64, 1>& desc) { sd->descriptor = desc; }
...
void exportDescriptors() {
    ...
    class_<DescriptorBase,                     // The class being exported
        ...
        .def("distance", &DescriptorBase::distance)
        .def("setRandom", &DescriptorBase::setRandom);
    ...
    class_<BriskDescriptor, boost::shared_ptr<BriskDescriptor>,
           ...
           >("BriskDescriptor", init<>())
        .def("distance", &BriskDescriptor::distance)
        .def("initFromArray", &briskInitFromArray)
        .def("setDescriptor", &briskInitFromArray)
        .def("data", &briskData)
        .def("descriptor", &briskData);

    class_<SurfDescriptor, boost::shared_ptr<SurfDescriptor>, bases<DescriptorBase> >("SurfDescriptor", init<>())
        .def("distance", &SurfDescriptor::distance)
        .def_readwrite("laplacianBit", &SurfDescriptor::laplacianBit)
        .def_readwrite("size", &SurfDescriptor::size)
        .def_readwrite("octave", &SurfDescriptor::octave)
        .def_readwrite("angle", &SurfDescriptor::angle)
        .def_readwrite("response", &SurfDescriptor::response)
