...
void exportMatch(const std::string &indexName) {
    ...
    class_<match_t>((indexName + "Match").c_str(), init<>())
        .def(init<INDEX_T, INDEX_T, float>())
        .def("item", &match_t::item)
        .def("getItem0", &match_t::getItem0)
        .def("getItem1", &match_t::getItem1)
        .def_readonly("distance", &match_t::distance)
        .def_readwrite("isTriangulated", &match_t::isTriangulated);
}
...
void exportMatchingAlgorithms() {
    ...
    class_<DenseMatcher, boost::shared_ptr<DenseMatcher> >("DenseMatcher", init<>())
        .def(init<boost::uint8_t, boost::uint8_t>())
        .def("match", &DenseMatcher::matchSlow);

    exportMatch<KeypointIdentifier>("KeypointIdentifier");

    class_<MatchingAlgorithm, boost::shared_ptr<MatchingAlgorithm>, boost::noncopyable>("MatchingAlgorithm", no_init)
        .def("doSetup", &MatchingAlgorithm::doSetup)
        .def("sizeA", &MatchingAlgorithm::sizeA)
        .def("sizeB", &MatchingAlgorithm::sizeB)
    ...
    class_<FullMatcher, boost::shared_ptr<FullMatcher> >("FullMatcher", init<>())
        .def("initParameters", &FullMatcher::initParameters)
        .def("setParameters", &FullMatcher::setParameters,
             "setParameters(float distanceThreshold, double pureRotationEpipolarThreshold, double "
             "reprojectionThreshold, double epipolarThreshold);")
        .def("setMatchData", &FullMatcher::setMatchData);

    class_<std::vector<kid_match_t> >("KeypointIdentifierMatchVector")
        .def(vector_indexing_suite<std::vector<kid_match_t> >());

    class_<KeypointIdentifierMatchingAlgorithm, boost::shared_ptr<KeypointIdentifierMatchingAlgorithm>,
           bases<MatchingAlgorithm>, boost::noncopyable>("KeypointIdentifierMatchingAlgorithm", no_init)
        .def("getMatches", &KeypointIdentifierMatchingAlgorithm::getMatches,
