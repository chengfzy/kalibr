...
boost::shared_ptr<MaskedDistortedCameraSystem> createTestCameraSystem() {
    ...
    boost::shared_ptr<sm::kinematics::Transformation> T_odometry_front(new sm::kinematics::Transformation);
    T_odometry_front->setRandom();
    // LEFT_CAMERA = 1,
    cam_t left = createTestCamera();
    boost::shared_ptr<sm::kinematics::Transformation> T_odometry_left(new sm::kinematics::Transformation);
    T_odometry_left->setRandom();
    // BACK_CAMERA = 2,
    cam_t back = createTestCamera();
    boost::shared_ptr<sm::kinematics::Transformation> T_odometry_back(new sm::kinematics::Transformation);
    T_odometry_back->setRandom();
    // RIGHT_CAMERA = 3
    cam_t right = createTestCamera();
    boost::shared_ptr<sm::kinematics::Transformation> T_odometry_right(new sm::kinematics::Transformation);
    ...
    boost::shared_ptr<MaskedDistortedCameraSystem> cameraSystem(new MaskedDistortedCameraSystem(vT, vG, false));

    return cameraSystem;
}

boost::shared_ptr<aslam::Frame<aslam::cameras::MaskedDistortedOmniCameraGeometry> > buildTestFrame(
    boost::shared_ptr<aslam::Frame<aslam::cameras::MaskedDistortedOmniCameraGeometry> >& frontFrame) {
    assert(frontFrame);
    ...
    for (int i = 0; i < N; ++i) {
        ...
        kp.setInverseMeasurementCovariance(Eigen::Matrix2d::Identity(2, 2) * recip_sigma);
        ...
        kp.setDescriptorRawPtr(
            new aslam::SurfDescriptor(&surfDescriptor[0], laplacianBit, size, octave, angle, response));
...
boost::shared_ptr<aslam::MultiFrame> buildTestMultiFrame() {
    ...
    boost::shared_ptr<aslam::MultiFrame> frame(new aslam::MultiFrame(cameraSystem));
    int sec = 0;
    int nsec = 0;
    ...
    // see aslam_frames/include/aslam/Frame.hpp
    for (int c = 0; c < 4; ++c) {
        boost::shared_ptr<aslam::Frame<aslam::cameras::MaskedDistortedOmniCameraGeometry> > f;
        f.reset(new aslam::Frame<aslam::cameras::MaskedDistortedOmniCameraGeometry>);
        buildTestFrame(f);
        f->setGeometry(cameraSystem->geometryAs<aslam::cameras::MaskedDistortedOmniCameraGeometry>(c));
        frame->setFrame(c, f);
    }
    return frame;
}
...
TEST(AslamCameraSystemTestSuite, testMultiFrameSerialization) {
    boost::serialization::void_cast_register<aslam::SurfDescriptor, aslam::DescriptorBase>(
        static_cast<aslam::SurfDescriptor*>(NULL), static_cast<aslam::DescriptorBase*>(NULL));
