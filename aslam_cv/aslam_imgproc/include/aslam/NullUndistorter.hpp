...
class NullUndistorter : public UndistorterBase {
  public:
    ...
    NullUndistorter(const sm::PropertyTree& undistorterConfig, const sm::PropertyTree& cameraConfig);
    ...
    NullUndistorter(const sm::PropertyTree& cameraConfig, int interpolation, double scale, bool makeCopy);
    /// \param[in] scale                Allows to scale the resulting image.
    /// \param[in] makeCopy             If the image is not scaled and one still wants a copy.
    NullUndistorter(boost::shared_ptr<distorted_geometry_t> distortedGeometry, int interpolation, double scale,
                    bool makeCopy);
    /// \param[in] distortedGeometry    the distorted geometry type
    /// \param[in] config               a configuration object for the other parameters
    /// \param[in] makeCopy             If the image is not scaled and one still wants a copy.
    NullUndistorter(boost::shared_ptr<distorted_geometry_t> distortedGeometry, const sm::PropertyTree& config);
    ~NullUndistorter();

    /// \brief Initialize the Undistorter.
    /// \param[in] scale                Allows to scale the resulting image.
    void init(boost::shared_ptr<distorted_geometry_t> distortedGeometry, int interpolation, double scale,
              bool makeCopy);
  ...
  private:
    ...
    void setIdealGeometry(boost::shared_ptr<distorted_geometry_t> distortedGeometry);
