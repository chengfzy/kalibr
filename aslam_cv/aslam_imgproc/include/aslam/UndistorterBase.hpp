...
enum InterpolationType {
    ...
    Area = cv::INTER_AREA,    // resampling using pixel area relation. It may be the preferred method for image
                              // decimation, as it gives moire-free results. But when the image is zoomed, it is similar
...
class UndistorterBase {
  public:
    ...
    static boost::shared_ptr<UndistorterBase> createUndistorter(const sm::PropertyTree& undistorterConfig,
                                                                const sm::PropertyTree& cameraConfig);

    /// \brief Given the underlying camera geometry, undistort the image
    virtual void undistortImage(const cv::Mat& inImage, cv::Mat& outImage) const = 0;
