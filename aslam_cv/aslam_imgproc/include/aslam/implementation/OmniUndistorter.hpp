...
template <typename MASK_T>
OmniUndistorter<MASK_T>::OmniUndistorter(const sm::PropertyTree& undistorterConfig,
...
template <typename MASK_T>
OmniUndistorter<MASK_T>::OmniUndistorter(const sm::PropertyTree& cameraConfig, int interpolation, double alpha,
                                         double scale) {
    boost::shared_ptr<distorted_geometry_t> distortedGeometry(new distorted_geometry_t(cameraConfig));
    init(distortedGeometry, interpolation, alpha, scale);
}
...
template <typename MASK_T>
OmniUndistorter<MASK_T>::OmniUndistorter(boost::shared_ptr<distorted_geometry_t> distortedGeometry, int interpolation,
                                         double alpha, double scale) {
    init(distortedGeometry, interpolation, alpha, scale);
}

template <typename MASK_T>
OmniUndistorter<MASK_T>::OmniUndistorter(boost::shared_ptr<distorted_geometry_t> distortedGeometry,
...
template <typename MASK_T>
void OmniUndistorter<MASK_T>::init(boost::shared_ptr<distorted_geometry_t> distortedGeometry, int interpolation,
                                   double alpha, double scale) {
...
template <typename MASK_T>
void OmniUndistorter<MASK_T>::constructUndistortedFrame(const cv::Mat& image, frame_t& outFrame) const {
    SM_ASSERT_TRUE(std::runtime_error, _idealGeometry, "Camera has not yet been set.")
    cv::Mat undistImage;
...
template <typename MASK_T>
void OmniUndistorter<MASK_T>::undistortImage(const cv::Mat& inImage, cv::Mat& outImage) const {
...
void initUndistOmniToPinholeMap(boost::shared_ptr<CAMERA_T> camera_geometry,
                                const Eigen::Matrix3d& _pinholeCameraMatrix, cv::Size size, cv::OutputArray _map1,
                                cv::OutputArray _map2) {
    using namespace cv;
    ...
    for (int i = 0; i < size.height; i++) {
        ...
        double _x = i * invR(0, 1) + invR(0, 2), _y = i * invR(1, 1) + invR(1, 2), _w = i * invR(2, 1) + invR(2, 2);

        for (int j = 0; j < size.width; j++, _x += invR(0, 0), _y += invR(1, 0), _w += invR(2, 0)) {
...
template <typename MASK_T>
void OmniUndistorter<MASK_T>::undistortImageToPinhole(const cv::Mat& inImage, cv::Mat& outImage) const {
...
template <typename MASK_T>
boost::shared_ptr<typename OmniUndistorter<MASK_T>::ideal_geometry_t> OmniUndistorter<MASK_T>::idealGeometry() const {
    return _idealGeometry;
}

template <typename MASK_T>
boost::shared_ptr<typename OmniUndistorter<MASK_T>::ideal_pinhole_geometry_t>
OmniUndistorter<MASK_T>::idealPinholeGeometry() const {
    return _idealPinholeGeometry;
}

template <typename MASK_T>
boost::shared_ptr<typename OmniUndistorter<MASK_T>::distorted_geometry_t> OmniUndistorter<MASK_T>::distortedGeometry()
    const {
    return _distortedGeometry;
}

template <typename MASK_T>
void OmniUndistorter<MASK_T>::setIdealGeometry(boost::shared_ptr<distorted_geometry_t> distortedGeometry, double alpha,
                                               double scale) {
    _distortedGeometry = distortedGeometry;
    Eigen::Matrix3d distortedCameraMatrix = distortedGeometry->projection().getCameraMatrix();

    // get distortion: (k1, k2, p1, p2)'
    Eigen::MatrixXd distortionParameters;
    distortedGeometry->projection().distortion().getParameters(distortionParameters);
    ...
    // see:
    // http://opencv.willowgarage.com/documentation/cpp/calib3d_camera_calibration_and_3d_reconstruction.html#getOptimalNewCameraMatrix
    cv::Mat idealCameraMatrixCV = cv::getOptimalNewCameraMatrix(
        distortedCameraMatrixCV, distortionParametersCV, cv::Size(width, height), alpha, cv::Size(newWidth, newHeight));
    ...
    typename ideal_geometry_t::projection_t idealProjection(
        distortedGeometry->projection().xi(), idealCameraMatrix(0, 0), idealCameraMatrix(1, 1), idealCameraMatrix(0, 2),
        idealCameraMatrix(1, 2), newWidth, newHeight, typename ideal_geometry_t::projection_t::distortion_t());

    // set new idealGeometry
    _idealGeometry = boost::shared_ptr<ideal_geometry_t>(
        new ideal_geometry_t(idealProjection, distortedGeometry->shutter(), distortedGeometry->mask()));

    // compute the undistortion and rectification transformation map
    // see:
    // http://opencv.willowgarage.com/documentation/cpp/calib3d_camera_calibration_and_3d_reconstruction.html#initUndistortRectifyMap
    cv::initUndistortRectifyMap(distortedCameraMatrixCV, distortionParametersCV, cv::Mat(), idealCameraMatrixCV,
                                cv::Size(newWidth, newHeight), CV_16SC2, mapX, mapY);
}
...
void OmniUndistorter<MASK_T>::setIdealPinholeGeometry(boost::shared_ptr<ideal_geometry_t> idealGeometry) {
    ...
    // get surrounding box
    Eigen::Vector2d keypoint_top_left(0, 0), keypoint_bottom_right(width, height);
    Eigen::Vector3d point_top_left, point_bottom_right;
    ...
    initUndistOmniToPinholeMap(idealGeometry, newCameraMatrix, newsize, mapX_pinhole, mapY_pinhole);

    // create pinhole camera
    typename ideal_pinhole_geometry_t::projection_t idealPinholeProjection(
        newCameraMatrix(0, 0), newCameraMatrix(1, 1), newCameraMatrix(0, 2), newCameraMatrix(1, 2), width, height,
        typename ideal_pinhole_geometry_t::projection_t::distortion_t());

    _idealPinholeGeometry = boost::shared_ptr<ideal_pinhole_geometry_t>(new ideal_pinhole_geometry_t(
        idealPinholeProjection, _distortedGeometry->shutter(), _distortedGeometry->mask()));
}

template <typename MASK_T>
boost::shared_ptr<FrameBase> OmniUndistorter<MASK_T>::buildFrame(cv::Mat& inImage) {
