...
static void icvGetRectangles(boost::shared_ptr<CAMERA_T> camera_geometry, CvSize imgSize, cv::Rect_<float>& inner,
    ...
    for (y = k = 0; y < N; y++) {
        for (x = 0; x < N; x++) {
            Eigen::Vector2d point(x * imgSize.width / (N - 1), y * imgSize.height / (N - 1));

            // normalize
            Eigen::Matrix3d cameraMatrix = camera_geometry->projection().getCameraMatrix();
...
///
template <typename CAMERA_T>
Eigen::Matrix3d getOptimalNewCameraMatrix(boost::shared_ptr<CAMERA_T> camera_geometry, CvSize imgSize, double alpha,
                                          CvSize newImgSize) {
...
///
template <typename CAMERA_T>
void initUndistortRectifyMap(boost::shared_ptr<CAMERA_T> camera_geometry, const Eigen::Matrix3d& _R,
                             const Eigen::Matrix3d& _newCameraMatrix, Size size, int m1type, OutputArray _map1,
                             OutputArray _map2) {
