...
void calculateOverlappingFOV(boost::shared_ptr<CAMERA_1_T> camera1, cameras::ImageMask& outMask1,
                             double& outOverlappingRatio1, const Eigen::VectorXi& /* sampleDistances */, double scale) {
    // get dimensions
...
void calculateOverlappingFOV(boost::shared_ptr<CAMERA_1_T> camera1, boost::shared_ptr<CAMERA_1_T> camera2,
                             const sm::kinematics::Transformation& T_cam1_cam2, cameras::ImageMask& outMask1,
                             cameras::ImageMask& outMask2, double& outOverlappingRatio1, double& outOverlappingRatio2,
                             const Eigen::VectorXi& sampleDistances, double scale) {
    // get dimensions
    int width1 = (int)camera1->projection().ru() * scale;
    int height1 = (int)camera1->projection().rv() * scale;
    ...
    for (int x = 0; x < width1; x++) {
        for (int y = 0; y < height1; y++) {
            ...
            if (camera1->projection().keypointToHomogeneous(Eigen::Vector2d((int)x / scale, (int)y / scale), point)) {
                double norm = point.norm();
                int i = 0;
                // check points on the beam until one is found or no other points to check
                while (i < sampleDistances.size() && outMask1CV.at<unsigned char>(y, x) == 0) {
                    ...
                    if (camera2->projection().homogeneousToKeypoint(tempPoint, keypointLocation)) {
    ...
    for (int x = 0; x < width2; x++) {
        for (int y = 0; y < height2; y++) {
            // if visible
            // TODO: BB: what about rounding mistakes?
            if (camera2->projection().keypointToHomogeneous(Eigen::Vector2d((int)x / scale, (int)y / scale), point)) {
                double norm = point.norm();
                int i = 0;
                // check points on the beam until one is found or no other points to check
                while (i < sampleDistances.size() && outMask2CV.at<unsigned char>(y, x) == 0) {
                    // Send point to distance sampleDistances(i) on the beam
                    Eigen::Vector4d tempPoint = point;
                    tempPoint(3) = norm / (norm + sampleDistances(i));
                    ...
                    Eigen::Vector2d keypointLocation;
                    if (camera1->projection().homogeneousToKeypoint(tempPoint, keypointLocation)) {
