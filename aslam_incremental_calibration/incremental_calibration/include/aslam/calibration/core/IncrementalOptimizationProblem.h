...
/** The class IncrementalOptimizationProblem implements a container for
  ...
  */
class IncrementalOptimizationProblem : public aslam::backend::OptimizationProblemBase {
  public:
    ...
    typedef std::unordered_map<const DesignVariable*, std::pair<size_t, size_t> > DesignVariablesPCountId;
    ...
    typedef std::unordered_map<size_t, DesignVariablesP> DesignVariablePGroups;
    /// Error term type
    ...
    typedef std::unordered_map<DesignVariable*, Eigen::MatrixXd> DesignVariablesBackup;
    /// Self type
    ...
    void permuteDesignVariables(const std::vector<size_t>& permutation, size_t groupId);
    ...
    const OptimizationProblem* getOptimizationProblem(const OptimizationProblemsSPCIt& problemIt) const;
    /// Returns an optimization problem from an iterator
    OptimizationProblem* getOptimizationProblem(const OptimizationProblemsSPIt& problemIt);
    ...
    OptimizationProblemsSPIt getOptimizationProblem(const OptimizationProblemSP& problem);
    ...
    bool isDesignVariableInProblem(const DesignVariable* designVariable) const;
  ...
  protected:
    ...
    /// Returns design variable indexed by idx
    virtual const DesignVariable* designVariableImplementation(size_t idx) const;
