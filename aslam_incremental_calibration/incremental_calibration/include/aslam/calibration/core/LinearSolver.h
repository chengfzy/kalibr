...
/** The class LinearSolver implements a specific linear solver for
  ...
  */
class LinearSolver : public aslam::backend::LinearSystemSolver {
  public:
     ...
     */
    void solve(cholmod_sparse* A, cholmod_dense* b, std::ptrdiff_t j, Eigen::VectorXd& x);
    /**
    ...
    const aslam::backend::CompressedColumnMatrix<std::ptrdiff_t>& getJacobianTranspose() const;
  ...
  protected:
    ...
    virtual void initMatrixStructureImplementation(const std::vector<aslam::backend::DesignVariable*>& dvs,
    ...
    /// Jacobian builder
    aslam::backend::CompressedColumnJacobianTransposeBuilder<std::ptrdiff_t> _jacobianBuilder;
