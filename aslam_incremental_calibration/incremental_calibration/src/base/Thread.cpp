...
#else
static pid_t gettid(void) { return -ENOSYS; }
#endif
...
Thread::Identifier::Identifier(pthread_t posix) : mPosix(posix), mKernel(-1), mProcess(-1) {}

Thread::Identifier::Identifier(const Identifier& other)
    : mPosix(other.mPosix), mKernel(other.mKernel), mProcess(other.mProcess) {}
...
Thread::Thread(double cycle, size_t stackSize)
    : mState(initialized), mCancel(false), mPriority(inherit), mStackSize(stackSize), mCycle(cycle), mNumCycles(0) {}
...
void Thread::safeSetPriority(Priority priority) {
    ...
    if ((mIdentifier.mPosix != 0) && (priority != inherit)) {
        ...
        if (ret) throw SystemException(ret, "Thread::safeSetPriority()::pthread_getschedparam()");
        if (!ret) {
            const int minPriority = sched_get_priority_min(policy);
            const int maxPriority = sched_get_priority_max(policy);
            if ((minPriority != -1) && (maxPriority != -1)) {
                param.sched_priority = minPriority + round((maxPriority - minPriority) / critical * priority);
                ret = pthread_setschedparam(mIdentifier.mPosix, policy, &param);
                if (ret) throw SystemException(ret, "Thread::safeSetPriority()::pthread_setschedparam()");
            }
        }
    }
}

size_t Thread::getStackSize() const { return mStackSize; }
...
const Timer& Thread::getTimer() const { return mTimer; }

Condition& Thread::getTrigger() { return mTrigger; }

const Condition& Thread::getTrigger() const { return mTrigger; }
...
void Thread::Identifier::write(std::ostream& stream) const { stream << mPosix; }
...
bool Thread::Identifier::operator==(const Identifier& identifier) const { return (mPosix == identifier.mPosix); }
...
Thread::Identifier::operator const void*() const { return (void*)mPosix; }
...
bool Thread::start(Priority priority, double wait) {
    ...
    if (safeWait(wait)) {
        ...
        if (ret) throw SystemException(ret, "Thread::start()::pthread_attr_init()");
        ret = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
        if (ret) throw SystemException(ret, "Thread::start()::pthread_attr_setdetachstate()");
        ret = pthread_attr_setinheritsched(&attr, PTHREAD_INHERIT_SCHED);
        if (ret) throw SystemException(ret, "Thread::start()::pthread_attr_setinheritsched()");
        if (mStackSize) {
            ret = pthread_attr_setstacksize(&attr, mStackSize);
            if (ret) throw SystemException(ret, "Thread::start()::pthread_attr_setstacksize()");
        }
        mNumCycles = 0;
        State state = mState;
        safeSetState(starting);
        ret = pthread_create(&mIdentifier.mPosix, &attr, Thread::start, this);
        if (ret) throw SystemException(ret, "Thread::start()::pthread_create()");
        result = !ret;
        if (result) {
        ...
        if (ret) throw SystemException(ret, "Thread::start()::pthread_attr_destroy()");
    }
    return result;
}
...
bool Thread::interrupt(double wait) {
    ...
    if (safeIsBusy()) {
        mCancel = true;
        if (mState == running) safeSetState(interrupting);
        mTrigger.signal();
...
bool Thread::safeWait(double seconds) const {
    ...
    if (seconds != 0.0) {
        ...
        if (self == this) throw InvalidOperationException("Thread::safeWait(): bad wait");
        if (safeIsBusy()) result = mCleaned.wait(mMutex, seconds);
    }
    return result;
}
...
bool Thread::safeExists() const { return (mIdentifier.mPosix != 0); }
...
bool Thread::safeIsBusy() const { return ((mState != initialized) && (mState != interrupted) && (mState != finished)); }
...
void* Thread::start(void* thread) { return ((Thread*)thread)->run(); }
