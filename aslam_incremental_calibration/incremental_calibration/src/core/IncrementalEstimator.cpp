...
IncrementalEstimator::IncrementalEstimator(size_t groupId, const Options& options,
    ...
    optOptions.linearSystemSolver = boost::make_shared<LinearSolver>(linearSolverOptions);
...
IncrementalEstimator::IncrementalEstimator(const sm::PropertyTree& config)
    ...
    _optimizer = boost::make_shared<Optimizer>(
        ...
        boost::make_shared<LinearSolver>(sm::PropertyTree(config, "optimizer/linearSolver")),
    ...
    _options.infoGainDelta = config.getDouble("infoGainDelta", _options.infoGainDelta);
    _options.checkValidity = config.getBool("checkValidity", _options.checkValidity);
...
const IncrementalOptimizationProblem* IncrementalEstimator::getProblem() const { return _problem.get(); }

const IncrementalEstimator::Options& IncrementalEstimator::getOptions() const { return _options; }

IncrementalEstimator::Options& IncrementalEstimator::getOptions() { return _options; }

const LinearSolverOptions& IncrementalEstimator::getLinearSolverOptions() const {
...
const IncrementalEstimator::OptimizerOptions& IncrementalEstimator::getOptimizerOptions() const {
    return _optimizer->options();
}

IncrementalEstimator::OptimizerOptions& IncrementalEstimator::getOptimizerOptions() { return _optimizer->options(); }

size_t IncrementalEstimator::getMargGroupId() const { return _margGroupId; }

double IncrementalEstimator::getInformationGain() const { return _informationGain; }

const aslam::backend::CompressedColumnMatrix<std::ptrdiff_t>& IncrementalEstimator::getJacobianTranspose() const {
    return _optimizer->getSolver<LinearSolver>()->getJacobianTranspose();
}

std::ptrdiff_t IncrementalEstimator::getRankPsi() const { return _rankPsi; }

std::ptrdiff_t IncrementalEstimator::getRankPsiDeficiency() const { return _rankPsiDeficiency; }

std::ptrdiff_t IncrementalEstimator::getRankTheta() const { return _rankTheta; }

std::ptrdiff_t IncrementalEstimator::getRankThetaDeficiency() const { return _rankThetaDeficiency; }

double IncrementalEstimator::getSVDTolerance() const { return _svdTolerance; }

double IncrementalEstimator::getQRTolerance() const { return _qrTolerance; }

size_t IncrementalEstimator::getPeakMemoryUsage() const { return _peakMemoryUsage; }

size_t IncrementalEstimator::getMemoryUsage() const { return _memoryUsage; }

double IncrementalEstimator::getNumFlops() const { return _numFlops; }

const Eigen::MatrixXd& IncrementalEstimator::getNobsBasis(bool scaled) const {
    if (scaled)
...
const Eigen::MatrixXd& IncrementalEstimator::getSigma2ThetaObs(bool scaled) const {
    if (scaled)
...
const Eigen::VectorXd& IncrementalEstimator::getSingularValues(bool scaled) const {
    if (scaled)
...
double IncrementalEstimator::getInitialCost() const { return _initialCost; }

double IncrementalEstimator::getFinalCost() const { return _finalCost; }
...
IncrementalEstimator::ReturnValue IncrementalEstimator::reoptimize() {
    ...
    size_t JCols = 0;
    for (auto it = _problem->getGroupsOrdering().cbegin(); it != _problem->getGroupsOrdering().cend(); ++it)
...
IncrementalEstimator::ReturnValue IncrementalEstimator::addBatch(const BatchSP& problem, bool force) {
    // query the time
    const double timeStart = Timestamp::now();
    ...
    if (!force) _problem->saveDesignVariables();
    ...
    if (_options.checkValidity && (srv.iterations == _optimizer->options().maxIterations || srv.JFinal >= srv.JStart))
        solutionValid = false;
    ...
    if (((ret.informationGain > _options.infoGainDelta || ret.rankTheta > _rankTheta) && solutionValid) || force) {
        // warning for rank going down
        if (ret.rankTheta < _rankTheta && _options.verbose)
            std::cerr << "IncrementalEstimator::addBatch(): "
                         "WARNING: RANK GOING DOWN!"
                      << std::endl;
    ...
    if (!keepBatch) {
        ...
        if (_problem->getNumOptimizationProblems() > 0) restoreLinearSolver();
    }
...
size_t IncrementalEstimator::getNumBatches() const { return _problem->getNumOptimizationProblems(); }

void IncrementalEstimator::orderMarginalizedDesignVariables() {
    auto groupsOrdering = _problem->getGroupsOrdering();
    auto margGroupIt = std::find(groupsOrdering.begin(), groupsOrdering.end(), _margGroupId);
