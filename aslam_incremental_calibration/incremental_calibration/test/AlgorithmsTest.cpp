...
TEST(AslamCalibrationTestSuite, testAlgorithms) {
    ...
    const std::vector<double> input = {dist.getSample(), dist.getSample(), dist.getSample(),
                                       dist.getSample(), dist.getSample(), dist.getSample()};
    const std::vector<size_t> p = {1, 3, 0, 2, 5, 4};
    auto pinput = input;
    permute(pinput, p);
    ASSERT_EQ(pinput,
              std::vector<double>({input[p[0]], input[p[1]], input[p[2]], input[p[3]], input[p[4]], input[p[5]]}));
    ASSERT_THROW(permute(pinput, std::vector<size_t>({1, 3, 0})), OutOfBoundException<size_t>);
    ASSERT_THROW(permute(pinput, std::vector<size_t>({10, 3, 0, 2, 5, 4})), OutOfBoundException<size_t>);
    ...
    sm::eigen::assertNear(JCov1, JCov2, 1e-6, SM_SOURCE_FILE_POS, "J'*J and R'*R does not match");
    Eigen::MatrixXd JCov3 = computeCovariance(RFactorSparse, 0, RFactorSparse.cols() - 1);
    sm::eigen::assertNear(JCov1, JCov3, 1e-6, SM_SOURCE_FILE_POS, "J'*J and covariance recovery does not match");
    ...
    const double sumLogDiagR1 = computeSumLogDiagR(RFactorSparse, 0, RFactorSparse.cols() - 1);
    const double sumLogDiagR2 = computeSumLogDiagR(RFactor, 0, RFactor.cols() - 1);
    ...
    ASSERT_NEAR(sumLogDiagR1, std::fabs(std::log2(std::fabs(JCov1Det))) / 2, 1e-9);
    Eigen::MatrixXd NS, CS, Sigma, SigmaP, Omega;
    const double svLogSum = marginalize(JTransposeSparse, 0, NS, CS, Sigma, SigmaP, Omega);
    sm::eigen::assertNear(JCov1, Sigma, 1e-6, SM_SOURCE_FILE_POS, "SVD covariance recovery failed");
