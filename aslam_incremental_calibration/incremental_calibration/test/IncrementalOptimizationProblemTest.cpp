...
TEST(AslamCalibrationTestSuite, testIncrementalOptimizationProblem) {
    ...
    ASSERT_THROW(incProblem.getOptimizationProblem(3), OutOfBoundException<size_t>);
    ...
    ASSERT_EQ(dvs1, IncrementalOptimizationProblem::DesignVariablesP({dv3.get()}));
    ASSERT_THROW(incProblem.getDesignVariablesGroup(2), OutOfBoundException<size_t>);
    auto ets1 = incProblem.getErrorTerms(0);
    ASSERT_EQ(ets1, IncrementalOptimizationProblem::ErrorTermsSP({et1, et2, et3, et4}));
    auto ets2 = incProblem.getErrorTerms(1);
    ASSERT_EQ(ets2, IncrementalOptimizationProblem::ErrorTermsSP({et5, et6, et7}));
    ...
    ASSERT_THROW(incProblem.setGroupsOrdering({1, 0, 2}), OutOfBoundException<size_t>);
    ASSERT_THROW(incProblem.setGroupsOrdering({1, 2}), OutOfBoundException<size_t>);
    ASSERT_THROW(incProblem.setGroupsOrdering({0, 0}), OutOfBoundException<size_t>);
    ...
    ASSERT_EQ(dvs0update,
              IncrementalOptimizationProblem::DesignVariablesP({dv2.get(), dv1.get(), dv4.get(), dv5.get()}));
