...
TEST(AslamCalibrationTestSuite, testOptimizationProblem) {
    ...
    ASSERT_EQ(problem.getDesignVariablesGroup(1), OptimizationProblem::DesignVariablesSP({dv2, dv3}));
    ASSERT_EQ(problem.getDesignVariablesGroup(0), OptimizationProblem::DesignVariablesSP({dv1}));
    ...
    ASSERT_THROW(problem.setGroupsOrdering({2, 0, 3}), OutOfBoundException<size_t>);
    ...
    ASSERT_THROW(problem.permuteDesignVariables({1, 0}, 2), OutOfBoundException<size_t>);
