...
TEST(AslamCalibrationTestSuite, testVectorDesignVariable) {
    ...
    ASSERT_EQ(dv1.getValue(), aslam::calibration::VectorDesignVariable<3>::Container::Zero());
    dv1.setValue(aslam::calibration::VectorDesignVariable<3>::Container::Ones());
    ASSERT_EQ(dv1.getValue(), aslam::calibration::VectorDesignVariable<3>::Container::Ones());

    // Constructor with initial value using static size
    aslam::calibration::VectorDesignVariable<3> dv2(aslam::calibration::VectorDesignVariable<3>::Container::Ones());

    // Constructor with initial value using dynamic size
    aslam::calibration::VectorDesignVariable<Eigen::Dynamic> dv3(
        aslam::calibration::VectorDesignVariable<Eigen::Dynamic>::Container::Random(5, 1));
    dv3.setValue(aslam::calibration::VectorDesignVariable<Eigen::Dynamic>::Container::Ones(5, 1));
    ASSERT_EQ(dv3.getValue(), aslam::calibration::VectorDesignVariable<Eigen::Dynamic>::Container::Ones(5, 1));
    ...
    ASSERT_THROW(dv1.setParameters(Eigen::Vector2d::Ones()), aslam::calibration::OutOfBoundException<int>);
}
