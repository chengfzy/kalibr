...
When a shared_ptr is returned the problemOwnsErrorTerms flag is ignored. Otherwise true makes the problem deleting the
...
void addQuadraticIntegralErrorTerms(OptimizationProblem& problem, const TTime& a, const TTime& b, int numberOfPoints,
    ...
    SM_ASSERT_TRUE_DBG(std::runtime_error, !integrator.isAtEnd(),
                       "too few integration points given : " << numberOfPoints);
    ...
    for (; !integrator.isAtEnd(); integrator.next()) {
        ...
        internal::addErrorTermToProblem(problem,
                                        errorTermFactory(integrator.getIntegrationScalar(), commonFactor * valueFactor),
                                        problemOwnsErrorTerms);
    }
}
