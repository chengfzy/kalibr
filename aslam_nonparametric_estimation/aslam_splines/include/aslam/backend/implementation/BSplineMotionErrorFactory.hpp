...
void addMotionErrorTerms(OptimizationProblemBase& problem, SplineDv& splineDv, const Eigen::MatrixXd& W,
    ...
    for (int i = 0; i < spline.numValidTimeSegments(); ++i) {
        ...
        MarginalizationPriorErrorTerm::Ptr err(new MarginalizationPriorErrorTerm(dvs, R * c, R));
