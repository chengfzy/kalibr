...
BSplineDesignVariable<D>::BSplineDesignVariable(const bsplines::BSpline &bspline) : _bspline(bspline) {
    ...
    for (int i = 0; i < _bspline.numVvCoefficients(); ++i) {
        _designVariables.push_back(
            new aslam::backend::DesignVariableMappedVector<D>(_bspline.fixedSizeVvCoefficientVector<D>(i)));
    }
}
...
aslam::backend::VectorExpression<D> BSplineDesignVariable<D>::toExpression(double tk, int derivativeOrder) {
    ...
    boost::shared_ptr<aslam::splines::BSplineVectorExpressionNode<D> > root(
        new aslam::splines::BSplineVectorExpressionNode<D>(&_bspline, derivativeOrder, dvs, tk));
