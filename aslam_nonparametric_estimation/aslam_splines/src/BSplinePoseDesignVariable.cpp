...
BSplinePoseDesignVariable::BSplinePoseDesignVariable(const bsplines::BSplinePose &bsplinePose)
    ...
    for (int i = 0; i < _bsplinePose.numVvCoefficients(); ++i) {
        _designVariables.push_back(
            new aslam::backend::DesignVariableMappedVector<6>(_bsplinePose.fixedSizeVvCoefficientVector<6>(i)));
    }
}
...
aslam::backend::TransformationExpression BSplinePoseDesignVariable::transformation(double tk) {
    ...
    boost::shared_ptr<BSplineTransformationExpressionNode> root(
        new BSplineTransformationExpressionNode(&_bsplinePose, dvs, tk));
...
aslam::backend::EuclideanExpression BSplinePoseDesignVariable::linearAcceleration(double tk) {
    ...
    boost::shared_ptr<BSplineAccelerationExpressionNode> root(
        new BSplineAccelerationExpressionNode(&_bsplinePose, dvs, tk));
...
aslam::backend::EuclideanExpression BSplinePoseDesignVariable::angularVelocityBodyFrame(double tk) {
    ...
    boost::shared_ptr<BSplineAngularVelocityBodyFrameExpressionNode> root(
        new BSplineAngularVelocityBodyFrameExpressionNode(&_bsplinePose, dvs, tk));
...
aslam::backend::EuclideanExpression BSplinePoseDesignVariable::angularAccelerationBodyFrame(double tk) {
    ...
    boost::shared_ptr<BSplineAngularAccelerationBodyFrameExpressionNode> root(
        new BSplineAngularAccelerationBodyFrameExpressionNode(&_bsplinePose, dvs, tk));
...
void BSplinePoseDesignVariable::addSegment(double t, Eigen::Matrix4d T) {
    ...
    _designVariables.push_back(new aslam::backend::DesignVariableMappedVector<6>(
        _bsplinePose.fixedSizeVvCoefficientVector<6>(_bsplinePose.numVvCoefficients() - 1)));
