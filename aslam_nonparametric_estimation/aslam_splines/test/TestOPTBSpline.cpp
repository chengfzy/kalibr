...
template <typename TConf, int ISplineOrder, int IDim, bool BDimRequired>
struct ConfCreator {
    static inline TConf create() { return TConf(typename TConf::ManifoldConf(IDim), ISplineOrder); }
};

template <typename TConf, int ISplineOrder, int IDim>
struct ConfCreator<TConf, ISplineOrder, IDim, false> {
...
struct OPTSplineSpecializationTester {
    ...
    static void test(TestBSpline& /* spline */, double /* t */,
                     typename TestBSpline::TimeExpression /* timeExpression */) {}
};
...
struct OPTSplineTester {
    ...
    OPTSplineTester() : bspline(createMyConf()) {
        bspline.initConstantUniformSpline(0, duration * TimePolicy::getOne(), numSegments,
                                          bspline.getManifold().getDefaultPoint());
        for (auto& i : bspline) {
  ...
  public:
    ...
    void testUpdate() {
        ...
        for (typename TestBSpline::SegmentIterator i = bspline.begin(), end = bspline.end(); i != end; i++) {
            ...
            point_t opu = op;
            ::manifolds::internal::DiffManifoldPointUpdateTraits<typename TSplineMap::CONF::ManifoldConf>::update(
                bspline.getManifold(), opu, Eigen::Map<const tangent_vector_t>(update, IDim));
    ...
    constexpr static std::uintmax_t getTimeUnit() { return (std::uintmax_t)TimePolicy::getOne(); }
    ...
    void testExpressions() {
        ...
        for (int k = 0; k < numberOfTimesToProbe; k++) {
            time_t t =
                ((k % 2 == 0) ? ((double)(k / 2) / ((numberOfTimesToProbe - 1) / 2)) : ((double)rand() / RAND_MAX)) *
                (duration * TimePolicy::getOne());
            ...
            typename TestBSpline::TimeExpression variableOffset(offsetVar.toExpression()),
                constOffset(variableOffset.evaluate()), timePoint(t),
            ...
            auto factTimeExp = bspline.template getExpressionFactoryAt<maxDerivativeOrder>(
                timeExpression, t / 2,
                t == bspline.getMaxTime() ? bspline.getMaxTime() + 1 : (t + bspline.getMaxTime()) / 2);

            typename TestBSpline::template Evaluator<maxDerivativeOrder> eval =
                bspline.template getEvaluatorAt<maxDerivativeOrder>(t);
            ...
            for (int derivativeOrder = 0; derivativeOrder <= maxDerivativeOrder; derivativeOrder++) {
                ...
                sm::eigen::assertEqual(fact.getEvaluator().evalD(derivativeOrder), expression.evaluate(),
                                       SM_SOURCE_FILE_POS);
                sm::eigen::assertEqual(eval.evalD(derivativeOrder), expression.evaluate(), SM_SOURCE_FILE_POS);
                sm::eigen::assertEqual(factTimeExp.getEvaluator().evalD(derivativeOrder), expressionTimeExp.evaluate(),
                                       SM_SOURCE_FILE_POS);
                sm::eigen::assertNear(expressionTimeExp.evaluate(), expression.evaluate(), 16 * eps,
                                      SM_SOURCE_FILE_POS);
                ...
                if (t != bspline.getMaxTime() && t != bspline.getMinTime()) {
                    ...
                    SCOPED_TRACE("");
                    testJacobian(expressionTimeExp, dvIndexCounter, false,
                                 test::ExpressionTraits<decltype(expressionTimeExp)>::defaultTolerance(),
                                 std::numeric_limits<time_t>::is_integer
                                     ? 1E-7
                                     : test::ExpressionTraits<decltype(expressionTimeExp)>::defaulEps());
                }
...
struct MaxDerivative<UnitQuaternionBSpline<IEigenSplineOrder, TTimePolicy, TScalar>, ISplineOrder> {
    enum {
        LIMIT = UnitQuaternionBSpline<IEigenSplineOrder>::TYPE::MaxSupportedDerivativeOrderJacobian,
        VALUE = LIMIT < ISplineOrder ? LIMIT : ISplineOrder
    };
};
...
struct OPTSplineSpecializationTester<UnitQuaternionBSpline<IEigenSplineOrder>, ISplineOrder, IDim> {
    ...
    static void test(TestBSpline& bspline, double t, typename TestBSpline::TimeExpression timeExpression) {
        ...
        auto factTimeExp = bspline.template getExpressionFactoryAt<3>(
            timeExpression, t / 2,
            t == bspline.getMaxTime() ? bspline.getMaxTime() + 1 : (t + bspline.getMaxTime()) / 2);
...
#ifndef COMPILE_FAST
    OPTSplineTester<EuclideanBSpline<2, Eigen::Dynamic>, 2, 3>, OPTSplineTester<EuclideanBSpline<3, 2>, 3, 2>,
...
TEST(OPTBSplineTestSuite, testPoseErrorWithOPTSplines) {
    try {
        ...
        RotationExpression rexp(
            Vector2RotationQuaternionExpressionAdapter::adapt(VectorExpression<4>(valueExpression)));
        sm::eigen::assertEqual(sm::kinematics::quat2r(valueExpression.toValue()), rexp.toRotationMatrix(),
                               SM_SOURCE_FILE_POS);
        ...
        {
            SCOPED_TRACE("");
            testExpression(T * he, testSpline.getSplineOrder() + 1);
        }
...
TYPED_TEST(OPTBSplineTestSuiteT2, testOptimizingEvaluationTime) {
    try {
        ...
        problem.addErrorTerm(
            toErrorTerm(convertToGME(testSpline.template getExpressionFactoryAt<0>(timeVar.toExpression(), 0, endTime)
                                         .getValueExpression(0)) -
                        GenericMatrixExpression<2, 1>(testSpline.template getEvaluatorAt<0>(halfTime).eval())));
