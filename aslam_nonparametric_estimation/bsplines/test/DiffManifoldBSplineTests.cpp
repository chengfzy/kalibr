...
TEST(DiffManifoldBSplineTestSuite, testInitialization) {
    ...
    SM_ASSERT_EQ(std::runtime_error, rbspline.firstRelevantSegment().getKnot(),
                 rbspline.getEvaluatorAt<0>(minTime)._firstRelevantControlVertexIt.getKnot(), "");
    ...
    SM_ASSERT_EQ(std::runtime_error, rbspline.getEvaluatorAt<0>(maxTime - 0.01).getKnot(),
                 rbspline.getEvaluatorAt<0>(maxTime).getKnot(), "");  // TODO improve: remove this irregularity
    ...
    for (; i != end; i++) {
        ...
        if (!(c < splineOrder - 1 || c >= numberOfSegments + splineOrder - 1)) {
            if (tKnot != maxTime) {
                ...
                SM_ASSERT_EQ(std::runtime_error, rbspline.getEvaluatorAt<0>(tKnot + 1e-13).getKnot(),
                             rbspline.getEvaluatorAt<0>(tKnot).getKnot(), "");
            } else {
...
TEST(DiffManifoldBSplineTestSuite, testCopyAndAssign) {
    ...
    for (int i = 0; i < nCopies; i++) {
        SM_ASSERT_EQ(std::runtime_error, rbsplineCopies[i].getAbsoluteNumberOfSegments(),
        ...
        sm::eigen::assertEqual(rbspline.getEvaluatorAt<0>(t).eval(), rbsplineCopies[i].getEvaluatorAt<0>(t).eval(),
                               SM_SOURCE_FILE_POS, "");
    }
}
...
TEST(DiffManifoldBSplineTestSuite, testAddingSegments) {
    ...
    for (unsigned int i = 0; i < 10; i++) {
        ...
        double lastKnot = minTime;
        unsigned int c = 0;
        for (TestSpline::SegmentConstIterator it = rbspline.getAbsoluteBegin(), end = rbspline.getAbsoluteEnd(),
                                              itToCompare = rbsplineToCompare.getAbsoluteBegin();
             it != end; it++) {
            if (c < vertices.size()) {
                SM_ASSERT_EQ(std::runtime_error, it->getControlVertex(), itToCompare->getControlVertex(),
                             "control vertices should match at segment data " << c);
                if (c > splineOrder - 1)
                    SM_ASSERT_EQ(std::runtime_error, it->getBasisMatrix(), itToCompare->getBasisMatrix(),
                                 "basis matrices should match at segment data " << c);
            }
...
TEST(DiffManifoldBSplineTestSuite, testGetBi) {
    ...
    for (int i = 0; i <= numTimeSteps; i++) {
        ...
        SM_ASSERT_NEAR(std::runtime_error, localBiVector.sum(), 1.0, 1e-10,
                       "the bi at a given time should always sum up to 1")
        ...
        SM_ASSERT_EQ(
            std::runtime_error,
            getIteratorDistance(firstIndex, lastIndex, (TestSpline::SegmentConstIterator)rbspline.end()),
            splineOrder - 1,
            "the distance between first and last local control point has to be exactly the spline order - 1. (at t = "
                << t << ")");

        SM_ASSERT_NEAR(std::runtime_error, (double)localCumulativeBiVector[0], 1.0, 1e-10, "");

        sm::eigen::assertNear(bspline.getLocalBiVector(t), rbspline.getEvaluatorAt<0>(t).getLocalBi(), 1E-9,
                              SM_SOURCE_FILE_POS);
        sm::eigen::assertNear(bspline.getLocalBiVector(t), rbsplineDS.getEvaluatorAt<0>(t).getLocalBi(), 1E-9,
                              SM_SOURCE_FILE_POS);
        sm::eigen::assertNear(bspline.getLocalCumulativeBiVector(t),
                              rbspline.getEvaluatorAt<0>(t).getLocalCumulativeBi(), 1E-9, SM_SOURCE_FILE_POS);
        sm::eigen::assertNear(bspline.getLocalCumulativeBiVector(t),
                              rbsplineDS.getEvaluatorAt<0>(t).getLocalCumulativeBi(), 1E-9, SM_SOURCE_FILE_POS);
        ...
        for (int j = 0, n = splineOrder; j < n; j++) {
            if (j > 0) {
                SM_ASSERT_LE(std::runtime_error, localCumulativeBiVector[j], localCumulativeBiVector[j - 1] + 1E-11,
                             " at t = " << t << ", j = " << j);
            }
            SM_ASSERT_NEAR(
                std::runtime_error, (double)localCumulativeBiVector[j],
                (double)localBiVector.segment(j, splineOrder - j).sum(), 1e-13,
                "cumulativeBiVector must be the sum of the localBiVector where it overlaps, but it is not at "
                    << j << " (localBiVector=" << localBiVector << ")");
        }
    }
}
...
#ifdef SPEEDMEASURE
TEST(ZLASTDiffManifoldBSplineTestSuite, printTimings) { sm::timing::Timing::print(std::cout); }
#endif
