...
inline bool operator<(const struct LongTime a, const struct LongTime b) { return a.v < b.v; }
inline bool operator<=(const struct LongTime a, const struct LongTime b) { return a.v <= b.v; }
inline bool operator>(const struct LongTime a, const struct LongTime b) { return a.v > b.v; }
inline bool operator>=(const struct LongTime a, const struct LongTime b) { return a.v >= b.v; }
inline bool operator==(const struct LongTime& a, const struct LongTime& b) { return a.v == b.v; }
inline std::ostream& operator<<(std::ostream& out, const LongDuration x) { return out << x.v; }

inline bool operator<(const struct LongDuration a, const struct LongDuration b) { return a.v < b.v; }
inline bool operator<=(const struct LongDuration a, const struct LongDuration b) { return a.v <= b.v; }
inline bool operator>(const struct LongDuration a, const struct LongDuration b) { return a.v > b.v; }
inline bool operator>=(const struct LongDuration a, const struct LongDuration b) { return a.v >= b.v; }
inline bool operator==(const struct LongDuration a, const struct LongDuration b) { return a.v == b.v; }
inline std::ostream& operator<<(std::ostream& out, const LongTime x) { return out << x.v; }

typedef struct {
    typedef struct LongTime time_t;
    typedef struct LongDuration duration_t;
    static duration_t computeDuration(time_t from, time_t till) { return LongDuration(till.v - from.v); }

    static double divideDurations(duration_t a, duration_t b) { return (double)a.v / b.v; }
    ...
    inline static double getDurationAsDouble(duration_t d) { return (double)d.v / getOne().v; }

    inline static duration_t getZero() { return LongDuration(0); }

    inline static duration_t getOne() { return LongDuration(1000000); }
...
struct InputTypeTraits {
    inline static InputT ones() { return InputT::Ones(); }

    inline static InputT random() { return InputT::Random(); }

    inline static void update(InputT& input, int c, double delta) { input[c] += delta; }
};
...
struct InputTypeTraits<double> {
    inline static double random() { return std::rand(); }
    inline static double ones() { return 1; }
    inline static void update(double& input, int /* c */, double delta) { input += delta; }
};
...
struct DExpTester : public JacobianTester<typename TDiffManifold::tangent_vector_t, typename TDiffManifold::point_t,
                                          typename TDiffManifold::dmatrix_t, DExpTester<TDiffManifold> > {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    typename TDiffManifold::point_t _pt;
    TDiffManifold _manifold;
    inline void eval(const typename TDiffManifold::tangent_vector_t& vec,
                     typename TDiffManifold::point_t& result) const {
        _manifold.expInto(_pt, vec, result);
    }
    inline void evalJac(const typename TDiffManifold::tangent_vector_t& vec,
                        typename TDiffManifold::dmatrix_t& result) const {
        _manifold.dexpInto(_pt, vec, result);
    }
    inline void randomParams() { _manifold.randomizePoint(_pt); }
...
struct SplineEvalDTester
    : public JacobianTester<Eigen::Matrix<double, 1, 1>, typename TSpline::point_t, typename TSpline::point_t,
                            SplineEvalDTester<TSpline, IDerivativeOrder> > {
    TSpline spline;
    SplineEvalDTester() { initMinimalSpline(spline); }
...
template <typename TSpline, int IDerivativeOrder>
struct SplineEvalRiDTester
    : public JacobianTester<Eigen::Matrix<double, 1, 1>, typename TSpline::point_t, typename TSpline::point_t,
                            SplineEvalRiDTester<TSpline, IDerivativeOrder> > {
    TSpline spline;
    int vectorPos;
    typename TSpline::tangent_vector_t vec;

    SplineEvalRiDTester() : vectorPos(0) { initMinimalSpline(spline); }

    template <int IDerivativeOrder2>
    inline void e(Eigen::Matrix<double, 1, 1> t, typename TSpline::point_t& result) const {
        ...
        cache.localRiPoints[vectorPos] =
            spline.getManifold().expAtId(vec * eval.getLocalCumulativeBi(0)[vectorPos + 1]);
...
inline void disturbePointInto(const TManifold& manifold, const typename TManifold::tangent_vector_t& vec,
                              const typename TManifold::point_t& p, typename TManifold::point_t& into) {
    into = p;
    manifolds::internal::DiffManifoldPointUpdateTraits<typename TManifold::configuration_t>::update(manifold, into,
                                                                                                    vec);
}

template <typename TSpline, int IDerivativeOrder>
struct SplineEvalRiDJacobianTester
    : public JacobianTester<typename TSpline::tangent_vector_t, typename TSpline::point_t, typename TSpline::dmatrix_t,
                            SplineEvalRiDJacobianTester<TSpline, IDerivativeOrder> > {
    TSpline spline;
    ...
    SplineEvalRiDJacobianTester() : vectorPos(0), controlVertexPos(0), _t(0) { initMinimalSpline(spline); }
    ...
    inline void randomizeInput(input_t& vec) { vec.setZero(); }
};
...
struct BSplineJacobianEvaluator {
    ...
    BSplineJacobianEvaluator() : _t(0), toBeDisturbedPoints(spline.getSplineOrder()) { initMinimalSpline(spline); }
    ...
    inline void eval(const input_t& input, typename TSpline::point_t& result) {
        updateSpline(input);
        typename TSpline::template Evaluator<IDerivativeOrder> eval =
            spline.template getEvaluatorAt<IDerivativeOrder>(_t);
        result = eval.evalD(IDerivativeOrder);
    }

    inline void evalJac(const input_t& input,
                        Eigen::Matrix<double, TSpline::PointSize,
                                      multiplyEigenSize(TSpline::Dimension, TSpline::SplineOrder)>& result) {
        updateSpline(input);
        typename TSpline::template Evaluator<IDerivativeOrder> eval =
            spline.template getEvaluatorAt<IDerivativeOrder>(_t);
    ...
    inline void randomizeInput(input_t& ret) { ret.setZero(); }
};
...
struct AngularDerivativeJacobianEvaluator : public BSplineJacobianEvaluator<TSpline, IDerivativeOrder> {
    ...
    inline void eval(const input_t& input, typename Eigen::Vector3d& result) {
        ...
        typename TSpline::template Evaluator<IDerivativeOrder> eval =
            this->spline.template getEvaluatorAt<IDerivativeOrder>(this->_t);
        result = eval.template evalAngularDerivative<IDerivativeOrder>();
    }

    inline void evalJac(const input_t& input,
                        Eigen::Matrix<double, TSpline::Dimension,
                                      multiplyEigenSize(TSpline::Dimension, TSpline::SplineOrder)>& result) {
        this->updateSpline(input);
        typename TSpline::template Evaluator<IDerivativeOrder> eval =
            this->spline.template getEvaluatorAt<IDerivativeOrder>(this->_t);
...
struct AngularDerivativeJacobianTestser
    : public JacobianTester<
          Eigen::Matrix<double, multiplyEigenSize(TSpline::Dimension, TSpline::SplineOrder), 1>, Eigen::Vector3d,
...
int getIteratorDistance(const Iterator& begin, const Iterator& end, const Iterator& limit) {
    int c = 0;
    Iterator i = begin;
    for (; i != end && i != limit; i++) {
        c++;
...
template <typename TSpline>
void copyKnots(TSpline& src, BSpline& dest) {
...
void setControlVerticesFromContainer(TSpline& spline, const Container& controlVertices) {
    ...
    spline.manipulateControlVertices(
        [&it, &controlVertices, &spline](int, typename TSpline::point_t& vertex) {
            vertex = *it;
            it++;
        },
        controlVertices.size());
}
...
void testDiffManifoldBSplineFitting(int numberOfSegments, double tolerance = 1E-9) {
    ...
    Eigen::Matrix<time_t, Eigen::Dynamic, 1> times =
        Eigen::Matrix<time_t, Eigen::Dynamic, 1>::LinSpaced(numberOfInterpolationPoints, minTime, maxTime);
    ...
    BSplineFitter<TestSpline>::initUniformSplineWithKnotDelta(rbspline, timesV, interpolationPointsV,
                                                              duration / numberOfSegments, lambda);
    auto knotGeneratorOrg =
        rbspline2.initConstantUniformSplineWithKnotDelta(minTime, maxTime, duration / numberOfSegments, identity);
    ...
    BSplineFitter<TestSpline>::fitSpline(rbspline2, timesV, interpolationPointsV, 0, 0, std::function<double(int)>(),
                                         FittingBackend::DENSE, true);
    ...
    BSplineFitter<TestSpline>::extendAndFitSpline(rbspline2, knotGenerator, timesV, interpolationPointsV, 0, 0,
                                                  FittingBackend::DENSE, true);
    ...
    BSplineFitter<TestSpline>::extendAndFitSpline(rbspline2, knotGenerator, timesV, interpolationPointsV, 0, 100,
                                                  FittingBackend::DENSE, true);
    ...
    BSplineFitter<TestSpline>::extendAndFitSpline(rbspline2, knotGenerator, timesV, interpolationPointsV, 0, 50,
                                                  FittingBackend::DENSE, true);

    sm::eigen::assertNear(p, rbspline2.template getEvaluatorAt<0>(maxTime).eval(), goalDist * 0.7, SM_SOURCE_FILE_POS,
                          "");
    sm::eigen::assertNear(goal, rbspline2.template getEvaluatorAt<0>(maxTime).eval(), goalDist * 0.7,
                          SM_SOURCE_FILE_POS, "");
    sm::eigen::assertNear(goal, rbspline2.template getEvaluatorAt<0>(newMaxTime).eval(), tolerance, SM_SOURCE_FILE_POS,
                          "");
