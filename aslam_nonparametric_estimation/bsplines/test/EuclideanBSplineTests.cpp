...
// Check that the the manifold bspline with the EuclideanSpace as manifold evaluates to the same as the former Euclidean
// space only bspline.
TEST(EuclideanBSplineTestSuite, testEuclideanDiffManifoldBSplineBehavesAsVectorSpaceBSpline) {
    ...
    // coefficients.block(0, 0, 1, bspline.numVvCoefficients()) =
    // Eigen::VectorXd::LinSpaced(bspline.numVvCoefficients(), 0, 1).transpose();
    ...
    for (int j = 0; j < repetitions; j++) {
        ...
        }
        {
            ...
            for (double t = minTime; t <= maxTime; t += duration / steps) bspline.eval(t);
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbspline.getEvaluatorAt<0>(t).eval();
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline dynamic static");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineDS.getEvaluatorAt<0>(t).eval();
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline static dynamic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineSD.getEvaluatorAt<0>(t).eval();
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline dynamic dynamic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineDD.getEvaluatorAt<0>(t).eval();
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline generic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbspline.getEvaluatorAt<0>(t).evalGeneric();
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline dynamic generic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineDS.getEvaluatorAt<0>(t).evalGeneric();
            timer.stop();
        }
        {
            sm::timing::Timer timer("eval rbspline dynamic dynamic generic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineDD.getEvaluatorAt<0>(t).evalGeneric();
            timer.stop();
        }
        {
        ...
        }
        {
            ...
            for (double t = minTime; t <= maxTime; t += duration / steps) bspline.evalIntegral(0, t);
            timer.stop();
        }
        {
            sm::timing::Timer timer("evalI rbspline");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbspline.evalIntegral(0, t);
            timer.stop();
        }
        {
            sm::timing::Timer timer("evalI rbspline dynamic static");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineDS.evalIntegral(0, t);
            timer.stop();
        }
        {
            sm::timing::Timer timer("evalI rbspline static dynamic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineSD.evalIntegral(0, t);
            timer.stop();
        }
        {
            sm::timing::Timer timer("evalI rbspline dynamic dynamic");
            for (double t = minTime; t <= maxTime; t += duration / steps) rbsplineDD.evalIntegral(0, t);
            timer.stop();
        }
    }
#endif
...
TEST(EuclideanBSplineTestSuite, testEuclideanDiffManifoldBSplineInitialization) {
    ...
    // interpolationPoints.block(0, 0, 1, numberOfInterpolationPoints) =
    // Eigen::VectorXd::LinSpaced(numberOfInterpolationPoints, 0, 1).transpose();
    ...
    for (int j = 0; j < repetitions; j++) {
        ...
        }
        {
            ...
            BSplineFitter<TestSpline>::initUniformSpline(rbspline, timesV, interpolationPointsV, numberOfSegments,
                                                         lambda);
            timer.stop();
        }
        {
            ...
            BSplineFitter<TestSplineDS>::initUniformSpline(rbsplineDS, timesV, interpolationPointsV, numberOfSegments,
                                                           lambda);
            timer.stop();
        }
        {
            ...
            BSplineFitter<TestSplineSD>::initUniformSpline(rbsplineSD, timesV, interpolationPointsVD, numberOfSegments,
                                                           lambda);
            timer.stop();
        }
        {
            ...
            BSplineFitter<TestSplineDD>::initUniformSpline(rbsplineDD, timesV, interpolationPointsVD, numberOfSegments,
                                                           lambda);
            timer.stop();
        }
    }
...
TEST(EuclideanBSplineTestSuite, testDiffManifoldBSplineFittingDenseSparseAgree) {
    ...
    BSplineFitter<TestSpline>::initUniformSpline(rbsplineDense, timesV, interpolationPointsV, numberOfSegments, lambda,
                                                 FittingBackend::DENSE);
    BSplineFitter<TestSpline>::initUniformSpline(rbsplineSparse, timesV, interpolationPointsV, numberOfSegments, lambda,
                                                 FittingBackend::SPARSE);

    SM_ASSERT_EQ(std::runtime_error, rbsplineDense.getAbsoluteNumberOfSegments(),
                 numberOfSegments + splineOrder * 2 - 1, "");
...
struct BSplineJacobianFunctor {
    ...
    BSplineJacobianFunctor(TestSplineDD& bs, double t, int dim) : bs_(bs), t_(t), _dim(dim), _evaluator(bs, t) {}
...
template <>
void testBSplineJacobianD<-1>(TestSplineDD& /* bs */, Eigen::MatrixXd& /* J */, Eigen::VectorXd& /* coefficients */,
                              int /* dim */){};
...
template <>
void testBSplineJacobiansSO<1>() {}

// Check that the Jacobian calculation is correct.
TEST(EuclideanBSplineTestSuite, testBSplineJacobian) { testBSplineJacobiansSO<10>(); }

TEST(EuclideanBSplineTestSuite, testDExp) { DExpTester<EuclideanSpaceConf<rows>::Manifold>::testFunc(10, 10); }
