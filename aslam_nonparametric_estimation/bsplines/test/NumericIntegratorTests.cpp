...
double fabs(Eigen::Vector3d a) { return a.norm(); }

template <typename TValue>
struct ZeroGetter {
    static TValue getZero() { return static_cast<TValue>(0); }
};

template <>
struct ZeroGetter<Eigen::Vector3d> {
    inline static Eigen::Vector3d getZero() { return Eigen::Vector3d::Zero(); }
};
...
void checkIntegral(TFunctor& f) {
    for (int i = 0; i < numberOfBounds; i++) {
        for (int j = 0; j < numberOfBounds; j++) {
            ...
            TValue val =
                integrateFunctor<TAlgorithm>(lBound, uBound, f, INumberOfPoints, ZeroGetter<TValue>::getZero());
...
struct XSquared {
    ...
    inline double operator()(double x) const { return x * x; }

    inline double calcIntegral(double a, double b) { return (b * b * b - a * a * a) / 3.; }
    inline const char* getName() { return "XSquared"; }
} xSquared;

struct XCubix {
    typedef double ValueT;
    inline double operator()(double x) const { return x * x * x; }

    inline double calcIntegral(double a, double b) { return (b * b * b * b - a * a * a * a) / 4.; }
...
struct SinX {
    ...
    inline double calcIntegral(double a, double b) { return cos(a) - cos(b); }
    inline const char* getName() { return "Sin"; }
} sinX;

struct ExpX {
    typedef double ValueT;
    inline double operator()(double x) const { return exp(x); }

    inline double calcIntegral(double a, double b) { return exp(b) - exp(a); }
...
struct SinhX {
    ...
    inline double calcIntegral(double a, double b) { return cosh(b) - cosh(a); }
...
TEST(NumericIntegratorTestSuite, eigenVectorValueIntegration) { checkIntegral(xSquaredInR3); }

TEST(NumericIntegratorTestSuite, testFunctionInterface) {
    struct Integrand {
        typedef double ValueT;
        inline static ValueT integrand(const double& x) { return x * x; }
    };
