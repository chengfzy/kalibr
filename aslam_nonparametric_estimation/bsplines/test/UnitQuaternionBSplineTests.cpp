...
TEST(UnitQuaternionBSplineTestSuite, testQuaternionBSplineCompilation) {
    ...
    const UnitQuaternionBSpline<splineOrder>::TYPE::point_t p =
        UnitQuaternionBSpline<splineOrder>::TYPE::point_t(1, 0, 0, 0);
...
TEST(UnitQuaternionBSplineTestSuite, evalRiD1) { SplineEvalRiDTester<UQTestSpline, 1>::testFunc(10, 10); }
TEST(UnitQuaternionBSplineTestSuite, evalRiD2) { SplineEvalRiDTester<UQTestSpline, 2>::testFunc(10, 10); }
TEST(UnitQuaternionBSplineTestSuite, evalRiD3) { SplineEvalRiDTester<UQTestSpline, 3>::testFunc(10, 10); }

TEST(UnitQuaternionBSplineTestSuite, evalD1) { SplineEvalDTester<UQTestSpline, 1>::testFunc(10, 10); }

TEST(UnitQuaternionBSplineTestSuite, evalD2) { SplineEvalDTester<UQTestSpline, 2>::testFunc(10, 10); }

TEST(UnitQuaternionBSplineTestSuite, evalD3) { SplineEvalDTester<UQTestSpline, 3>::testFunc(10, 10); }
...
TEST(UnitQuaternionBSplineTestSuite, evalAngularVelocityAndAcceleration) {
    ...
    for (int i = 0; i < numTimes; i++) {
        ...
        if (splineOrder == 2) {
            ...
            sm::eigen::assertNear(spline.getEvaluatorAt<0>(t).eval(), spline.getManifold().expAtId(direction * t), 1E-9,
                                  SM_SOURCE_FILE_POS);
        }
...
TEST(UnitQuaternionBSplineTestSuite, testBSplineJacobianD0) { BSplineJacobianTester<UQTestSpline, 0>::testFunc(10, 1); }
TEST(UnitQuaternionBSplineTestSuite, testBSplineJacobianD1) { BSplineJacobianTester<UQTestSpline, 1>::testFunc(10, 1); }
TEST(UnitQuaternionBSplineTestSuite, testBSplineJacobianD2) { BSplineJacobianTester<UQTestSpline, 2>::testFunc(10, 1); }
...
TEST(UnitQuaternionBSplineTestSuite, testDiffManifoldBSplineFitting) {
    double tolerance = 0.3;  // TODO improve : the unit quaternion fitting is quite bad. this huge tolerance actually
                             // checks almost nothing apart from compilation and running without exceptions.
