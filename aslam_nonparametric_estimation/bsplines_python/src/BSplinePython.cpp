...
class BiFunction {
  ...
  public:
    BiFunction(const T& biVector) : biVector_(biVector) {}
    double getBi(int i) {
...
boost::python::object getCumulativeBiFunction(const bsplines::BSpline* bs, double t) {
    ...
    return boost::python::make_function(
        boost::bind(&BiFunction<T>::getBi, BiFunction<T>(bs->getCumulativeBiVector(t)), _1),
        boost::python::default_call_policies(), boost::mpl::vector2<double, int>());
}
...
void import_bspline_python() {
    ...
    class_<BSpline>("BSpline", init<int>())
        ...
        .def("minimumKnotsRequired", &BSpline::minimumKnotsRequired,
             "The minimum number of knots required based on the spline order")
        .def("numCoefficientsRequired", &BSpline::numCoefficientsRequired,
             "The number of coefficients required for a specified number of valid time segments")
        .def("numKnotsRequired", &BSpline::numKnotsRequired,
             "The number of knots required for a target number of valid time segments")
        .def("numValidTimeSegments", numValidTimeSegments1,
             "The number of valid time segments for a given number of knots")
        .def("numValidTimeSegments", numValidTimeSegments2,
             "The number of valid time segments for the current knot sequence")
        .def("setKnotVectorAndCoefficients", &BSpline::setKnotVectorAndCoefficients,
             "Sets the knot vector and spline coefficients")
        ...
        .def("localCoefficientMatrix", &BSpline::localCoefficientMatrix,
             "Get the matrix of locally-active coefficients for a specified time in matrix form")
        .def("localCoefficientVector", &BSpline::localCoefficientVector,
             "Get the stacked vector of locally-active coefficients for a specified time.")
        .def("segmentCoefficientVector", &BSpline::localCoefficientVector,
             "Get the stacked vector of locally-active coefficients for a specified segment.")
        ...
        .def("initSplineSparse", &BSpline::initSplineSparse,
             "Initialize the spline to interpolate a set of points (Sparse Solution)")
        .def("initSplineSparseKnots", &BSpline::initSplineSparseKnots,
             "Initialize the spline to interpolate a set of points with a given knot sequence. (Sparse Solution)")
        .def("basisMatrix", &BSpline::basisMatrix, "Get the basis matrix active on the ith time segment.",
             return_value_policy<copy_const_reference>())
        .def("timeInterval", &timeInterval1,
             "Returns a tuple with the time interval that the spline is well-defined on.")
        .def("timeInterval", &timeInterval2, "Returns a tuple with the time interval of the ith segment.")
        .def("addCurveSegment", &BSpline::addCurveSegment,
             "Adds a curve segment on the right that interpolates the given point at the given time.")
        .def("removeCurveSegment", &BSpline::removeCurveSegment, "removes a curve segment on the left")
        .def("setLocalCoefficientVector", &BSpline::setLocalCoefficientVector,
             "Sets the local coefficient vector for a specified time")
        .def("localVvCoefficientVectorIndices", &BSpline::localVvCoefficientVectorIndices, "")
        .def("localCoefficientVectorIndices", &BSpline::localCoefficientVectorIndices,
             "For the elements of a local coefficient vector, this gets the indices into the full coefficient vector")
        .def("segmentVvCoefficientVectorIndices", &BSpline::segmentVvCoefficientVectorIndices, "")
        .def("segmentCoefficientVectorIndices", &BSpline::segmentCoefficientVectorIndices,
             "For the elements of a segment coefficient vector, this gets the indices into the full coefficient vector")
        .def("setCoefficientVector", &BSpline::setCoefficientVector,
             "Sets the full stacked coefficient vector of the spline")
