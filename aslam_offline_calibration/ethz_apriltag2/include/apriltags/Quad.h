...
class Quad {
  public:
    static const int minimumEdgeLength =
        6;                                  //!< Minimum size of a tag (in pixels) as measured along edges and diagonals
    ...
    //! Interpolate given that the lower left corner of the lower left cell is at (-1,-1) and the upper right corner of
    ...
    //! Points for the quad (in pixel coordinates), in counter clockwise order. These points are the intersections of
    //! segments.
    ...
    //! Given that the whole quad spans from (0,0) to (1,1) in "quad space", compute the pixel coordinates for a given
    //! point within that quad.
