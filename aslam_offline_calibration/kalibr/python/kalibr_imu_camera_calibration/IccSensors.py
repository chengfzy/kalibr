...
class IccCamera():
    ...
    def findOrientationPriorCameraToImu(self, imu):
        ...
        for im in imu.imuData:
            tk = im.stamp.toSec()
            if poseSpline.t_min() < tk < poseSpline.t_max():
                # DV expressions
    ...
    def initPoseSplineFromCamera(self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02):
        ...
        curve = np.matrix(
            [pose.transformationToCurveValue(np.dot(obs.T_t_c().T(), T_c_b)) for obs in self.targetObservations]).T
...
class IccCameraChain():
    ...
    def addDesignVariables(self, problem, noTimeCalibration=True, noChainExtrinsics=True):
        # add the design variables (T(R,t) & time)  for all individual cameras
...
class IccImu(object):
    ...
    def updateImuConfig(self):
        self.imuConfig.setImuPose(
            sm.Transformation(sm.r2quat(self.q_i_b_Dv.toRotationMatrix()), self.r_b_Dv.toEuclidean()).T())
    ...
    def addDesignVariables(self, problem):
        ...
        ic.addSplineDesignVariables(problem, self.gyroBiasDv, setActive=True, group_id=ic.HELPER_GROUP_ID)
    ...
    def addAccelerometerErrorTerms(self, problem, poseSplineDv, g_w, mSigma=0.0, accelNoiseScale=1.0):
        print
        ...
        for im in self.imuData:
            ...
            if poseSplineDv.spline().t_min() < tk < poseSplineDv.spline().t_max():
    ...
    def getTransformationFromBodyToImu(self):
        ...
        return sm.Transformation(sm.r2quat(self.q_i_b_Dv.toRotationMatrix()),
                                 np.dot(self.q_i_b_Dv.toRotationMatrix(), self.r_b_Dv.toEuclidean()))
    ...
    def findOrientationPrior(self, referenceImu):
        ...
        for im in referenceImu.imuData:
            tk = im.stamp.toSec()
            if angularVelocity.t_min() < tk < angularVelocity.t_max():
                # DV expressions
        ...
        referenceAbsoluteOmega = lambda dt=np.array([0.]): np.asarray(
            [np.linalg.norm(angularVelocityDv.toEuclidean(im.stamp.toSec() + dt[0], 0)) for im in self.imuData if (
                    im.stamp.toSec() + dt[0] > angularVelocity.t_min() and im.stamp.toSec() + dt[
                0] < angularVelocity.t_max())])
        absoluteOmega = lambda dt=np.array([0.]): np.asarray([np.linalg.norm(im.omega) for im in self.imuData if (
                im.stamp.toSec() + dt[0] > angularVelocity.t_min() and im.stamp.toSec() + dt[
            0] < angularVelocity.t_max())])

        if len(referenceAbsoluteOmega()) == 0 or len(absoluteOmega()) == 0:
            sm.logFatal(
                "The time ranges of the IMUs published as topics {0} and {1} do not overlap. "
                "Please make sure that the sensors are synchronized correctly.".format(
                    referenceImu.imuConfig.getRosTopic(), self.imuConfig.getRosTopic()))
            sys.exit(-1)
        ...
        print self.timeOffset, "[s]", (
            "" if self.estimateTimedelay else " (this offset is not accounted for in the calibration)")
...
class IccScaledMisalignedImu(IccImu):
    ...
    def updateImuConfig(self):
        ...
        self.imuConfig.setIntrisicsMatrices(self.M_accel_Dv.toMatrix3x3(), self.q_gyro_i_Dv.toRotationMatrix(),
                                            self.M_gyro_Dv.toMatrix3x3(), self.M_accel_gyro_Dv.toMatrix3x3())
    ...
    def addDesignVariables(self, problem):
        ...
        self.M_accel_Dv = aopt.MatrixBasicDv(np.eye(3), np.array([[1, 0, 0], [1, 1, 0], [1, 1, 1]], dtype=int))
        problem.addDesignVariable(self.M_accel_Dv, ic.HELPER_GROUP_ID)
        self.M_accel_Dv.setActive(True)

        self.M_gyro_Dv = aopt.MatrixBasicDv(np.eye(3), np.array([[1, 0, 0], [1, 1, 0], [1, 1, 1]], dtype=int))
    ...
    def addAccelerometerErrorTerms(self, problem, poseSplineDv, g_w, mSigma=0.0, accelNoiseScale=1.0):
        ...
        for im in self.imuData:
            ...
            if tk > poseSplineDv.spline().t_min() and tk < poseSplineDv.spline().t_max():
                ...
                C_i_b = self.q_i_b_Dv.toExpression()
                r_b = self.r_b_Dv.toExpression()
                a = M * (C_i_b * (C_b_w * (a_w - g_w) + w_dot_b.cross(r_b) + w_b.cross(w_b.cross(r_b))))
...
class IccScaledMisalignedSizeEffectImu(IccScaledMisalignedImu):
    class ImuParameters(IccScaledMisalignedImu.ImuParameters):
        ...
        def printDetails(self, dest=sys.stdout):
            ...
            print >> dest, "    rx_i [m]:"
            print >> dest, self.formatIndented("      ", np.array(self.data["accelerometers"]["rx_i"]))
            print >> dest, "    ry_i [m]:"
            print >> dest, self.formatIndented("      ", np.array(self.data["accelerometers"]["ry_i"]))
            print >> dest, "    rz_i [m]:"
            print >> dest, self.formatIndented("      ", np.array(self.data["accelerometers"]["rz_i"]))
    ...
    def updateImuConfig(self):
        ...
        self.imuConfig.setAccelerometerLeverArms(self.rx_i_Dv.toEuclidean(), self.ry_i_Dv.toEuclidean(),
    ...
    def addAccelerometerErrorTerms(self, problem, poseSplineDv, g_w, mSigma=0.0, accelNoiseScale=1.0):
        ...
        for im in self.imuData:
            ...
            if tk > poseSplineDv.spline().t_min() and tk < poseSplineDv.spline().t_max():
                ...
                a = M * (C_i_b * (C_b_w * (a_w - g_w)) + Ix * (
                        C_i_b * (w_dot_b.cross(rx_b) + w_b.cross(w_b.cross(rx_b)))) + Iy * (
                                 C_i_b * (w_dot_b.cross(ry_b) + w_b.cross(w_b.cross(ry_b)))) + Iz * (
                                 C_i_b * (w_dot_b.cross(rz_b) + w_b.cross(w_b.cross(rz_b)))))
