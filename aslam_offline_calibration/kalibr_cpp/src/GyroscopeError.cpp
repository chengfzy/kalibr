...
GyroscopeError::GyroscopeError(const Eigen::Vector3d& measurement, const Eigen::Matrix3d& invR,
...
GyroscopeNoBiasError::GyroscopeNoBiasError(const Eigen::Vector3d& measurement, const Eigen::Matrix3d& invR,
                                           const aslam::backend::EuclideanExpression& angularVelocity)
    : _measurement(measurement), _angularVelocity(angularVelocity) {
    setInvR(invR);
...
void GyroscopeNoBiasError::evaluateJacobiansImplementation(aslam::backend::JacobianContainer& _jacobians) const {
...
GyroscopeErrorEccentric::GyroscopeErrorEccentric(
    ...
    const aslam::backend::MatrixExpression& Ma, const aslam::backend::RotationExpression& C_b_w,
    ...
    const aslam::backend::EuclideanExpression& r_b, const aslam::backend::EuclideanExpression& bias,
    const aslam::backend::EuclideanExpression& g_w) {
    _measurement = measurement;
    setInvR(invR);
    _predictedMeasurement = M * (C_i_b * angularVelocity_b +
                                 Ma * (C_i_b * (C_b_w * (acceleration_w - g_w) + angularAcceleration_b.cross(r_b) +
                                                angularVelocity_b.cross(angularVelocity_b.cross(r_b))))) +
                            bias;
