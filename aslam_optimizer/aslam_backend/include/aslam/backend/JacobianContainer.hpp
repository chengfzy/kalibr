...
class JacobianContainer {
  public:
     ...
     */
    void evaluateHessian(const Eigen::VectorXd& e, const Eigen::MatrixXd& invR, SparseBlockMatrix& outHessian,
                         Eigen::VectorXd& outRhs) const;
  ...
  private:
    void buildCorrelatedHessianBlock(const Eigen::VectorXd& e, const Eigen::MatrixXd& J1, int j1_block,
                                     SparseBlockMatrix& outHessian, map_t::const_iterator it,
                                     map_t::const_iterator it_end) const;

    void buildHessianBlock(const Eigen::VectorXd& e, SparseBlockMatrix& outHessian, Eigen::VectorXd& outRhs,
...
void JacobianContainer::add(DesignVariable* dv, const Eigen::MatrixBase<DERIVED>& Jacobian) {
    ...
    SM_ASSERT_EQ(Exception, Jacobian.cols(), dv->minimalDimensions(),
                 "The Jacobian must have the same number of cols as dv->minimalDimensions()");
    // If the designe variable isn't active. Don't bother adding it.
    if (!dv->isActive()) return;
    SM_ASSERT_GE_DBG(Exception, dv->blockIndex(), 0,
                     "The design variable is active but the block index is less than zero.");
    ...
    if (it == _jacobianMap.end()) {
        ...
        SM_ASSERT_TRUE_DBG(Exception, it->first == dv,
