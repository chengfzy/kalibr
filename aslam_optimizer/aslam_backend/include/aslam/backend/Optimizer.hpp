...
class Optimizer {
  public:
    ...
    /// \brief Set up to work on the optimization problem.
    void setProblem(OptimizationProblemBase* problem, bool optimizerOwnsProblem) {
        setProblem(optimizerOwnsProblem ? boost::shared_ptr<OptimizationProblemBase>(problem)
                                        : boost::shared_ptr<OptimizationProblemBase>(problem, sm::null_deleter()));
    }
    ...
    inline const Eigen::VectorXd& epsilon() const { return rhs(); }
    ...
    /// \brief get the covariance matrix as a sparse matrix. This matrix will only be filled in by what was computed
    /// above.
    ...
    /// \brief Do a bunch of checks to see if the problem is well-defined. This includes checking that every error term
    /// is
