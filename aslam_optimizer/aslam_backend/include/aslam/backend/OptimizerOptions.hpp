...
struct OptimizerOptions {
    ...
    /// \brief Should we reset the linear solver every iteration? This is currently necessary for any problem where
    /// error terms have the possibility of changing design variables.
    bool resetSolverEveryIteration;

    /// \brief The number of times the linear solver may fail before the optimisation is aborted. (>0 only if a fallback
    /// is available!)
    int linearSolverMaximumFails;

    /// \brief which linear solver should we use. Options are currently "block_cholesky", "sparse_cholesky",
    /// "sparse_qr".
