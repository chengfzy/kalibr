...
template <typename I>
void CompressedColumnJacobianTransposeBuilder<I>::initMatrixStructure(const std::vector<DesignVariable*>& dvs,
    ...
    for (; it != errors.end(); ++it) {
        ...
        // std::cout << "Error " << i << "/" << errors.size() << ", Jacobian has " <<
        // ((double)_J_transpose.values().size() * (double)64 * (1e-9))  << " GB of data\n";
        eRow += (*it)->dimension();
    }
    //_e.resize(eRow);
    _isInitialized = true;
...
void CompressedColumnJacobianTransposeBuilder<I>::setupThreadedJob(MEMBER_FUNCTION_PTR ptr, size_t nThreads,
                                                                   bool useMEstimator) {
    if (nThreads <= 1) {
...
template <typename I>
void CompressedColumnJacobianTransposeBuilder<I>::evaluateJacobians(int /* threadId */, int startIdx, int endIdx,
                                                                    bool useMEstimator) {
    Eigen::VectorXd ee;
