...
template <typename I>
JacobianColumnPointer CompressedColumnMatrix<I>::appendJacobiansSymbolic(int Jrows,
                                                                         const std::vector<DesignVariable*>& dvs) {
    ...
    for (size_t i = 0; i < dvs.size(); ++i) {
        ...
        if (dv->isActive()) {
            SM_ASSERT_GE_DBG(Exception, dv->blockIndex(), 0,
                             "This design variable (" << i << ") is active but does not have a block index!");
    ...
    SM_ASSERT_FALSE_DBG(Exception, activeDvs.empty(),
                        "There are no active design variables associated with this error term.");
    ...
    for (std::vector<DesignVariable*>::const_iterator it = activeDvs.begin(); it != activeDvs.end(); ++it) {
        ...
        for (int c = 0; c < Jrows; ++c) {
            ...
            for (int r = 0; r < dv.minimalDimensions(); ++r) {
                SM_ASSERT_LT_DBG(Exception, (int)(dv.columnBase() + r), (int)_rows,
                                 "This element is outside of the matrix bounds");
    ...
    for (size_t j = 0; j < _cols; ++j)
        for (I p = _col_ptr[j]; p < _col_ptr[j + 1]; ++p)
            if (std::fabs(_values[p]) > std::numeric_limits<double>::epsilon())
                stream << std::fixed << std::setprecision(18) << _row_ind[p] + 1 << " " << j + 1 << " " << _values[p]
                       << std::endl;
}

template <typename I>
void CompressedColumnMatrix<I>::fromCholmodSparse(const cholmod_sparse* cs) {
    if (cs == NULL) return;
    _rows = cs->nrow;
    _cols = cs->ncol;
