...
template <typename T>
void DesignVariableAdapter<T>::getParametersImplementation(Eigen::MatrixXd& value) const {
    _dv->getParameters(value);
}

template <typename T>
void DesignVariableAdapter<T>::setParametersImplementation(const Eigen::MatrixXd& value) {
    _dv->getParameters(_backup);
    _dv->setParameters(value);
}

template <typename T>
void DesignVariableAdapter<T>::minimalDifferenceImplementation(const Eigen::MatrixXd& /* xHat */,
                                                               Eigen::VectorXd& /* outDifference */) const {
    // TODO: implement
}

template <typename T>
void DesignVariableAdapter<T>::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& /* xHat */,
