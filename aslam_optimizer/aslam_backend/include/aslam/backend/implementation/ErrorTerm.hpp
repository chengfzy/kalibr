...
ErrorTermFs<C>::ErrorTermFs()
    : _evalJacobianTimer("ErrorTerm: Evaluate Jacobian", true), _buildHessianTimer("ErrorTerm: Build Hessian", true) {
...
struct ErrorTermFsFunctor {
    ...
    ErrorTermFs<C>& _et;
    ErrorTermFsFunctor(ErrorTermFs<C>& et) : _et(et) {}
...
template <int C>
void ErrorTermFs<C>::buildHessianImplementation(SparseBlockMatrix& outHessian, Eigen::VectorXd& outRhs,
                                                bool useMEstimator) {
...
void ErrorTermFs<C>::checkJacobiansNumerical(double tolerance) {
    ...
    sm::eigen::NumericalDiff<detail::ErrorTermFsFunctor<C> > numdiff(functor, tolerance);
    int inputSize = 0;
    ...
    for (size_t i = 0; i < numDesignVariables(); i++) {
        ...
        const Eigen::MatrixXd JNum = JNumComp.block(0, offset, C, d->minimalDimensions());
