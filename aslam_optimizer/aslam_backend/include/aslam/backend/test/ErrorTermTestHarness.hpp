...
class ErrorTermTestHarness {
  public:
    ...
    void testAll(double tolerance = 1e-6) {
        ...
        sm::eigen::assertNear(J.asSparseMatrix(), estJ.asSparseMatrix(), tolerance, SM_SOURCE_FILE_POS,
                              "Testing jacobians vs. finite differences (Matrix A are the analytical Jacobians, Matrix "
                              "B is from finite differences)");
    }
