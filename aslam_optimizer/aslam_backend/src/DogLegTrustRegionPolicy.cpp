...
bool DogLegTrustRegionPolicy::solveSystemImplementation(double J, bool previousIterationFailed, int nThreads,
    ...
    // otherwise check the GN step
        ...
        if (!gnComputed) {
            ...
            if (!solutionSuccess) return solutionSuccess;
        ...
        } else  // otherwise interpolate on the line between the cauchy point and gn step
        {
            ...
            _L0 =
                1 / 2 * _sd_scale * (1 - _beta) * (1 - _beta) * _solver->rhs().squaredNorm() + _beta * (2 - _beta) * J;
            _stepType = "DL";
        }
    }
...
std::ostream& DogLegTrustRegionPolicy::printState(std::ostream& out) const {
    ...
    if (_stepType == "DL") out << ", beta:" << _beta;
    return out;
}

bool DogLegTrustRegionPolicy::revertOnFailure() { return true; }

bool DogLegTrustRegionPolicy::requiresAugmentedDiagonal() const { return false; }
