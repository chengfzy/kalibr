...
namespace aslam {
namespace backend {
ErrorTerm::ErrorTerm() : _squaredError(0.0), _timestamp(0) { _mEstimatorPolicy = boost::make_shared<NoMEstimator>(); }
...
void ErrorTerm::setMEstimatorPolicy(const boost::shared_ptr<MEstimator>& mEstimator) { _mEstimatorPolicy = mEstimator; }

/// \brief clear the m-estimator policy.
void ErrorTerm::clearMEstimatorPolicy() { _mEstimatorPolicy = boost::make_shared<NoMEstimator>(); }

/// \brief compute the M-estimator weight from a squared error.
double ErrorTerm::getMEstimatorWeight(double squaredError) const { return _mEstimatorPolicy->getWeight(squaredError); }

/// \brief compute the M-estimator weight from a squared error.
double ErrorTerm::getCurrentMEstimatorWeight() const { return _mEstimatorPolicy->getWeight(_squaredError); }

/// \brief How many design varibles is this error term connected to?
size_t ErrorTerm::numDesignVariables() const { return _designVariables.size(); }
...
void ErrorTerm::setDesignVariables(const std::vector<DesignVariable*>& designVariables) {
    ...
    SM_ASSERT_EQ(aslam::UnsupportedOperationException, _designVariables.size(), 0,
                 "The design variable container already has objects. The design variables may only be set once");
    /// \todo: set the back-link in the design variable.
    for (unsigned i = 0; i < designVariables.size(); ++i) {
        SM_ASSERT_TRUE(aslam::InvalidArgumentException, designVariables[i] != NULL,
                       "Design variable " << i << " is null");
    }
...
Eigen::VectorXd ErrorTerm::vsError() const { return vsErrorImplementation(); }

std::string ErrorTerm::getMEstimatorName() { return _mEstimatorPolicy->name(); }
...
double ErrorTerm::getRawSquaredError() const { return _squaredError; }

void ErrorTerm::getDesignVariables(DesignVariable::set_t& dvs) {
    dvs.insert(_designVariables.begin(), _designVariables.end());
}

size_t ErrorTerm::dimension() const { return getDimensionImplementation(); }

const std::vector<DesignVariable*>& ErrorTerm::designVariables() const { return _designVariables; }

/// \brief Get the column base of this error term in the Jacobian matrix.
size_t ErrorTerm::rowBase() const { return _rowBase; }

/// \brief Set the column base of this error term in the Jacobian matrix.
void ErrorTerm::setRowBase(size_t b) { _rowBase = b; }

void ErrorTerm::setTime(const sm::timing::NsecTime& t) { _timestamp = t; }
...
struct ErrorTermFunctor {
    ...
    ErrorTerm& _et;
    ErrorTermFunctor(ErrorTerm& et) : _et(et) {}
    ...
    Eigen::VectorXd operator()(const Eigen::VectorXd& dr) {
        ...
        for (size_t i = 0; i < _et.numDesignVariables(); i++) {
            ...
            SM_ASSERT_LE_DBG(aslam::Exception, offset + d->minimalDimensions(), dr.size(),
                             "The offset is out of bounds.");
            d->update(&dr[offset], d->minimalDimensions());
            offset += d->minimalDimensions();
        }
        SM_ASSERT_EQ_DBG(aslam::Exception, offset, dr.size(),
                         "The input vector is too large. It wasn't covered by the design variables.");
        _et.evaluateError();
