...
const typename ErrorTermDs::error_t& ErrorTermDs::error() const { return _error; }

Eigen::VectorXd ErrorTermDs::vsErrorImplementation() const { return _error; }

typename ErrorTermDs::inverse_covariance_t ErrorTermDs::invR() const { return _sqrtInvR * _sqrtInvR.transpose(); }
...
struct ErrorTermDsFunctor {
    ...
    ErrorTermDs& _et;
    ErrorTermDsFunctor(ErrorTermDs& et) : _et(et) {}
...
void ErrorTermDs::buildHessianImplementation(SparseBlockMatrix& outHessian, Eigen::VectorXd& outRhs,
                                             bool useMEstimator) {
...
void ErrorTermDs::clearJacobians() { _jacobians.clear(); }

// void ErrorTermDs::setError(const Eigen::MatrixBase<double>& e)
void ErrorTermDs::setError(const Eigen::VectorXd& e) { _error = e; }
...
void ErrorTermDs::setInvR(const Eigen::MatrixXd& invR) {
    ...
    SM_ASSERT_EQ(Exception, invR.rows(), (int)dimension(),
                 "The covariance matrix does not match the size of the error");
...
void ErrorTermDs::getInvR(Eigen::MatrixXd& invR) const { invR = _sqrtInvR * _sqrtInvR.transpose(); }

Eigen::MatrixXd ErrorTermDs::vsInvR() const { return _sqrtInvR * _sqrtInvR.transpose(); }

void ErrorTermDs::vsSetInvR(const Eigen::MatrixXd& invR) { setInvR(invR); }

void ErrorTermDs::setSqrtInvR(const Eigen::MatrixXd& sqrtInvR) { _sqrtInvR = sqrtInvR; }

const typename ErrorTermDs::inverse_covariance_t& ErrorTermDs::sqrtInvR() const { return _sqrtInvR; }
...
void ErrorTermDs::checkJacobiansNumerical(double tolerance) {
    ...
    sm::eigen::NumericalDiff<detail::ErrorTermDsFunctor> numdiff(functor, tolerance);
    int inputSize = 0;
    for (size_t i = 0; i < numDesignVariables(); i++) {
        inputSize += designVariable(i)->minimalDimensions();
    }
    const Eigen::MatrixXd JNumComp = numdiff.estimateJacobian(Eigen::VectorXd::Zero(inputSize));
    int offset = 0;
    for (size_t i = 0; i < numDesignVariables(); i++) {
        DesignVariable* d = designVariable(i);
        const Eigen::MatrixXd JAna = _jacobians.Jacobian(d);
        const Eigen::MatrixXd JNum = JNumComp.block(0, offset, _dimensionErrorTerm, d->minimalDimensions());
...
void ErrorTermDs::resizeJacobianContainer(int nrows) { _jacobians = JacobianContainer(nrows); }
