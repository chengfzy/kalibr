...
void JacobianContainer::add(const JacobianContainer& rhs) {
    SM_ASSERT_EQ(Exception, _rows, rhs._rows,
                 "The JacobianContainers cannot be added. They don't have the same number of rows.");
    // Merge the two maps.
    ...
    for (; lt != lt_end && !done; ++lt) {
        ...
        if (!done && rt->first->blockIndex() == lt->first->blockIndex()) {
            SM_ASSERT_TRUE_DBG(Exception, rt->first == lt->first,
                               "Two design variables had the same block index but different pointer values");
            // add the Jacobians.
            lt->second += rt->second;
        }
    }
...
size_t JacobianContainer::numDesignVariables() const { return _jacobianMap.size(); }

/// \brief How many rows does this set of Jacobians have?
int JacobianContainer::rows() const { return _rows; }

JacobianContainer::map_t::const_iterator JacobianContainer::begin() const { return _jacobianMap.begin(); }

JacobianContainer::map_t::const_iterator JacobianContainer::end() const { return _jacobianMap.end(); }

JacobianContainer::map_t::iterator JacobianContainer::begin() { return _jacobianMap.begin(); }

JacobianContainer::map_t::iterator JacobianContainer::end() { return _jacobianMap.end(); }
...
void JacobianContainer::applyChainRule(const Eigen::MatrixXd& df_dx) {
    ...
    map_t::iterator it = _jacobianMap.begin(), it_end = _jacobianMap.end();
    for (; it != it_end; ++it) it->second = df_dx * it->second;
    _rows = df_dx.rows();
}

void JacobianContainer::buildCorrelatedHessianBlock(const Eigen::VectorXd& e, const Eigen::MatrixXd& J1, int j1_block,
                                                    SparseBlockMatrix& outHessian, map_t::const_iterator it,
                                                    map_t::const_iterator it_end) const {
    // Recursion base case.
    if (it == it_end) return;
    int j2_block = it->first->blockIndex();
    SM_ASSERT_LE_DBG(Exception, j1_block, j2_block,
                     "The recursion should proceed in ordered blocks in order to only populate the upper diagonal of "
                     "the Hessian. This violates the ordering.");
    ...
    SM_ASSERT_EQ_DBG(Exception, J1t_invR_J2->rows(), J1.cols(),
                     "The Hessian block has an unexpected number of rows. Block J1^T invR J2: ("
                         << j1_block << ", " << j2_block << "). J1 is: " << J1.cols() << "x" << J1.rows()
                         << ", J2 is: " << it->second.rows() << "x" << it->second.cols());
...
void JacobianContainer::buildHessianBlock(const Eigen::VectorXd& e, SparseBlockMatrix& outHessian,
                                          Eigen::VectorXd& outRhs, map_t::const_iterator it,
                                          map_t::const_iterator it_end) const {
    // Recursion base case.
    if (it == it_end) return;
...
void JacobianContainer::evaluateHessian(const Eigen::VectorXd& e, const Eigen::MatrixXd& sqrtInvR,
                                        SparseBlockMatrix& outHessian, Eigen::VectorXd& outRhs) const {
    SM_ASSERT_EQ_DBG(Exception, e.size(), _rows, "The error and this Jacobian container should have the same size");
    SM_ASSERT_EQ_DBG(Exception, e.size(), sqrtInvR.rows(),
                     "The error and the covariance matrix don't have compatible sizes");
...
void JacobianContainer::clear() { _jacobianMap.clear(); }
...
SparseBlockMatrix JacobianContainer::asSparseMatrix(const std::vector<int>& colBlockIndices) const {
    ...
    for (int col = 0; it != _jacobianMap.end(); ++it, col++) {
        ...
        SM_ASSERT_GE_LT_DBG(aslam::IndexOutOfBoundsException, (size_t)it->first->blockIndex(), 0,
                            colBlockIndices.size(), "Block index is out of bounds");
