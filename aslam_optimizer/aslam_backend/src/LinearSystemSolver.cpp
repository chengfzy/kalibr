...
void LinearSystemSolver::setupThreadedJob(boost::function<void(size_t, size_t, size_t, bool)> job, size_t nThreads,
                                          bool useMEstimator) {
    if (nThreads <= 1) {
        ...
        for (unsigned i = 0; i < nThreads; ++i) indices[i + 1] = indices[i] + nJPerThread;
...
double LinearSystemSolver::evaluateError(size_t nThreads, bool useMEstimator) {
    ...
    double error = 0.0;
    for (unsigned i = 0; i < _threadLocalErrors.size(); ++i) error += _threadLocalErrors[i];
    return error;
}

const Eigen::VectorXd& LinearSystemSolver::e() const { return _e; }

const Eigen::VectorXd& LinearSystemSolver::rhs() const { return _rhs; }

void LinearSystemSolver::setConditioner(const Eigen::VectorXd& diag) {
    SM_ASSERT_EQ(Exception, (size_t)diag.size(), _JCols,
                 "The diagonal conditioner must have the same number of rows as the Hessian matrix");
...
void LinearSystemSolver::initMatrixStructure(const std::vector<DesignVariable*>& dvs,
                                             const std::vector<ErrorTerm*>& errors, bool useDiagonalConditioner) {
...
size_t LinearSystemSolver::JRows() const { return _JRows; }

/// \brief the number of columns in the Jacobian matrix
size_t LinearSystemSolver::JCols() const { return _JCols; }
