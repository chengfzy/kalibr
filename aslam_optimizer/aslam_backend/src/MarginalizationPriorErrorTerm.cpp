...
MarginalizationPriorErrorTerm::MarginalizationPriorErrorTerm(
    const std::vector<aslam::backend::DesignVariable*>& designVariables, const Eigen::VectorXd& d,
    const Eigen::MatrixXd& R)
    : aslam::backend::ErrorTermDs(R.rows()),
      _designVariables(designVariables),
      _d(d),
      _R(R),
      _dimensionDesignVariables(R.cols()) {
    SM_ASSERT_GT(aslam::InvalidArgumentException, designVariables.size(), 0,
                 "The prior error term doesn't make much sense with zero design variables.");
    ...
    for (vector<aslam::backend::DesignVariable*>::iterator it = _designVariables.begin(); it != _designVariables.end();
         ++it) {
...
double MarginalizationPriorErrorTerm::evaluateErrorImplementation() {
    ...
    SM_ASSERT_EQ(aslam::Exception, diff.rows(), _R.cols(),
                 "Dimension of R and the minimal difference vector mismatch!");
...
// Computes the difference vector of all design variables between the linearization point at marginalization and the
// current guess, on the tangent space (i.e. log(x_bar - x))
...
void MarginalizationPriorErrorTerm::evaluateJacobiansImplementation(JacobianContainer& outJ) const {
    ...
    for (vector<aslam::backend::DesignVariable*>::const_iterator it = _designVariables.begin();
         it != _designVariables.end(); ++it, ++it_marg) {
        int dimDesignVariable = (*it)->minimalDimensions();
        Eigen::MatrixXd M;
        Eigen::VectorXd diff;
        (*it)->minimalDifferenceAndJacobian(*it_marg, diff, M);
        SM_ASSERT_EQ(aslam::Exception, M.rows(), dimDesignVariable,
                     "Minimal difference jacobian and design variable dimension mismatch!");
        outJ.add(*it, _R.block(0, colIndex, _dimensionErrorTerm, dimDesignVariable) * M);
        colIndex += dimDesignVariable;
    }
}

aslam::backend::DesignVariable* MarginalizationPriorErrorTerm::getDesignVariable(int i) { return _designVariables[i]; }
