...
void marginalize(std::vector<aslam::backend::DesignVariable*>& inDesignVariables,
                 std::vector<aslam::backend::ErrorTerm*>& inErrorTerms, int numberOfInputDesignVariablesToRemove,
                 bool useMEstimator,
                 boost::shared_ptr<aslam::backend::MarginalizationPriorErrorTerm>& outPriorErrorTermPtr,
                 Eigen::MatrixXd& outCov, std::vector<aslam::backend::DesignVariable*>& outDesignVariablesInRTop,
                 size_t numTopRowsInCov, size_t /* numThreads */) {
    ...
    for (std::vector<aslam::backend::DesignVariable*>::const_iterator it = inDesignVariables.begin();
         it != inDesignVariables.end(); ++it) {
    ...
    SM_INFO_STREAM("Marginalization optimization problem initialized with "
                   << inDesignVariables.size() << " design variables and " << inErrorTerms.size() << " error terrms");
    ...
    // SM_ASSERT_FALSE(aslam::Exception, rankDeficient, "Right now, we don't want the jacobian to be rank deficient -
    // ever...");
    ...
    if (jrows < jcols) {
        ...
        //			  SM_ASSERT_TRUE_DBG(aslam::Exception, rankDeficient || (numRowsToKeep ==
        // dimOfRemainingDesignVariables), "must be the same if full rank!");

        // get the top left block
        SM_ASSERT_GE(aslam::Exception, R.rows(), numTopRowsInCov,
                     "Cannot extract " << numTopRowsInCov << " rows of R because it only has " << R.rows() << " rows.");
        SM_ASSERT_GE(aslam::Exception, R.cols(), numTopRowsInCov,
                     "Cannot extract " << numTopRowsInCov << " cols of R because it only has " << R.cols() << " cols.");
        // outRtop = R.block(0, 0, numTopRowsInRtop, numTopRowsInRtop);

        // cut off the zero rows at the bottom
        R_reduced = R.block(dimOfDesignVariablesToRemove, dimOfDesignVariablesToRemove, dimOfRemainingDesignVariables,
                            dimOfRemainingDesignVariables);
        // R_reduced = R.block(dimOfDesignVariablesToRemove, dimOfDesignVariablesToRemove, numRowsToKeep,
        // dimOfRemainingDesignVariables);
    ...
    boost::shared_ptr<aslam::backend::MarginalizationPriorErrorTerm> err(
        new aslam::backend::MarginalizationPriorErrorTerm(remainingDesignVariables, d_reduced, R_reduced));
