...
void OptimizationProblem::addDesignVariable(boost::shared_ptr<DesignVariable> dv) {
    // \todo This is also bad and slow.
    for (unsigned i = 0; i < _designVariables.size(); ++i) {
        SM_ASSERT_NE(std::runtime_error, dv.get(), _designVariables[i].get(),
                     "That design variable has already been added");
    }
...
void OptimizationProblem::addErrorTerm(const boost::shared_ptr<ErrorTerm>& et) {
    ...
    for (size_t i = 0; i < et->numDesignVariables(); ++i) {
        ...
        SM_ASSERT_TRUE_DBG(aslam::InvalidArgumentException, isDesignVariableInProblem(dv),
                           "It is illegal to add an error term that contains a missing design variable. Add the design "
                           "variables to the problem before adding the error terms.");
...
bool OptimizationProblem::isDesignVariableInProblem(const DesignVariable* dv) {
    for (size_t i = 0; i < _designVariables.size(); ++i) {
        if (_designVariables[i].get() == dv) return true;
    }
    return false;
}
...
size_t OptimizationProblem::numDesignVariablesImplementation() const { return _designVariables.size(); }

DesignVariable* OptimizationProblem::designVariableImplementation(size_t i) { return _designVariables[i].get(); }

const DesignVariable* OptimizationProblem::designVariableImplementation(size_t i) const {
    return _designVariables[i].get();
}

size_t OptimizationProblem::numErrorTermsImplementation() const { return _errorTerms.size(); }

ErrorTerm* OptimizationProblem::errorTermImplementation(size_t i) { return _errorTerms[i].get(); }

const ErrorTerm* OptimizationProblem::errorTermImplementation(size_t i) const { return _errorTerms[i].get(); }
