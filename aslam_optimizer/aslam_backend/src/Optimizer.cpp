...
Optimizer::Optimizer(const OptimizerOptions& options) : _options(options) { initializeLinearSolver(); }

Optimizer::~Optimizer() {}

/// \brief Set up to work on the optimization problem.
void Optimizer::setProblem(boost::shared_ptr<OptimizationProblemBase> problem) { _problem = problem; }
...
void Optimizer::initializeLinearSolver() {
    ...
    if (_options.linearSolver == "cholmod") {
        ...
        _options.verbose&& std::cout << "Unknown linear solver specified: " << _options.linearSolver
                                     << ". Using cholmod.\n";
        _solver.reset(new sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd>());
    }
}
...
void Optimizer::initialize() {
    ...
    SM_ASSERT_FALSE(Exception, _designVariables.empty(),
                    "It is illegal to run the optimizer with all marginalized design variables.");
    ...
    initMx.stop();
    _options.verbose&& std::cout << "Optimization problem initialized with " << _designVariables.size()
                                 << " design variables and " << _errorTerms.size() << " error terms\n";
    _options.verbose&& std::cout << "The dense part of the state is " << _H.rowBaseOfBlock(_marginalizedStartingBlock)
                                 << " parameters and the sparse part is "
                                 << _H.cols() - _H.rowBaseOfBlock(_marginalizedStartingBlock) << " parameters\n";
}
        ...
        for (int i = 0; i < _H.bRows(); i++) {
            ...
            if (H) {
                for (int j = 0; j < H->rows(); j++) {
                    if (fabs((*H)(j, j)) > maximum) maximum = fabs((*H)(i, j));
                }
            }
        }
...
SolutionReturnValue Optimizer::optimizeDogLeg() {
    ...
    while (srv.iterations < _options.maxIterations && deltaX > _options.convergenceDeltaX &&
    ...
    // while( !evaluateStoppingCriterion() )
    {
        ...
        do {
            ...
            // otherwise check the GN step
                ...
                if (!gnComputed) {
                    ...
                    timeSchur.start();
                    applySchurComplement(_H, _rhs, _lambda, _marginalizedStartingBlock, _options.doLevenbergMarquardt,
                                         _A, _invVi, _b);
                    timeSchur.stop();
                    ...
                    if (!solutionSuccess) {
                        ...
                        if (!_fallbackSolver) {  // initialise a new solver
                            ...
                            _options.verbose&& std::cout << "Optimizer: The linear solution failed. Fallback to QR. ("
                                                         << linearSolverFailCounter << ")" << std::endl;
                        }

                        solutionSuccess = _fallbackSolver->solve(_A, &_dx_gn[0], &_b[0]);
                        if (!solutionSuccess) {
                            _options.verbose&& std::cout << "Optimizer: The linear solution failed. Again... ("
                                                         << linearSolverFailCounter << ")" << std::endl;
                            _dx_gn = _dx_sd;
                        }
                        ...
                        // SM_ASSERT_TRUE(Exception, linearSolverFailCounter <= _options.linearSolverMaximumFails, "The
            ...
            _options.verbose&& std::cout << "   (" << stepIterations << "): J: " << _J << ", dJ: " << deltaJ
                                         << ", deltaX: " << deltaX << ", delta: " << _p_delta
                                         << ", StepType: " << stepType;
        ...
        _options.verbose&& std::cout << "[" << srv.iterations << "]: J: " << _J << ", dJ: " << deltaJ
                                     << ", deltaX: " << deltaX << ", delta: " << _delta << std::endl
                                     << std::endl;
    }
...
SolutionReturnValue Optimizer::optimize() {
    ...
    while (srv.iterations < _options.maxIterations && deltaX > _options.convergenceDeltaX &&
        ...
        timeSchur.start();
        applySchurComplement(_H, _rhs, _lambda, _marginalizedStartingBlock, _options.doLevenbergMarquardt, _A, _invVi,
                             _b);
        timeSchur.stop();
        // Solve the dense system.
        if (_options.resetSolverEveryIteration) {
        ...
        if (_J < _p_J) _p_J = _J;
        _options.verbose&& std::cout << "[" << srv.iterations << "]: J: " << _J << ", dJ: " << deltaJ
                                     << ", deltaX: " << deltaX << ", lambda: " << _lambda << std::endl;
    }
    srv.JFinal = _J;
    srv.lmLambdaFinal = _lambda;
    srv.dXFinal = deltaX;
...
size_t Optimizer::numDenseDesignVariables() const { return _marginalizedStartingBlock; }

size_t Optimizer::numSparseDesignVariables() const { return _designVariables.size() - _marginalizedStartingBlock; }
...
OptimizerOptions& Optimizer::options() { return _options; }
...
const SparseBlockMatrix& Optimizer::H() const { return _H; }

/// \brief return the full rhs
const Eigen::VectorXd& Optimizer::rhs() const { return _rhs; }

/// \brief return the reduced system lhs
const SparseBlockMatrix& Optimizer::A() const { return _A; }

/// \brief return the reduced system rhs
const Eigen::VectorXd& Optimizer::b() const { return _b; }

/// \brief return the reduced system dx
const Eigen::VectorXd& Optimizer::dx() const { return _dx; }

/// The value of the objective function.
double Optimizer::J() const { return _J; }

void Optimizer::printTiming() const { sm::timing::Timing::print(std::cout); }
...
void Optimizer::computeCovarianceBlocks(const std::vector<std::pair<int, int> >& blockIndices) {
    ...
    boost::shared_ptr<sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd> > solver(
        new sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd>());
...
const SparseBlockMatrix& Optimizer::P() const { return _invH; }

const SparseBlockMatrix& Optimizer::getCovariance() const { return _invH; }
