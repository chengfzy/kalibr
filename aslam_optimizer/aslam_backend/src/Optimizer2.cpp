...
Optimizer2::Optimizer2(const Optimizer2Options& options) : _options(options) {
    initializeLinearSolver();
    initializeTrustRegionPolicy();
}

Optimizer2::Optimizer2(const sm::PropertyTree& config, boost::shared_ptr<LinearSystemSolver> linearSystemSolver,
    ...
    initializeLinearSolver();
    initializeTrustRegionPolicy();
    // USING C++11 would allow to do constructor delegation and more elegant code, i.e., directly call the upper
    // constructor
}
...
void Optimizer2::setProblem(boost::shared_ptr<OptimizationProblemBase> problem) { _problem = problem; }

void Optimizer2::initializeTrustRegionPolicy() {
    if (!_options.trustRegionPolicy) {
        _options.verbose&& std::cout
            << "No trust region policy set in the options. Defaulting to levenberg_marquardt\n";
    ...
    if (_solver->name() == "sparse_qr" && _trustRegionPolicy->name() == "levenberg_marquardt") {
        _options.verbose&& std::cout << "The sparse_qr solver is not compatible with levenberg_marquardt. Changing to "
                                        "the dog_leg trust region policy\n";
...
void Optimizer2::initialize() {
    ...
    for (size_t i = 0; i < _problem->numDesignVariables(); ++i) {
        ...
        if (dv->isActive()) _designVariables.push_back(dv);
    }
...
size_t Optimizer2::numDesignVariables() const { return _designVariables.size(); }
...
Optimizer2Options& Optimizer2::options() { return _options; }
...
const Eigen::VectorXd& Optimizer2::dx() const { return _dx; }

/// The value of the objective function.
double Optimizer2::J() const { return _J; }

void Optimizer2::printTiming() const { sm::timing::Timing::print(std::cout); }
...
void Optimizer2::computeCovarianceBlocks(const std::vector<std::pair<int, int> >& /* blockIndices */,
                                         SparseBlockMatrix& /* outP */, double /* lambda */) {
    SM_THROW(Exception, "Broken");
}
...
const LinearSystemSolver* Optimizer2::getBaseSolver() const { return _solver.get(); }

const Matrix* Optimizer2::getJacobian() const { return _solver->Jacobian(); }
