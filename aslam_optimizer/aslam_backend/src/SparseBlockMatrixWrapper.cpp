...
size_t SparseBlockMatrixWrapper::rows() const { return _M.rows(); }

/// \brief The number of columns in the matrix.
size_t SparseBlockMatrixWrapper::cols() const { return _M.cols(); }

/// \brief Get a value from the matrix at row r and column c
double SparseBlockMatrixWrapper::operator()(size_t r, size_t c) const { return _M(r, c); }
...
void SparseBlockMatrixWrapper::toDenseInto(Eigen::MatrixXd& outM) const { _M.toDenseInto(outM); }

/// \brief Initialize the matrix from a dense matrix
void SparseBlockMatrixWrapper::fromDense(const Eigen::MatrixXd& M) { fromDenseTolerance(M, 0.0); }
...
void SparseBlockMatrixWrapper::fromDenseTolerance(const Eigen::MatrixXd& inM, double tolerance) {
    SM_ASSERT_EQ(Matrix::Exception, (size_t)inM.rows(), (size_t)_M.rows(),
                 "The dense matrix is the wrong size. The SparseBlockMatrix needs an existing block structure");
    SM_ASSERT_EQ(Matrix::Exception, (size_t)inM.cols(), (size_t)_M.cols(),
                 "The dense matrix is the wrong size. The SparseBlockMatrix needs an existing block structure");
    _M.clear(true);
    // For each block.
