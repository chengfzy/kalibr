...
SparseQrLinearSystemSolver::SparseQrLinearSystemSolver(const SparseQRLinearSolverOptions& options)
    : _factor(NULL), _options(options) {}

SparseQrLinearSystemSolver::SparseQrLinearSystemSolver(const sm::PropertyTree& config) : _factor(NULL) {
...
void SparseQrLinearSystemSolver::initMatrixStructureImplementation(const std::vector<DesignVariable*>& dvs,
...
bool SparseQrLinearSystemSolver::solveSystem(Eigen::VectorXd& outDx) {
    ...
    cholmod_dense* sol =
        _cholmod.solve(&_cholmodLhs, _factor, &_cholmodRhs, _options.qrTol, _options.colNorm, _options.normTol);
    if (_useDiagonalConditioner) {
        J_transpose.popDiagonalBlock();
    }
    if (!sol) {
    ...
    _cholmod.free(sol);
    if (_options.verbose) std::cout << "numerical rank: " << _factor->rank << std::endl;
    // std::cout << "solve system complete\n";
    return true;
}

const SparseQRLinearSolverOptions& SparseQrLinearSystemSolver::getOptions() const { return _options; }

SparseQRLinearSolverOptions& SparseQrLinearSystemSolver::getOptions() { return _options; }

void SparseQrLinearSystemSolver::setOptions(const SparseQRLinearSolverOptions& options) { _options = options; }

const CompressedColumnMatrix<SuiteSparse_long>& SparseQrLinearSystemSolver::getJacobianTranspose() const {
    return _jacobianBuilder.J_transpose();
}

SuiteSparse_long SparseQrLinearSystemSolver::getRank() const {
    SM_ASSERT_FALSE(Exception, _factor == NULL, "QR decomposition has not run yet");
    return _factor->rank;
}
...
std::vector<SuiteSparse_long> SparseQrLinearSystemSolver::getPermutationVector() const {
    SM_ASSERT_FALSE(Exception, _factor == NULL, "QR decomposition has not run yet");
    return std::vector<SuiteSparse_long>(_factor->Q1fill, _factor->Q1fill + _cholmodLhs.nrow);
}

Eigen::Matrix<SparseQrLinearSystemSolver::index_t, Eigen::Dynamic, 1>
SparseQrLinearSystemSolver::getPermutationVectorEigen() const {
    SM_ASSERT_FALSE(Exception, _factor == NULL, "QR decomposition has not run yet");
    Eigen::Map<Eigen::Matrix<SparseQrLinearSystemSolver::index_t, Eigen::Dynamic, 1> > pv(_factor->Q1fill,
                                                                                          _cholmodLhs.nrow);
    return pv;
}

const CompressedColumnMatrix<SuiteSparse_long>& SparseQrLinearSystemSolver::getR() {
    if (_R.nnz() == 0) {
        CompressedColumnMatrix<SuiteSparse_long>& J_transpose = _jacobianBuilder.J_transpose();
        J_transpose.getView(&_cholmodLhs);
        cholmod_sparse* R;
...
size_t SparseQrLinearSystemSolver::getMemoryUsage() const { return _cholmod.getMemoryUsage(); }

void SparseQrLinearSystemSolver::analyzeSystem() {
    CompressedColumnMatrix<SuiteSparse_long>& J_transpose = _jacobianBuilder.J_transpose();
    J_transpose.getView(&_cholmodLhs);
    if (_factor == NULL) _factor = _cholmod.analyzeQR(&_cholmodLhs);
    SM_ASSERT_TRUE(Exception, _cholmod.factorize(&_cholmodLhs, _factor, _options.qrTol, true),
