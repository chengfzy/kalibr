...
TEST(CompressColumnMatrixTestSuite, testJcBuilder) {
    ...
    try {
        ...
        double e0 = 0;
        for (unsigned i = 0; i < errs.size(); ++i) e0 += errs[i]->evaluateError();
        ...
        for (unsigned j = 0; j < 2 * errs.size(); ++j) {
            ...
            for (unsigned i = 0; i < errs.size(); ++i) {
                ...
                ASSERT_DOUBLE_MX_EQ(Jrow, JrowFromError, 1e-6,
                                    "these should be similar. Number of threads: " << j << ", block row: " << i);
        ...
        for (int i = 0; i < D; ++i) delete dvs[i];
        for (int i = 0; i < E; ++i) delete errs[i];
        FAIL() << e.what();
    }
    for (unsigned i = 0; i < dvs.size(); ++i) delete dvs[i];
    for (unsigned i = 0; i < errs.size(); ++i) delete errs[i];
}
