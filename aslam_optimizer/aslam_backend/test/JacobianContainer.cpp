...
TEST(JacobianContainerTests, testOrdering) {
    ...
    // by block index
    JacobianContainer::map_t::const_iterator itk = jc.begin(), itkm1 = jc.begin(), it_end = jc.end();
    itk++;
    for (; itk != it_end; ++itk, ++itkm1) ASSERT_LT((itkm1)->first->blockIndex(), itk->first->blockIndex());
}
...
TEST(JacobianContainerTests, testChainRule) {
    try {
        ...
        jc.applyChainRule(H);
        ASSERT_EQ(5, jc.rows());
        sm::eigen::assertEqual(H * J1, jc.Jacobian(&dv1), SM_SOURCE_FILE_POS,
                               "Checking for correct application of the chain rule");
        sm::eigen::assertEqual(H * J2, jc.Jacobian(&dv2), SM_SOURCE_FILE_POS,
                               "Checking for correct application of the chain rule");
        sm::eigen::assertEqual(H * J3, jc.Jacobian(&dv3), SM_SOURCE_FILE_POS,
                               "Checking for correct application of the chain rule");
        sm::eigen::assertEqual(H * J4, jc.Jacobian(&dv4), SM_SOURCE_FILE_POS,
                               "Checking for correct application of the chain rule");
    } catch (const std::exception& e) {
        FAIL() << "Exception: " << e.what();
    }
...
TEST(JacobianContainerTests, testAddContainers) {
    try {
        ...
        sm::eigen::assertEqual(jc1PlusJc2.Jacobian(&dv1), J1, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        sm::eigen::assertEqual(jc1PlusJc2.Jacobian(&dv2), J2 + J2, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        sm::eigen::assertEqual(jc1PlusJc2.Jacobian(&dv3), J3, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        sm::eigen::assertEqual(jc1PlusJc2.Jacobian(&dv4), J4, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        /// Check that addition is symmetric
        JacobianContainer jc2PlusJc1 = jc2;
        jc2PlusJc1.add(jc1);
        ASSERT_EQ(4u, jc2PlusJc1.numDesignVariables());
        sm::eigen::assertEqual(jc2PlusJc1.Jacobian(&dv1), J1, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        sm::eigen::assertEqual(jc2PlusJc1.Jacobian(&dv2), J2 + J2, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        sm::eigen::assertEqual(jc2PlusJc1.Jacobian(&dv3), J3, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
        sm::eigen::assertEqual(jc2PlusJc1.Jacobian(&dv4), J4, SM_SOURCE_FILE_POS,
                               "Checking for correct Jacobians after add");
    } catch (const std::exception& e) {
        FAIL() << "Exception: " << e.what();
    }
...
TEST(JacobianContainerTests, testBuildHessian) {
    try {
        ...
        ASSERT_DOUBLE_MX_EQ(rhs.segment(Hessian.rowBaseOfBlock(1), J2.cols()), -J2.transpose() * invR * e, 1e-9,
                            "Block 1");
    } catch (const std::exception& e) {
        FAIL() << "Exception: " << e.what();
    }
...
TEST(JacobianContainerTests, testBuildHessian2) {
    try {
        ...
        ASSERT_DOUBLE_MX_EQ(rhs.segment(Hessian.rowBaseOfBlock(1), J2.cols()), -2.0 * J2.transpose() * invR * e, 1e-9,
                            "Block 1");
    } catch (const std::exception& e) {
        FAIL() << "Exception: " << e.what();
    }
