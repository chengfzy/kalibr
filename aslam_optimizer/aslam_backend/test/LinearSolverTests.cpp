    ...
    for (int nThreads = 0; nThreads < 4; ++nThreads) {
        {
            useDiag = false;
            SCOPED_TRACE(("No Diagonal and " + boost::lexical_cast<std::string>(nThreads) + " threads").c_str());
            compareSolvers<SparseCholeskyLinearSystemSolver, BlockCholeskyLinearSystemSolver>(D, E, useM, useDiag,
                                                                                              nThreads);
        }
        {
            useDiag = true;
...
TEST(LinearSolverTestSuite, testSparseQR) {
    ...
    for (int nThreads = 0; nThreads < 4; ++nThreads) {
        ...
        /*
                {
                    useDiag = true;
                    SCOPED_TRACE(("With Diagonal and " + boost::lexical_cast<std::string>(nThreads) + "
           threads").c_str()); compareSolvers<SparseCholeskyLinearSystemSolver, SparseQrLinearSystemSolver>(D,E,useM,
           useDiag, nThreads);
                }*/
    }
}
