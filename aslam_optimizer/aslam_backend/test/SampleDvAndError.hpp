...
class Point2d : public aslam::backend::DesignVariable {
  ...
  protected:
    ...
    virtual void getParametersImplementation(Eigen::MatrixXd& value) const { value = _v; }
...
class LinearErr : public aslam::backend::ErrorTermFs<2> {
  public:
    ...
    virtual void evaluateJacobiansImplementation(aslam::backend::JacobianContainer& outJ) const { outJ.add(_p2d, -_J); }
};
...
class LinearErr3 : public aslam::backend::ErrorTermFs<4> {
  public:
    ...
    LinearErr3(Point2d* p2d1, Point2d* p2d2, Point2d* p3) : _p2d1(p2d1), _p2d2(p2d2), _p3(p3) {
        ...
        _J3.setRandom();
        parent_t::setDesignVariables((aslam::backend::DesignVariable*)_p2d1, (aslam::backend::DesignVariable*)_p2d2,
                                     (aslam::backend::DesignVariable*)_p3);
...
inline void buildSystem(int D, int E, std::vector<aslam::backend::DesignVariable*>& dvs,
    ...
    for (int i = 0; i < E; ++i) {
        ...
        if (mod == 0) {
            ...
            // std::cout << "le3\n";
            errs.push_back(new LinearErr3((Point2d*)dvs[i % dvs.size()], (Point2d*)dvs[(i + 1) % dvs.size()],
                                          (Point2d*)dvs[(i + 2) % dvs.size()]));
        }
...
inline void deleteSystem(std::vector<aslam::backend::DesignVariable*>& dvs,
                         std::vector<aslam::backend::ErrorTerm*>& errs) {
    using namespace aslam::backend;
