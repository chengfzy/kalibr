...
TEST(OptimizerTestSuite, testOptimizerMatrices) {
    try {
        ...
        //    {
        //      std::cout << "\tdv[" << j << "] has block index " <<
        //      problem.errorTerm(i).designVariable(j)->blockIndex() << std::endl; std::cout << "\tdv[" << j << "]: " <<
        //      ( (void*)problem.errorTerm(i).designVariable(j)) << std::endl;
        //    }
        //   }
        for (int p = 0; p < P; ++p) problem.designVariable(p)->revertUpdate();
        ...
        sm::eigen::assertNear(estH, H, 1e-14, SM_SOURCE_FILE_POS,
                              "Did we build the Hessian correctly? A is from J^T * J and B is from the optimizer.");
        Eigen::VectorXd estRhs = -J.transpose() * e;
        sm::eigen::assertNear(estRhs, optimizer.rhs(), 1e-14, SM_SOURCE_FILE_POS,
                              "Did we build the right hand side correctly?");
        for (unsigned i = 0; i < problem.numErrorTerms(); ++i) {
            cloneH->clear();
            J.setZero();
            int ridx = 2 * i;
            ...
            sm::eigen::assertNear(*cloneH, eH, 1e-14, SM_SOURCE_FILE_POS,
                                  "Did we build the Hessian correctly? A is from J^T * J and B is from the optimizer.");
        }
