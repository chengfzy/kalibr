...
TEST(SparseMatrixFunctionTests, testSchurComplement1) {
    try {
        ...
        b.setZero();
        boost::shared_ptr<SparseBlockMatrix> A(
            H.slice(0, marginalizedStartingBlock, 0, marginalizedStartingBlock, true));
        ...
        *H.block(4, 4, true) = sm::eigen::randomCovariance<3>() * 100;
        ...
        // std::cout << "H:\n" << H << std::endl;
        applySchurComplement(H, e, lambda, marginalizedStartingBlock, true, *A, invVi, b);
        Eigen::MatrixXd Hd = H.toDense();
        Eigen::MatrixXd U = Hd.topLeftCorner(marginalizedStartingIndex, marginalizedStartingIndex);
        Eigen::MatrixXd V =
            Hd.bottomRightCorner(H.rows() - marginalizedStartingIndex, H.rows() - marginalizedStartingIndex);
        ...
        for (int i = 0; i < H.bRows() - marginalizedStartingBlock; ++i) {
            ...
            Eigen::VectorXd dsi =
                denseDs.segment(H.rowBaseOfBlock(i + marginalizedStartingBlock) - marginalizedStartingIndex,
                                H.rowsOfBlock(i + marginalizedStartingBlock));
