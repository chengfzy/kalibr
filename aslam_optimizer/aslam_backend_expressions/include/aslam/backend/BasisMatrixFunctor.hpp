...
struct BasisMatrixNullaryFunctor {
  ...
  public:
    ...
    inline BasisMatrixNullaryFunctor(const Eigen::DenseIndex& cwiseIndex, const Eigen::DenseIndex& i,
                                     const Eigen::DenseIndex& j)
        : _cwiseIndex(cwiseIndex), _i(i), _j(j) {}
    inline result_type operator()(Eigen::DenseIndex& index) const { return (_cwiseIndex == index) ? 1 : 0; }
