...
class DesignVariableGenericVector : public DesignVariable, public GenericMatrixExpressionNode<D, 1, Scalar_> {
  public:
    ...
    DesignVariableGenericVector(vector_t v = vector_t::Zero()) : base_t(v) {}
    virtual ~DesignVariableGenericVector() {}
    const vector_t& value() const { return base_t::evaluate(); }
  ...
  protected:
    ...
    virtual void revertUpdateImplementation() { this->_currentValue = _p_v; }
    ...
    virtual int minimalDimensionsImplementation() const { return D; }
    ...
    // TODO implement: virtual void minimalDifferenceImplementation(const Eigen::MatrixXd& xHat, Eigen::VectorXd&
    // outDifference) const;

    /// Computes the minimal distance in tangent space between the current value of the DV and xHat and the jacobian
    // TODO implement: virtual void minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat,
    // Eigen::VectorXd& outDifference, Eigen::MatrixXd& outJacobian) const;
