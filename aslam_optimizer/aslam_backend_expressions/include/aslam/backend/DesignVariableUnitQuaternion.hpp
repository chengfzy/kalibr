...
template <typename Scalar_ = double, enum QuaternionMode Mode_ = DefaultQuaternionMode,
          enum UnitQuaternionGeometry Geometry_ = DefaultUnitQuaternionGeometry>
class DesignVariableUnitQuaternion : public DesignVariableGenericVector<4, Scalar_> {
  ...
  public:
    ...
    DesignVariableUnitQuaternion(vector_t v = Calculator::getIdentity()) : base_t(v) {}
  ...
  protected:
    ...
    virtual void updateImplementation(const double *dp, int size) {
        ...
        this->_currentValue = Calculator::template update<Geometry_>(
            this->_currentValue, Eigen::Map<const Eigen::Matrix<double, 3, 1> >(dp).template cast<Scalar_>());
    }
