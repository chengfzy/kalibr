...
class Differential {
  public:
    ...
    inline domain_t getDomainBasisVectorByIndex(Eigen::DenseIndex index) const {
        static_assert(domain_t::RowsAtCompileTime != Eigen::Dynamic && domain_t::ColsAtCompileTime != Eigen::Dynamic,
                      "dynamic dimension is not supported yet");
        return domain_t::NullaryExpr(domain_t::RowsAtCompileTime, domain_t::ColsAtCompileTime,
                                     internal::BasisMatrixNullaryFunctor(index, index % domain_t::RowsAtCompileTime,
                                                                         index / domain_t::RowsAtCompileTime));
    }
    ...
    virtual void addToJacobianContainer(JacobianContainer& jc, const DesignVariable* dv,
                                        const dyn_matrix_t& jacobian) const = 0;
};
...
class NullDifferential : public Differential<TDomain, TScalar> {
  public:
    ...
    virtual void applyInto(const typename base_t::domain_t& /* tangent_vector */,
                           typename base_t::result_vector_t& result) const {
        result.setZero();
    }
    ...
    virtual void addToJacobianContainer(JacobianContainer& /* jc */, const DesignVariable* /* dv */,
                                        const typename base_t::dyn_matrix_t& /* jacobian */) const {}
...
struct DifferentialCalculator {
    ...
    inline static void addToJacobianByApplication(const TDiff& /* diff */, JacobianContainer& /* jc */,
                                                  const DesignVariable* /* dv */) {
...
struct DifferentialCalculator<TDiff, Eigen::Dynamic> {
    ...
    inline static typename DifferentialCalculator<TDiff, 1>::compose_result_t compose(const TDiff& diff,
                                                                                      const matrix_t& jacobian) {
        // TODO runtime check domain columns (there has to be one)
...
class ComposedDifferential : public Differential<TDomain, TScalar> {
  public:
    ...
    ComposedDifferential(const next_differential_t& next_differential) : _next_differential(next_differential) {}
...
class MatrixDifferential : public Differential<Eigen::Matrix<TScalar, ICols, 1>, TScalar> {
  ...
  public:
    ...
    inline MatrixDifferential(TMatrix mat) : _mat(mat) {}
...
class ComposedMatrixDifferential
    : public ComposedDifferential<Eigen::Matrix<TScalar, IDomainRows, 1>, TNextDomain, TScalar,
                                  ComposedMatrixDifferential<TNextDomain, TScalar, TMatrix, IDomainRows> > {
  protected:
    TMatrix _mat;
  ...
  public:
    ...
    typedef typename base_t::dyn_matrix_t dyn_matrix_t;
    typedef typename base_t::dyn_vector_t dyn_vector_t;
    typedef ComposedMatrixDifferential<
        TNextDomain, TScalar,
        Eigen::Matrix<TScalar, Eigen::internal::remove_all<TMatrix>::type::RowsAtCompileTime,
    ...
    inline ComposedMatrixDifferential(TMatrix mat, const typename base_t::next_differential_t& next_differential)
        : base_t(next_differential), _mat(mat) {}
    ...
    inline typename base_t::next_domain_t applyBasisVector(int index) const { return _mat.col(index); }
