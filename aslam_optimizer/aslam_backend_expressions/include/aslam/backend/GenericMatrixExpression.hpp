...
template <int IRows, int ICols, typename TScalar = double,
          typename TNode = GenericMatrixExpressionNode<IRows, ICols, TScalar> >
class GenericMatrixExpression {
  public:
    ...
    /// \brief Evaluate the full Eigen matrix.
    matrix_t evaluate() const { return toFullMatrix(); }
