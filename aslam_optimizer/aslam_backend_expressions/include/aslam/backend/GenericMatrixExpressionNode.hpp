...
class ConstantGenericMatrixExpressionNode;
...
class GenericMatrixExpressionNode {
  ...
  protected:
    ...
    inline GenericMatrixExpressionNode(const matrix_t& value) : _currentValue(value), _valueDirty(false) {}

  public:
    GenericMatrixExpressionNode(int rows = IRows, int cols = ICols, bool valueDirty = true)
        : _currentValue(rows, cols), _valueDirty(valueDirty) {}
    virtual ~GenericMatrixExpressionNode() {}

    inline matrix_t& getCurrentValue() { return _currentValue; }
    inline const matrix_t& getCurrentValue() const { return _currentValue; }
    ...
    const matrix_t toMatrix() const { return evaluate(); };

    void getDesignVariables(DesignVariable::set_t& designVariables) const {
        return getDesignVariablesImplementation(designVariables);
    }
    bool isConstant() const { return isConstantImplementation(); }

    void inline invalidate() { _valueDirty = true; };
  ...
  protected:
    ...
    virtual void evaluateJacobiansImplementation(JacobianContainer& outJacobians,
                                                 const differential_t& chainRuleDifferentail) const = 0;
    virtual bool isConstantImplementation() const { return false; }
...
class ConstantGenericMatrixExpressionNode : public GenericMatrixExpressionNode<IRows, ICols, TScalar> {
  public:
    ...
    ConstantGenericMatrixExpressionNode(const Eigen::MatrixBase<DERIVED>& value) : base_t(value) {}
    ConstantGenericMatrixExpressionNode(int rows = IRows, int cols = ICols) : base_t(rows, cols, false) {}

  protected:
    virtual bool isConstantImplementation() const { return true; }
    virtual void evaluateImplementation() const {}
    virtual void evaluateJacobiansImplementation(
        JacobianContainer& /* outJacobians */,
        const typename base_t::differential_t& /* chainRuleDifferentail */) const {}
