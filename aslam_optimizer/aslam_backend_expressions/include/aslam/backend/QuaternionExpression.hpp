...
inline constexpr bool isRealFirst(const QuaternionMode mode) { return ((int)mode & internal::REAL_IS_LAST_MASK) == 0; }
...
template <typename TScalar = double, enum QuaternionMode EMode = DefaultQuaternionMode,
          typename TNode = typename GenericMatrixExpression<4, 1, TScalar>::node_t>
class QuaternionExpression : public GenericMatrixExpression<4, 1, TScalar, TNode> {
  public:
    ...
    typedef QuaternionExpression<TScalar, EMode, typename GenericMatrixExpression<4, 1, TScalar>::node_t>
        self_with_default_node_t;
    ...
    inline base_t toVectorExpression() { return base_t(this->root()); }
...
class UnitQuaternionExpression : public QuaternionExpression<TScalar, EMode, TNode> {
  public:
    ...
    typedef UnitQuaternionExpression<TScalar, EMode, typename GenericMatrixExpression<4, 1, TScalar>::node_t>
        self_with_default_node_t;
    typedef typename base_t::value_t value_t;
    ...
    inline self_with_default_node_t inverse() const { return conjugate(); }
