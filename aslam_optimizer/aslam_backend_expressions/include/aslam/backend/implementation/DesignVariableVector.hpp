...
void DesignVariableVector<D>::evaluateJacobiansImplementation(JacobianContainer& outJacobians) const {
    SM_ASSERT_EQ_DBG(aslam::InvalidArgumentException, outJacobians.rows(), D,
                     "The Jacobian container dimension doesn't match the state. Are you missing a chain rule?");
    outJacobians.add(const_cast<DesignVariableVector<D>*>(this), Eigen::Matrix<double, D, D>::Identity());
}

template <int D>
void DesignVariableVector<D>::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
                                                              const Eigen::MatrixXd& applyChainRule) const {
    SM_ASSERT_EQ_DBG(aslam::InvalidArgumentException, applyChainRule.cols(), D,
                     "The chain rule matrix dimension doesn't match the state.");
...
template <int D>
void DesignVariableVector<D>::getParametersImplementation(Eigen::MatrixXd& value) const {
    value = _v;
}

template <int D>
void DesignVariableVector<D>::setParametersImplementation(const Eigen::MatrixXd& value) {
    _p_v = _v;
    _v = value;
}

template <int D>
void DesignVariableVector<D>::minimalDifferenceImplementation(const Eigen::MatrixXd& xHat,
...
template <int D>
void DesignVariableVector<D>::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat,
