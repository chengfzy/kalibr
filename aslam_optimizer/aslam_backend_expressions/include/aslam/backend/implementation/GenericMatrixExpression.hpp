...
_TEMPLATE _CLASS::GenericMatrixExpression(node_ptr_t root) : _root(root) {}

_TEMPLATE
_CLASS::GenericMatrixExpression(node_t* root) : _root(root, sm::null_deleter()) {}
...
_TEMPLATE
typename _CLASS::matrix_t _CLASS::toFullMatrix() const { return _root->evaluate(); }
...
void _CLASS::evaluateJacobians(JacobianContainer& outJacobians, const Eigen::MatrixXd& applyChainRule) const {
    _root->evaluateJacobians(outJacobians, MatrixDifferential<TScalar, Eigen::Matrix<TScalar, Eigen::Dynamic, IRows> >(
                                               applyChainRule.cast<TScalar>()));
}
...
_TEMPLATE
inline _CLASS::operator GenericMatrixExpression<IRows, ICols, TScalar>() const {
    ...
    class ResultNode : public result_t::template UnaryOperationResult<ResultNode, self_t> {
      public:
        ...
        virtual void evaluateImplementation() const { this->_currentValue = this->getOperandNode().evaluate(); }

        inline typename base_t::apply_diff_return_t applyDiff(
            const typename base_t::operand_t::tangent_vector_t& tangent_vector) const {
...
GenericMatrixExpression<IRows, IColsOther, TScalar> _CLASS::operator*(
...
typename _CLASS::default_self_t _CLASS::operator+(
    const GenericMatrixExpression<IRows, ICols, TScalar, TOtherNode>& other) const {
    typedef default_self_t result_t;
    ...
    inline static void calcCrossInto(const TLhs& lhs, const TRhs& rhs, TResult& result) {
        ...
        if (lhs.cols() == 1)
            for (int i = 0; i < resultCols; i++)
                result.template block<3, 1>(0, i) = lhs.cross(rhs.template block<3, 1>(0, i));
        else
            for (int i = 0; i < resultCols; i++)
                result.template block<3, 1>(0, i) = lhs.template block<3, 1>(0, i).cross(rhs);
    }
};
...
template <int IColsOther, typename TOtherNode,
          typename std::enable_if<IRows == 3 && (ICols == 1 || IColsOther == 1), int>::type>
GenericMatrixExpression<3, (ICols == 1 ? IColsOther : ICols), TScalar> _CLASS::cross(
    ...
    class ResultNode : public result_t::template BinaryOperationResult<ResultNode, self_t, other_t> {
      public:
        ...
        virtual void evaluateImplementation() const {
            calculator_t::calcCrossInto(this->getLhsNode().evaluate(), this->getRhsNode().evaluate(),
                                        this->_currentValue);
        }
