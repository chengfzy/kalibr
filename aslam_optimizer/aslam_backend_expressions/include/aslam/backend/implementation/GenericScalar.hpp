...
#define MEMBER(RET, DECL)       \
    template <typename Scalar_> \
    RET _CLASS::DECL

MEMBER(, GenericScalar(Scalar_ p)) : _p(p), _p_p(p) {}

MEMBER(, ~GenericScalar()) {}

MEMBER(void, revertUpdateImplementation()) { _p = _p_p; }
...
MEMBER(Scalar_, toScalarImplementation()) const { return _p; }

MEMBER(void, evaluateJacobiansImplementation(JacobianContainer& outJacobians, const Eigen::MatrixXd* applyChainRule))
const {
    if (applyChainRule)
...
MEMBER(GenericScalarExpression<Scalar_>, toExpression()) { return GenericScalarExpression<Scalar_>(this); }
...
MEMBER(void, minimalDifferenceImplementation(const Eigen::MatrixXd& xHat, Eigen::VectorXd& outDifference)) const {
    SM_ASSERT_TRUE(aslam::InvalidArgumentException, (xHat.rows() == 1) & (xHat.cols() == 1),
                   "xHat has incompatible dimensions");
    outDifference = Eigen::VectorXd(1);
    outDifference(0) = (double)(_p - Scalar(xHat(0, 0)));
}

MEMBER(void, minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat, Eigen::VectorXd& outDifference,
