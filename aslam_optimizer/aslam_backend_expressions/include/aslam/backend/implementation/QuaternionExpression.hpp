...
namespace internal {
inline constexpr int getRealIndex(QuaternionMode mode) { return isRealFirst(mode) ? 0 : 3; }
inline constexpr int getIIndex(QuaternionMode mode) { return isRealFirst(mode) ? 1 : 0; }
inline constexpr int getJIndex(QuaternionMode mode) { return isRealFirst(mode) ? 2 : 1; }
inline constexpr int getKIndex(const QuaternionMode mode) { return isRealFirst(mode) ? 3 : 2; }
...
struct EigenQuaternionCalculator {
    ...
    inline static Eigen::Matrix<TScalar, 3, 4> dlog(const vector_t &v) {
        ...
        if (isRealFirst(EMode)) {
            return convertFromOtherMode<QuaternionMode::LAST_IS_REAL_AND_TRADITIONAL_MULT_ORDER, 3>(J.transpose())
                .transpose();
        } else {
            return J;
        }
    ...
    static Eigen::Matrix<TScalar, 4, 3> dUpdate(const vector_t &p) {
        return convertFromOtherMode<QuaternionMode::LAST_IS_REAL_AND_OPPOSITE_MULT_ORDER, 3>(
            ((isTraditionalMultOrder(EMode) != (EGeometry == UnitQuaternionGeometry::RIGHT_TRANSLATED))
                 ...
                 : sm::kinematics::quatPlus(convertToOtherMode<QuaternionMode::LAST_IS_REAL_AND_OPPOSITE_MULT_ORDER>(p)
                                                .template cast<double>()))
                .template block<4, 3>(0, 0)
    ...
    class ResultNode : public result_t::template BinaryOperationResult<ResultNode, self_t, other_t> {
      ...
      private:
        virtual void evaluateImplementation() const {
            this->_currentValue = internal::EigenQuaternionCalculator<TScalar, EMode>::quatMult(
                this->getLhsNode().evaluate(), this->getRhsNode().evaluate());
        }
    };
    ...
    class ResultNode : public result_t::template UnaryOperationResult<ResultNode, self_t> {
      public:
        ...
        inline typename base_t::apply_diff_return_t applyDiff(
            ...
            return -calc_t::quatMult(operandValConj, calc_t::quatMult(tangentVector, operandValConj)) /
...
typename _CLASS::self_with_default_node_t _CLASS::conjugate() const {
    ...
    class ResultNode : public result_t::template UnaryOperationResult<ResultNode, self_t> {
      ...
      private:
        virtual void evaluateImplementation() const {
            this->_currentValue =
                internal::EigenQuaternionCalculator<TScalar, EMode>::conjugate(this->getOperandNode().evaluate());
        }
    };
...
GenericMatrixExpression<3, 1, TScalar> _CLASS::rotate3Vector(
    ...
    class ResultNode : public result_t::template BinaryOperationResult<ResultNode, self_t, other_t> {
      public:
        ...
        inline typename base_t::apply_diff_return_t applyLhsDiff(
            ...
            return calc_t::getImagPart(
                calc_t::quatMult(calc_t::quatMult(lhsVal, rhsVal), calc_t::conjugate(tangent_vector)) +
      ...
      private:
        virtual void evaluateImplementation() const {
            auto lhsVal = this->getLhsNode().evaluate();
            this->_currentValue = calc_t::getImagPart(
                calc_t::quatMult(calc_t::quatMult(lhsVal, this->getRhsNode().evaluate()), calc_t::conjugate(lhsVal)));
        }
    };
