...
struct ExpressionTraits {
    ...
    static double defaultTolerance() { return Limits::is_integer ? 0.001 : sqrt(Limits::epsilon()) * 1E2; }
    static double defaulEps() { return Limits::is_integer ? 1 : sqrt(Limits::epsilon()) * 10; }
};
...
struct ExpressionNumDiffTraits {
    ...
    inline static Eigen::MatrixXd numericallyCalcJacobian(TExpression expression, int jacobianCols,
        ...
        sm::eigen::NumericalDiff<ExpressionNodeFunctor<TExpression>> numdiff(
            ExpressionNodeFunctor<TExpression>(expression, dvs), scalar_t(eps));
...
class ExpressionTester {
  public:
    ...
    inline static std::vector<DesignVariable*> prepareAndCheckDesignVariables(const TExpression& exp,
        ...
        for (DesignVariable* dp : dvs) {
            ...
            if (setActive) {
                ...
                SM_ASSERT_TRUE(std::runtime_error, dp->isActive(),
                               "There are design variables inactive. This is most likely not intended here!");
            }
        }
    ...
    void test() {
        if (_expectedNumberOfDesignVariables > 0) ASSERT_EQ(_expectedNumberOfDesignVariables, _dvs.size());
...
class ExpressionJacobianTestTraits {
  public:
    ...
    static void testJacobian(const ExpressionTester<TExpression>& expressionTester) {
        ...
        Eigen::MatrixXd Jest = ExpressionNumDiffTraits<TExpression>::numericallyCalcJacobian(
            expression, jacobianCols, expressionTester.getDesignVariables(), expressionTester.getEps());

        auto JccrM = expressionTester.getJacobian(Jccr);

        sm::eigen::assertNear(Jest, JcM, expressionTester.getTolerance(), SM_SOURCE_FILE_POS,
                              "Testing the Jacobian with finite differences");
...
template <typename TExpression>
inline void testJacobian(TExpression expression, int expectedNumberOfDesignVariables = -1, bool printResult = false,
                         double tolerance = test::ExpressionTraits<TExpression>::defaultTolerance(),
                         double eps = test::ExpressionTraits<TExpression>::defaulEps()) {
    test::ExpressionTester<TExpression>(expression, expectedNumberOfDesignVariables, false, false, printResult,
                                        tolerance, eps)
        .test();
}

/**
 * Tests an expression. So far the number of design variables is checked against expectedNumberOfDesignVariables and the
 * expression's jacobian is compared to a finite difference approach (parameterized by tolerance and eps). This method
 * activates all the expressions design variables and sets their blockIndex().
 * @param expression
 ...
 */
template <typename TExpression>
inline void testExpression(TExpression expression, int expectedNumberOfDesignVariables, bool printResult = false,
                           double tolerance = test::ExpressionTraits<TExpression>::defaultTolerance(),
                           double eps = test::ExpressionTraits<TExpression>::defaulEps()) {
    test::ExpressionTester<TExpression>(expression, expectedNumberOfDesignVariables, true, true, printResult, tolerance,
                                        eps)
        .test();
}
