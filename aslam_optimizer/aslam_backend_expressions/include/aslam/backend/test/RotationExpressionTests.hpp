...
class ExpressionJacobianTestTraits<RotationExpression> {
  private:
    struct RotationExpressionNodeFunctor {
        ...
        RotationExpressionNodeFunctor(RotationExpression dv, Eigen::Vector3d p) : _p(p), _dv(dv) {}
  ...
  public:
    static void testJacobian(const ExpressionTester<RotationExpression>& expressionTester) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            sm::eigen::assertNear(
                Jc.asSparseMatrix(), Jest, expressionTester.getTolerance(), SM_SOURCE_FILE_POS,
