...
void EuclideanDirection::revertUpdateImplementation() { _C = _p_C; }
...
int EuclideanDirection::minimalDimensionsImplementation() const { return 2; }

EuclideanExpression EuclideanDirection::toExpression() { return EuclideanExpression(this); }

Eigen::Vector3d EuclideanDirection::toEuclideanImplementation() const { return _magnitude * _C.col(2); }
...
void EuclideanDirection::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
...
void EuclideanDirection::getParametersImplementation(Eigen::MatrixXd& value) const { value = _C; }

void EuclideanDirection::setParametersImplementation(const Eigen::MatrixXd& value) {
    _p_C = _C;
    _C = value;
}

void EuclideanDirection::minimalDifferenceImplementation(const Eigen::MatrixXd& xHat,
    ...
    SM_ASSERT_TRUE(aslam::InvalidArgumentException, (xHat.rows() == 3) && (xHat.cols() == 3),
                   "xHat does have incompatible dimensions");
    outDifference = sm::kinematics::R2AxisAngle(_C * xHat.transpose());
}

void EuclideanDirection::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& /* xHat */,
