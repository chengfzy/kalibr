...
namespace aslam {
namespace backend {
EuclideanPoint::EuclideanPoint(const Eigen::Vector3d& p) : _p(p), _p_p(p) {}
EuclideanPoint::~EuclideanPoint() {}

/// \brief Revert the last state update.
void EuclideanPoint::revertUpdateImplementation() { _p = _p_p; }
...
int EuclideanPoint::minimalDimensionsImplementation() const { return 3; }

Eigen::Vector3d EuclideanPoint::toEuclideanImplementation() const { return _p; }
...
void EuclideanPoint::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
...
EuclideanExpression EuclideanPoint::toExpression() { return EuclideanExpression(this); }

HomogeneousExpression EuclideanPoint::toHomogeneousExpression() {
    return EuclideanExpression(this).toHomogeneousExpression();
}

void EuclideanPoint::getParametersImplementation(Eigen::MatrixXd& value) const { value = _p; }

void EuclideanPoint::setParametersImplementation(const Eigen::MatrixXd& value) {
    _p_p = _p;
    _p = value;
}

void EuclideanPoint::minimalDifferenceImplementation(const Eigen::MatrixXd& xHat,
...
void EuclideanPoint::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat,
