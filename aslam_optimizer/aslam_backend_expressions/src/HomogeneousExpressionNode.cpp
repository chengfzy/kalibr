...
Eigen::Vector4d HomogeneousExpressionNode::toHomogeneous() const { return toHomogeneousImplementation(); }
...
void HomogeneousExpressionNode::evaluateJacobians(JacobianContainer& outJacobians,
...
HomogeneousExpressionNodeMultiply::HomogeneousExpressionNodeMultiply(
    boost::shared_ptr<TransformationExpressionNode> lhs, boost::shared_ptr<HomogeneousExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {
...
void HomogeneousExpressionNodeMultiply::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
                                                                        const Eigen::MatrixXd& applyChainRule) const {
...
HomogeneousExpressionNodeEuclidean::HomogeneousExpressionNodeEuclidean(boost::shared_ptr<EuclideanExpressionNode> p)
    : _p(p) {}
...
void HomogeneousExpressionNodeEuclidean::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
                                                                         const Eigen::MatrixXd& applyChainRule) const {
    _p->evaluateJacobians(outJacobians, applyChainRule * Eigen::MatrixXd::Identity(4, 3));
}

void HomogeneousExpressionNodeEuclidean::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
