...
namespace aslam {
namespace backend {
MappedHomogeneousPoint::MappedHomogeneousPoint(double* p) : _p(p), _p_p(_p) {
...
void MappedHomogeneousPoint::revertUpdateImplementation() { _p = _p_p; }
...
int MappedHomogeneousPoint::minimalDimensionsImplementation() const { return 3; }

Eigen::Vector4d MappedHomogeneousPoint::toHomogeneousImplementation() const { return _p; }
...
void MappedHomogeneousPoint::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
...
HomogeneousExpression MappedHomogeneousPoint::toExpression() { return HomogeneousExpression(this); }

void MappedHomogeneousPoint::getParametersImplementation(Eigen::MatrixXd& value) const { value = _p; }

void MappedHomogeneousPoint::setParametersImplementation(const Eigen::MatrixXd& value) {
    _p_p = _p;
    _p = value;
}

void MappedHomogeneousPoint::minimalDifferenceImplementation(const Eigen::MatrixXd& xHat,
                                                             Eigen::VectorXd& outDifference) const {
    SM_ASSERT_TRUE(aslam::InvalidArgumentException, (xHat.rows() == 4) && (xHat.cols() == 1),
                   "The dimension of xHat does not conform to a 4x1 quaternion vector");
    outDifference = sm::kinematics::qlog(sm::kinematics::qplus(_p_p, sm::kinematics::quatInv(xHat)));
}

void MappedHomogeneousPoint::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat,
