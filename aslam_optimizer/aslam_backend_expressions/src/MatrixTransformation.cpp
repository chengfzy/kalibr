...
MatrixTransformation::MatrixTransformation(const Eigen::Matrix3d& A) : _A(A), _A_a(A) {
    _UpdatePattern << 1, 1, 1, 1, 1, 1, 1, 1,
        1;                 // ## general case (if no pattern is defined: every entry of the matrix will be updated
    _UpdateDimension = 9;  // ## number of unknowns: 9 (3x3 Matrix)
}

MatrixTransformation::MatrixTransformation(const Eigen::Matrix3d& A, const Eigen::Matrix3d& UpdatePattern)
    : _A(A), _A_a(A) {
...
void MatrixTransformation::revertUpdateImplementation() { _A = _A_a; }
...
void MatrixTransformation::updateImplementation(const double* dp, int size) {
    ...
    int j = 0;
    for (int i = 0; i < 9; i++) {
        _A(i % 3, int(floor(static_cast<double>(i / 3)))) +=
            _UpdatePattern(i % 3, int(floor(static_cast<double>(i / 3)))) * dp[j];
        if (_UpdatePattern(i % 3, int(floor(static_cast<double>(i / 3)))) == 1) {
            j++;
        }
    }
...
// ##
Eigen::Matrix3d MatrixTransformation::toFullMatrixImplementation() { return _A; }
...
void MatrixTransformation::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
    ...
    for (int i = 0; i < 9; i++) {
        if (_UpdatePattern(i % 3, int(floor(static_cast<double>(i / 3)))) == 1) {
            SM_ASSERT_GT_DBG(aslam::Exception, _UpdateDimension, j,
                             "Incorrect update dimension! It doesn't match the pattern");
            finalJacobian.col(j) =
                applyChainRule.col(i);  // took out only the rows for the values, which are design variables
            j++;
        }
    }
...
MatrixExpression MatrixTransformation::toExpression() { return MatrixExpression(this); }
...
void MatrixTransformation::getParametersImplementation(Eigen::MatrixXd& value) const { value = _A; }

void MatrixTransformation::setParametersImplementation(const Eigen::MatrixXd& value) {
    _A_a = _A;
    _A = value;
}

void MatrixTransformation::minimalDifferenceImplementation(const Eigen::MatrixXd& xHat,
                                                           Eigen::VectorXd& outDifference) const {
    SM_ASSERT_TRUE(aslam::InvalidArgumentException, (xHat.rows() == 3) && (xHat.cols() == 3),
                   "xHat has incompatible dimensions");
    outDifference = sm::kinematics::R2AxisAngle(_A * xHat.transpose());
}

void MatrixTransformation::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat,
