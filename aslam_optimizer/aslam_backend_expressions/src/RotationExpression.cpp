...
RotationExpression::RotationExpression(boost::shared_ptr<RotationExpressionNode> node) : _root(node) {
    SM_ASSERT_TRUE(Exception, _root.get() != NULL,
                   "It is illegal to initialized a rotation expression with a null node");
}

/// \brief Evaluate the rotation matrix.
Eigen::Matrix3d RotationExpression::toRotationMatrix() { return _root->toRotationMatrix(); }
