...
RotationQuaternion::RotationQuaternion(const Eigen::Matrix3d& C)
    : _q(sm::kinematics::r2quat(C)), _p_q(sm::kinematics::r2quat(C)), _C(C) {}
...
Eigen::Matrix3d RotationQuaternion::toRotationMatrixImplementation() const { return _C; }
...
void RotationQuaternion::evaluateJacobiansImplementation(JacobianContainer& outJacobians,
                                                         const Eigen::MatrixXd& applyChainRule) const {
    outJacobians.add(const_cast<RotationQuaternion*>(this), applyChainRule);
}

RotationExpression RotationQuaternion::toExpression() { return RotationExpression(this); }
...
void RotationQuaternion::getParametersImplementation(Eigen::MatrixXd& value) const { value = _q; }

void RotationQuaternion::setParametersImplementation(const Eigen::MatrixXd& value) {
    _p_q = _q;
    _q = value;
    _C = sm::kinematics::quat2r(_q);
}

void RotationQuaternion::minimalDifferenceImplementation(const Eigen::MatrixXd& xHat,
...
void RotationQuaternion::minimalDifferenceAndJacobianImplementation(const Eigen::MatrixXd& xHat,
