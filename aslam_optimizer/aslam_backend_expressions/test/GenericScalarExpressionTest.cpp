...
struct GenericScalarExpressionNodeTestSuites : public ::testing::Test {
    ...
    static PrimScalar getRandScalar() { return PrimScalar(sm::random::rand() * 10.0); }
};

typedef ::testing::Types<GenericScalar<FixedPointNumber<int, long(1E6)>>,
                         GenericScalar<FixedPointNumber<long long, long(1E9)>>, GenericScalar<double>>
    Types;
...
TYPED_TEST(GenericScalarExpressionNodeTestSuites, testScalarProductWithConst) {
    try {
        ...
        SCOPED_TRACE("");
        testExpression(point1.toExpression() * p2, 1, false,
                       test::ExpressionTraits<typename TestFixture::TestGenericScalarExpression>::defaultTolerance(),
                       1);
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TYPED_TEST(GenericScalarExpressionNodeTestSuites, testCast) {
    try {
        ...
        SCOPED_TRACE("");
        testExpression(typename TestFixture::TestGenericScalarExpression(
                           getRandScalar<FixedPointNumber<long long, long(1E9)>>().toExpression()),
                       1, false, test::ExpressionTraits<GenericScalarExpression<long long>>::defaultTolerance(),
                       test::ExpressionTraits<GenericScalarExpression<long long>>::defaulEps());
        SCOPED_TRACE("");
        testExpression(typename TestFixture::TestGenericScalarExpression(
                           getRandScalar<FixedPointNumber<int, long(1E6)>>().toExpression()),
                       1, false, test::ExpressionTraits<GenericScalarExpression<int>>::defaultTolerance(),
                       test::ExpressionTraits<GenericScalarExpression<int>>::defaulEps());
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
}
...
TYPED_TEST(GenericScalarExpressionNodeTestSuites, testOpsFailure) {
    if (std::numeric_limits<typename TestFixture::PrimScalar>::is_integer ||
        is_fixed_point_number<typename TestFixture::PrimScalar>::value)
        return;
