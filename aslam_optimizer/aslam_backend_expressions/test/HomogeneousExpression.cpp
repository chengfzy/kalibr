...
class HpErr : public aslam::backend::ErrorTermFs<4> {
  ...
  protected:
    ...
    virtual void evaluateJacobiansImplementation(JacobianContainer& jacobians) { _p.evaluateJacobians(jacobians); }
};
...
struct HomogeneousExpressionNodeFunctor {
    ...
    HomogeneousExpressionNodeFunctor(HomogeneousExpression dv) : _dv(dv) {}
...
TEST(HomogeneousExpressionNodeTestSuites, testSimpleError) {
    try {
        ...
        SCOPED_TRACE("");
        sm::eigen::assertNear(J.asSparseMatrix(), estJ.asSparseMatrix(), 1e-6, SM_SOURCE_FILE_POS,
...
TEST(HomogeneousExpressionNodeTestSuites, testTransformedPoint) {
    try {
        ...
        sm::eigen::assertNear(Tp.toHomogeneous(), T.toTransformationMatrix() * p.toHomogeneous(), 1e-14,
...
TEST(HomogeneousExpressionNodeTestSuites, testTransformedInverse) {
    try {
        ...
        sm::eigen::assertNear(Tp.toHomogeneous(), T.toTransformationMatrix().inverse() * p.toHomogeneous(), 1e-14,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(HomogeneousExpressionNodeTestSuites, testTransformationExpression1) {
    try {
        ...
        sm::eigen::assertNear(T01p.toHomogeneous(),
                              T0.toTransformationMatrix() * T1.toTransformationMatrix() * p.toHomogeneous(), 1e-14,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(HomogeneousExpressionNodeTestSuites, testTransformationExpression3) {
    try {
        ...
        sm::eigen::assertNear(T10p.toHomogeneous(),
                              (T0.toTransformationMatrix() * T1.toTransformationMatrix()).inverse() * p.toHomogeneous(),
