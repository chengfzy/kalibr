...
TEST(EuclideanExpressionNodeTestSuites, testRotatedPoint) {
    try {
        ...
        sm::eigen::assertNear(Cp.toEuclidean(), C.toRotationMatrix() * p.toEuclidean(), 1e-14, SM_SOURCE_FILE_POS,
                              "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
}
...
TEST(EuclideanExpressionNodeTestSuites, testRotatedInverse) {
    try {
        ...
        sm::eigen::assertNear(Cp.toEuclidean(), C.toRotationMatrix().transpose() * p.toEuclidean(), 1e-14,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
}
...
TEST(EuclideanExpressionNodeTestSuites, testRotationExpression1) {
    try {
        ...
        sm::eigen::assertNear(C01p.toEuclidean(), C0.toRotationMatrix() * C1.toRotationMatrix() * p.toEuclidean(),
                              1e-14, SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
}
...
TEST(EuclideanExpressionNodeTestSuites, testNegation) {
    try {
        ...
        sm::eigen::assertNear(-point1.toEuclidean(), pNegated.toEuclidean(), 1e-14, SM_SOURCE_FILE_POS,
                              "Testing the result is unchanged");
...
TEST(EuclideanExpressionNodeTestSuites, testMatrixTransformedPoint) {
    try {
        ...
        sm::eigen::assertNear(Ap.toEuclidean(), A.toMatrix3x3() * p.toEuclidean(), 1e-14, SM_SOURCE_FILE_POS,
                              "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(EuclideanExpressionNodeTestSuites, testMatrixUpdate) {
    try {
        ...
        updatePattern << 1, 1, 1, 0, 1, 1, 0, 0, 1;
        ...
        updateMatrix << .1, .2, .3, 0., .4, .5, 0., 0., .6;

        SCOPED_TRACE("");

        a.updateImplementation(update, 6);

        sm::eigen::assertNear(a.toMatrix3x3(), dataMatrix + updateMatrix, 1e-14, SM_SOURCE_FILE_POS,
                              "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(EuclideanExpressionNodeTestSuites, testMatrixdoubleTransformedPoint) {
    try {
        ...
        sm::eigen::assertNear(Ap.toEuclidean(), B.toMatrix3x3() * (A.toMatrix3x3() * p.toEuclidean()), 1e-14,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(EuclideanExpressionNodeTestSuites, testLowerMixedMatrixTransformedPoint) {
    try {
        ...
        sm::eigen::assertNear(Ap.toEuclidean(), B.toMatrix3x3() * A.toMatrix3x3() * p.toEuclidean(), 1e-13,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(EuclideanExpressionNodeTestSuites, testTransformationTransformedPoint) {
    try {
        ...
        testJacobian(p);
        SCOPED_TRACE("");
        sm::eigen::assertNear(p.toEuclidean(), A.toTransformationMatrix().topRightCorner<3, 1>(), 1e-14,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
...
TEST(EuclideanExpressionNodeTestSuites, testRotationParameters) {
    try {
        ...
        testJacobian(p);
        SCOPED_TRACE("");
        sm::eigen::assertNear(p.toEuclidean(), ypr->rotationMatrixToParameters(qr.toRotationMatrix()), 1e-14,
                              SM_SOURCE_FILE_POS, "Testing the result is unchanged");
    } catch (std::exception const& e) {
        FAIL() << e.what();
    }
