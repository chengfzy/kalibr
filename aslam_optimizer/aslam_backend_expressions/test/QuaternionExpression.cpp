...
template <typename Primitve_>
constexpr double getTolerance() {
    return 1e-14;
}

template <>
constexpr double getTolerance<float>() {
    return 1e-5;
}

#define testExp(exp, numVars)                         \
    {                                                 \
        SCOPED_TRACE("testJacobian(" #exp ")");       \
        aslam::backend::testExpression(exp, numVars); \
    }
...
void testQuaternionBasics() {
    ...
    typename QE::vector_t val = QE::value_t::Random(), val2 = QE::value_t::Random(), valUnit = QE::value_t::Random(),
                          valUnit2 = QE::value_t::Random();
    ...
    for (int i = 0; i < 3; i++)
        sm::eigen::assertNear((ImaginaryBases[i] * ImaginaryBases[(i + 1) % 3]).evaluate(),
                              ImaginaryBases[(i + 2) % 3].evaluate() * (QE::IsTraditionalMultOrder ? 1 : -1), tolerance,
                              SM_SOURCE_FILE_POS, "Testing first Hamiltonian equation.");

    sm::eigen::assertNear((qExp + qExp2).evaluate(), val + val2, tolerance, SM_SOURCE_FILE_POS, "Testing +.");
    sm::eigen::assertNear((qExp - qExp2).evaluate(), val - val2, tolerance, SM_SOURCE_FILE_POS, "Testing -.");
    sm::eigen::assertNear((qExp + qExp.conjugate()).evaluate(),
                          2 * val(aslam::backend::quaternion::internal::isRealFirst(EMode) ? 0 : 3) * one.evaluate(),
                          tolerance, SM_SOURCE_FILE_POS, "Testing an important conjugate equality.");
    sm::eigen::assertNear((qExp * qExp.inverse()).evaluate(), one.evaluate(), tolerance, SM_SOURCE_FILE_POS,
                          "Testing the important inverse equality.");
    sm::eigen::assertNear((qExpUnit * qExpUnit.inverse()).evaluate(), one.evaluate(), tolerance, SM_SOURCE_FILE_POS,
                          "Testing the important inverse equality.");

    if (EMode == QuaternionMode::LAST_IS_REAL_AND_OPPOSITE_MULT_ORDER) {
        using namespace sm::kinematics;
        sm::eigen::assertNear((qExp * qExp2).evaluate(),
                              qplus(val.template cast<double>(), val2.template cast<double>()).template cast<TScalar>(),
                              tolerance, SM_SOURCE_FILE_POS, "Testing conformance with qplus implementation.");
        const double n = val.dot(val);
        sm::eigen::assertNear(qExp.inverse().evaluate(),
                              quatInv(val.template cast<double>()).template cast<TScalar>() / n, tolerance,
                              SM_SOURCE_FILE_POS, "Testing conformance with quatInv implementation.");
        sm::eigen::assertNear(qExpUnit.inverse().evaluate(),
                              quatInv(valUnit.template cast<double>()).template cast<TScalar>(), tolerance,
                              SM_SOURCE_FILE_POS, "Testing conformance with quatInv implementation.");
    }
    ...
    sm::eigen::assertNear(qExpUnit.rotate3Vector(vec3dGV).evaluate(),
                          qcalc::getImagPart(qcalc::quatMult(qcalc::quatMult(valUnit, vec3d), qcalc::invert(valUnit))),
                          tolerance, SM_SOURCE_FILE_POS,
                          "Testing conformance with quaternion rotation implementation.");
