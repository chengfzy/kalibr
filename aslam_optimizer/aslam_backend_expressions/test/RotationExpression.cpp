...
TEST(RotationExpressionNodeTestSuites, testQuatInverse2) {
    try {
        ...
        sm::eigen::assertNear(invQ1, invQ2, 1e-14, SM_SOURCE_FILE_POS,
                              "Test that the inverse computes the correct result");
    } catch (const std::exception& e) {
        FAIL() << e.what();
    }
...
TEST(RotationExpressionNodeTestSuites, testQuatMultiply2) {
    try {
        ...
        sm::eigen::assertEqual(qr0.toRotationMatrix() * qr1.toRotationMatrix(), dvMultiply.toRotationMatrix(),
                               SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, testQuatMultiply3a) {
    try {
        ...
        sm::eigen::assertNear(qr2.toRotationMatrix() * qr0.toRotationMatrix() * qr1.toRotationMatrix(),
                              dvMultiply.toRotationMatrix(), 1e-14, SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, stQuatMulty3b) {
    try {
        ...
        sm::eigen::assertNear(dvMultiply02Left.toRotationMatrix(), dvMultiply02Right.toRotationMatrix(), 1e-14,
                              SM_SOURCE_FILE_POS, "Test multiplications");
        ...
        sm::eigen::assertNear(JcLeft.asSparseMatrix(), JcRight.asSparseMatrix(), 1e-14, SM_SOURCE_FILE_POS,
...
TEST(RotationExpressionNodeTestSuites, stQuatMulty3c) {
    try {
        ...
        sm::eigen::assertNear(qr0.toRotationMatrix() * qr1.toRotationMatrix() * qr2.toRotationMatrix(),
                              dvMultiplyRight.toRotationMatrix(), 1e-14, SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, stQuatMultInv1) {
    try {
        ...
        sm::eigen::assertNear(qr2.toRotationMatrix() * (qr0.toRotationMatrix() * qr1.toRotationMatrix()).transpose(),
                              dvMultiplyRight210.toRotationMatrix(), 1e-14, SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, stQuatMultInv2) {
    try {
        ...
        sm::eigen::assertNear(qr2.toRotationMatrix().transpose() * (qr0.toRotationMatrix() * qr1.toRotationMatrix()),
                              dvMultiplyRight210.toRotationMatrix(), 1e-14, SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, testMinimalDifferenceAndJacobian) {
    try {
        ...
        quatHat(0, 0) = quatInitial(0);
        quatHat(1, 0) = quatInitial(1);
        quatHat(2, 0) = quatInitial(2, 0);
        quatHat(3, 0) = quatInitial(3, 0);

        Eigen::MatrixXd params;
        quat.getParameters(params);
        Eigen::Vector4d quatBar;
        quatBar(0) = params(0, 0);
        quatBar(1) = params(1, 0);
        quatBar(2) = params(2, 0);
        quatBar(3) = params(3, 0);
        ...
        Eigen::Vector3d epsV;
        epsV.setRandom();
        epsV = epsV * 0.05;  // epsV(0) = 0.001; epsV(1) = 0.001; epsV(2) = 0.001;
        ...
        quatFinal(0) = params2(0, 0);
        quatFinal(1) = params2(1, 0);
        quatFinal(2) = params2(2, 0);
        quatFinal(3) = params2(3, 0);

        Eigen::Vector3d realMinDist =
            sm::kinematics::qlog(sm::kinematics::qplus(quatFinal, sm::kinematics::quatInv(quatInitial)));
...
TEST(RotationExpressionNodeTestSuites, testVector2RotationQuaternionExpressionAdapter) {
    try {
        ...
        sm::eigen::assertEqual(qr1.toRotationMatrix(), quat1Var.toRotationMatrix(), SM_SOURCE_FILE_POS,
                               "Test multiplications with adapter");
        {
            SCOPED_TRACE("");
            testJacobian(qr1);
        }
        ...
        RotationExpression dvMultiply = qr0 * qr1;
        sm::eigen::assertEqual(qr0.toRotationMatrix() * qr1.toRotationMatrix(), dvMultiply.toRotationMatrix(),
                               SM_SOURCE_FILE_POS, "Test multiplications with adapter");
        {
            SCOPED_TRACE("");
            testJacobian(dvMultiply);
        }
