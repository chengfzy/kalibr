...
void exportDesignVariableAdapter(const std::string& name) {
    ...
    class_<DesignVariableAdapter<dv_t>, boost::shared_ptr<DesignVariableAdapter<dv_t> >, bases<DesignVariable> >(
        name.c_str(), init<boost::shared_ptr<dv_t> >())
    ...
    // boost::python::implicitly_convertible<boost::shared_ptr<DesignVariableAdapter<dv_t> >,
    // boost::shared_ptr<DesignVariable> >();
}
