...
void exportDesignVariableMappedVector() {
    ...
    class_<DesignVariableMappedVector<D>, boost::shared_ptr<DesignVariableMappedVector<D> >, bases<DesignVariable> >(
        str.str().c_str(), no_init)
...
void exportDesignVariableVector() {
    ...
    class_<DesignVariableVector<D>, boost::shared_ptr<DesignVariableVector<D> >, bases<DesignVariable> >(
        str.str().c_str(), no_init)
...
void exportVectorExpression() {
    ...
    // class_< VectorExpression<D>, boost::shared_ptr<VectorExpression<D> >("VectorExpression",
    // init<boost::shared_ptr<VectorExpressionNode<D> > >() ) .def("value", &DesignVariableVector<D>::value,
    // return_value_policy<copy_const_reference>()) .def("toExpression", &DesignVariableVector<D>::toExpression)
    // ;

    class_<VectorExpression<D>, boost::shared_ptr<VectorExpression<D> > >(
        "VectorExpression", init<boost::shared_ptr<VectorExpressionNode<D> > >())
...
void exportBackendExpressions() {
    class_<EuclideanExpression, boost::shared_ptr<EuclideanExpression> >(
        "EuclideanExpression", init<boost::shared_ptr<EuclideanExpressionNode> >())
    ...
    class_<HomogeneousExpression, boost::shared_ptr<HomogeneousExpression> >(
        "HomogeneousExpression", init<boost::shared_ptr<HomogeneousExpressionNode> >())
    ...
    class_<TransformationExpression, boost::shared_ptr<TransformationExpression> >(
        "TransformationExpression", init<boost::shared_ptr<TransformationExpressionNode> >())
    ...
    class_<RotationExpression, boost::shared_ptr<RotationExpression> >(
        "RotationExpression", init<boost::shared_ptr<RotationExpressionNode> >())
    ...
    // class_<EuclideanPoint, boost::shared_ptr<EuclideanPoint>, bases<DesignVariable> >("EuclideanPointDv", init<const
    // Eigen::Vector3d>())
    //   .def("toExpression", &EuclideanPoint::toExpression)
    //   .def("toEuclidean", &EuclideanPoint::toEuclidean);
    // ;

    class_<RotationQuaternion, boost::shared_ptr<RotationQuaternion>, bases<DesignVariable> >(
        "RotationQuaternionDv", init<const Eigen::Vector4d>())
    ...
    class_<EuclideanPoint, boost::shared_ptr<EuclideanPoint>, bases<DesignVariable> >("EuclideanPointDv",
                                                                                      init<const Eigen::Vector3d>())
        .def("toExpression", &EuclideanPoint::toExpression)
    ...
    class_<HomogeneousPoint, boost::shared_ptr<HomogeneousPoint>, bases<DesignVariable> >("HomogeneousPointDv",
                                                                                          init<const Eigen::Vector4d>())
    ...
    class_<TransformationBasic, boost::shared_ptr<TransformationBasic>, bases<DesignVariable> >(
        "TransformationBasicDv", init<RotationExpression, EuclideanExpression>())
    ...
    class_<MatrixExpression, boost::shared_ptr<MatrixExpression> >("MatrixExpression",
                                                                   init<boost::shared_ptr<MatrixExpressionNode> >())
        .def("toMatrix3x3", &MatrixExpression::toMatrix3x3)
        .def(self * euclideanExpression)
        .def("getDesignVariables", &getDesignVariables<MatrixExpression>);
    class_<MatrixBasic, boost::shared_ptr<MatrixBasic>, bases<DesignVariable> >(
        "MatrixBasicDv", init<const Eigen::Matrix3d, const Eigen::Matrix3i>())
    ...
    class_<ScalarExpression, boost::shared_ptr<ScalarExpression> >("ScalarExpression",
                                                                   init<boost::shared_ptr<ScalarExpressionNode> >())
        .def(init<double>())
    ...
    class_<EuclideanDirection, boost::shared_ptr<EuclideanDirection>, bases<DesignVariable> >(
        "EuclideanDirection", init<const Eigen::Vector3d&>("EuclideanDirection(Vector3d p)"))
