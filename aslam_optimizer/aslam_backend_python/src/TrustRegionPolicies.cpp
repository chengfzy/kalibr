...
void exportTrustRegionPolicies() {
    ...
    // GN
    class_<GaussNewtonTrustRegionPolicy, boost::shared_ptr<GaussNewtonTrustRegionPolicy>, bases<TrustRegionPolicy>,
           boost::noncopyable>("GaussNewtonTrustRegionPolicy", init<>());

    // LM
    class_<LevenbergMarquardtTrustRegionPolicy, boost::shared_ptr<LevenbergMarquardtTrustRegionPolicy>,
           bases<TrustRegionPolicy>, boost::noncopyable>("LevenbergMarquardtTrustRegionPolicy", init<>())
        .def(init<double>("LevenbergMarquardtTrustRegionPolicy( double initalLambda )"));

    // DL
    class_<DogLegTrustRegionPolicy, boost::shared_ptr<DogLegTrustRegionPolicy>, bases<TrustRegionPolicy>,
           boost::noncopyable>("DogLegTrustRegionPolicy", init<>());
}
