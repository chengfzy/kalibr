...
template <class MatrixType>
SparseBlockMatrix<MatrixType>::SparseBlockMatrix(const Eigen::VectorXi& rbi, const Eigen::VectorXi& cbi,
                                                 bool hasStorage)
...
template <class MatrixType>
SparseBlockMatrix<MatrixType>::SparseBlockMatrix(const std::vector<int>& rbi, const std::vector<int>& cbi,
                                                 bool hasStorage)
    : _rowBlockIndices(&rbi[0], (&rbi[0]) + rbi.size()),
      _colBlockIndices(&cbi[0], (&cbi[0]) + cbi.size()),
      _blockCols(cbi.size()),
...
template <class MatrixType>
SparseBlockMatrix<MatrixType>::SparseBlockMatrix(SparseBlockMatrix&& source)
...
void SparseBlockMatrix<MatrixType>::cloneInto(SparseBlockMatrix<MatrixType>& ret) const {
    if (&ret != this) {
        ret.clear(true);
        ret = SparseBlockMatrix<MatrixType>(&_rowBlockIndices[0], &_colBlockIndices[0], _rowBlockIndices.size(),
...
template <class MatrixType>
SparseBlockMatrix<typename SparseBlockMatrix<MatrixType>::TransposedMatrixType>
SparseBlockMatrix<MatrixType>::transpose() const {
    SparseBlockMatrix<TransposedMatrixType> ret(&_colBlockIndices[0], &_rowBlockIndices[0], _colBlockIndices.size(),
...
SparseBlockMatrix<Eigen::MatrixXd> SparseBlockMatrix<MatrixType>::operator*(
    const SparseBlockMatrix<MatrixFactorType>& M) const {
    SparseBlockMatrix<Eigen::MatrixXd> ret(&_rowBlockIndices[0], &(M._colBlockIndices[0]), _rowBlockIndices.size(),
...
template <class MatrixType>
void SparseBlockMatrix<MatrixType>::sliceInto(int rmin, int rmax, int cmin, int cmax,
                                              SparseBlockMatrix& outMatrix) const {
    outMatrix.clear();
        ...
        for (IntType c = 0; c < csize; c++) {
            ...
            *Cp = nz;
            ...
            for (typename SparseBlockMatrix<MatrixType>::IntBlockMap::const_iterator it = _blockCols[i].begin();
                ...
                if (rstart == cstart) elemsToCopy = c + 1;  // do not copy the symmetric parts
                for (int r = 0; r < elemsToCopy; ++r) {
