...
template <typename T>
bool writeCCSMatrix(const char* filename, int rows, int cols, const T* Ap, const T* Ai, const double* Ax,
    ...
    for (int i = 0; i < cols; i++) {
        ...
        for (int j = rbeg; j < rend; j++) {
            ...
            if (upperTriangleSymmetric && Ai[j] != i) entries.push_back(TripletEntry(i, Ai[j], Ax[j]));
        }
    }
