...
class LinearSolverCholmod : public LinearSolver<MatrixType> {
  public:
    ...
    bool solve(const SparseBlockMatrix<MatrixType>& A, double* x, double* b) {
        ...
        fillCholmodExt(A, _cholmodFactor);  // _cholmodFactor used as bool, if not existing will copy the whole
        ...
        if (_cholmodCommon.status == CHOLMOD_NOT_POSDEF) {
            ...
            // writeCCSMatrix("debug.txt", _cholmodSparse->nrow, _cholmodSparse->ncol, (int*)_cholmodSparse->p,
  ...
  protected:
    ...
    void computeSymbolicDecomposition(const SparseBlockMatrix<MatrixType>& A) {
        ...
        if (!_blockOrdering) {
            ...
            if (_scalarPermutation.size() == 0) _scalarPermutation.resize(_cholmodSparse->ncol);
        ...
        //<< (_cholmodCommon.method[bestIdx].ordering == CHOLMOD_GIVEN ? "block" : "scalar") << " AMD ordering " <<
        // endl;
    }
