...
class LinearSolverQr : public LinearSolver<MatrixType> {
  public:
    LinearSolverQr() : LinearSolver<MatrixType>() {
    ...
    virtual ~LinearSolverQr() {
        ...
        if (_cholmodFactor) SuiteSparseQR_free(&_cholmodFactor, &_cholmodCommon);
    ...
    bool solve(const SparseBlockMatrix<MatrixType>& A, double* x, double* b) {
        ...
        //	writeCCSMatrix<long>("debug.txt", _cholmodSparse->nrow, _cholmodSparse->ncol, (long*)_cholmodSparse->p,
        //(long*)_cholmodSparse->i, (double*)_cholmodSparse->x, false);
        ...
        // try QR only:
        /*  SuiteSparseQR_factorization <double> *QR = SuiteSparseQR_factorize <double>(0, SPQR_DEFAULT_TOL,
        _cholmodSparse, cc) ;
        ...
        if (cc->status != CHOLMOD_OK) return false;
    ...
    void fillCholmodExt(const SparseBlockMatrix<MatrixType>& A, bool onlyValues) {
        ...
        if (_cholmodSparse->columnsAllocated < n) {
            ...
            _cholmodSparse->columnsAllocated =
                _cholmodSparse->columnsAllocated == 0 ? n : 2 * n;  // pre-allocate more space if re-allocating
        ...
        if (!onlyValues) {
            ...
            if (_cholmodSparse->nzmax < nzmax) {
                ...
                _cholmodSparse->nzmax =
                    _cholmodSparse->nzmax == 0 ? nzmax : 2 * nzmax;  // pre-allocate more space if re-allocating
        ...
        if (onlyValues) {
            ...
            A.template fillUpperTriangleCCS<long>((long*)_cholmodSparse->p, (long*)_cholmodSparse->i,
