...
class SparseBlockMatrix {
  public:
    ...
    //! returns the block at location r,c. if alloc=true he block is created if it does not exist and the values are set
    //! to 0
    ...
    //! assign operator
    inline SparseBlockMatrix& operator=(const SparseBlockMatrix& source) {
    ...
    inline SparseBlockMatrix& operator*=(double a) {
        scale(a);
        return *this;
    }
    // copy and scale as operator
    inline SparseBlockMatrix operator*(double a) const {
        SparseBlockMatrix ret(*this);
        ret.scale(a);
        return ret;
    }
  ...
  protected:
    ...
    template <typename M>
    friend class SparseBlockMatrix;
};
