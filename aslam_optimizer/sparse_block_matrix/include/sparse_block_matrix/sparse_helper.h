 ...
 */
template <typename T>
bool writeCCSMatrix(const char* filename, int rows, int cols, const T* p, const T* i, const double* v,
                    bool upperTriangleSymmetric = true);

// bool writeCCSMatrix(const char* filename, int rows, int cols, const int* Ap, const int* Ai, const double* Ax, bool
// upperTriangleSymmetric = true);
...
struct CholmodExt : public cholmod_sparse {
    ...
    ~CholmodExt() {
        if (p) delete[](INTTYPE*) p;
        p = 0;
        if (x) delete[](double*) x;
        x = 0;
        if (i) delete[](INTTYPE*) i;
        i = 0;
    }
