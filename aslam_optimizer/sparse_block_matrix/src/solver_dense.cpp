...
#define ALLOC_DENSE(s, p, l)                                                             \
    if (1) {                                                                             \
        ...
        DIM_TO_SOLVER(p, l)::LinearSolverType* linearSolver =                            \
            new LinearSolverDense<DIM_TO_SOLVER(p, l)::PoseMatrixType>();                \
...
void __attribute__((constructor)) init_solver_csparse() {
    ...
    factory->registerSolver(
        new DenseSolverCreator(SolverProperty("dense", "Dense solver (variable blocksize)", "Dense", false, -1, -1)));
    factory->registerSolver(
        new DenseSolverCreator(SolverProperty("dense3_2", "Dense solver (fixed blocksize)", "Dense", true, 3, 2)));
    factory->registerSolver(
        new DenseSolverCreator(SolverProperty("dense6_3", "Dense solver (fixed blocksize)", "Dense", true, 6, 3)));
    factory->registerSolver(
        new DenseSolverCreator(SolverProperty("dense7_3", "Dense solver (fixed blocksize)", "Dense", true, 7, 3)));
}
