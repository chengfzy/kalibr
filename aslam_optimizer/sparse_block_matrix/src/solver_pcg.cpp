...
#define ALLOC_PCG(s, p, l)                                                             \
    if (1) {                                                                           \
        ...
        DIM_TO_SOLVER(p, l)::LinearSolverType* linearSolver =                          \
            new LinearSolverPCG<DIM_TO_SOLVER(p, l)::PoseMatrixType>();                \
...
void __attribute__((constructor)) init_solver_csparse() {
    ...
    factory->registerSolver(new PCGSolverCreator(SolverProperty(
        "pcg", "PCG solver using block-Jacobi pre-conditioner (variable blocksize)", "PCG", false, -1, -1)));
    factory->registerSolver(new PCGSolverCreator(SolverProperty(
        "pcg3_2", "PCG solver using block-Jacobi pre-conditioner (fixed blocksize)", "PCG", true, 3, 2)));
    factory->registerSolver(new PCGSolverCreator(SolverProperty(
        "pcg6_3", "PCG solver using block-Jacobi pre-conditioner (fixed blocksize)", "PCG", true, 6, 3)));
    factory->registerSolver(new PCGSolverCreator(SolverProperty(
        "pcg7_3", "PCG solver using block-Jacobi pre-conditioner (fixed blocksize)", "PCG", true, 7, 3)));
}
