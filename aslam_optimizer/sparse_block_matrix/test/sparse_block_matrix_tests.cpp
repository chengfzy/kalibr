...
template <typename M>
sparse_block_matrix::SparseBlockMatrix<M> buildRandomMatrix(Eigen::MatrixXi rows, Eigen::MatrixXi cols,
                                                            double probabilityOfABlockBeingFilled) {
    typedef M matrix_t;
...
TEST(sparse_block_matrixTestSuite, testTranspose) {
    ...
    rows << 2, 4, 6, 8;
    cols << 1, 2, 3;
    sparse_block_matrix::SparseBlockMatrix<Matrix<double, 2, 1> > M3 =
        buildRandomMatrix<Matrix<double, 2, 1> >(rows, cols, 0.5);
    {
        SCOPED_TRACE("");
...
TEST(sparse_block_matrixTestSuite, testEigenVectorMultiplication) {
    ...
    for (int i = 0; i < src3.rows(); i++) src4[i] = src3(i);
    ...
    sm::eigen::assertNear(dstDense3.transpose(), dstp2Map2, 1e-6, SM_SOURCE_FILE_POS);
